...
class Atas_model extends CI_Model
{
     ...
     */
     public function getAllitemPlanosProjetoSetorUser($setor, $usuario_setor)
    {
        $usuario_atual = $this->session->userdata('user_id');
        $users_dados = $this->site->geUserByID($usuario_atual);
        $projeto_atual = $users_dados->projeto_atual;
        
        $statement = "select p.idplanos, p.data_termino as data_termino, p.data_retorno_usuario,p.data_entrega_demanda, p.descricao as descricao, i.descricao as item, e.nome_evento as eventos,
                    u.username, p.status, s.nome as setor
                    from sig_planos p
                    ...
                    inner join sig_setores s on s.id = us.setores_id
                    where p.projeto = $projeto_atual and s.id = $setor and u.id = $usuario_setor";
     ...
     public function getAllUserPlanosProjetoSetor($setor)
    {
        ...
        $statement = "select distinct u.id as id, u.first_name as nome
                    ...
                    inner join sig_users_setor us on us.users_id = p.responsavel
                    inner join sig_setores s on s.id = us.setores_id
                    where projeto = $projeto_atual and s.id = $setor";
        $q = $this->db->query($statement);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
     ...
     public function getAllitemPlanosProjetoSetorCont( $setor)
    {
        ...
        $statement = "select count(*) as quantidade
                    ...
                    inner join sig_users_setor us on us.users_id = p.responsavel
                    inner join sig_setores s on s.id = us.setores_id
                    where projeto = $projeto_atual and s.id = $setor and status in ('CONCLUÍDO','PENDENTE', 'AGUARDANDO VALIDAÇÃO')";
         $q = $this->db->query($statement);
     ...
     public function getAllitemPlanosProjetoSetorContConcluido($setor)
    {
       $usuario = $this->session->userdata('user_id');
        ...
        $statement = "select count(*) as quantidade
                    ...
                    inner join sig_users_setor us on us.users_id = p.responsavel
                    inner join sig_setores s on s.id = us.setores_id
                    where projeto = $projeto_atual and s.id = $setor and status = 'CONCLUÍDO'";
         $q = $this->db->query($statement);
     ...
     public function getAllitemPlanosProjetoSetorContPendente($status, $setor)
    {
        ...
        $statement = "select count(*) as quantidade
                    ...
                    inner join sig_users_setor us on us.users_id = p.responsavel
                    inner join sig_setores s on s.id = us.setores_id
                    where projeto = $projeto_atual and s.id = $setor and status = 'PENDENTE' ";
     ...
     public function getAllitemPlanosProjetoSetorContAguardandoValidacao($setor)
    {
        ...
        $statement = "select count(*) as quantidade
                    ...
                    inner join sig_users_setor us on us.users_id = p.responsavel
                    inner join sig_setores s on s.id = us.setores_id
                    where projeto = $projeto_atual and s.id = $setor and status = 'AGUARDANDO VALIDAÇÃO'";
...
order by su.nome asc
     */
    public function getAllSetorArea($id_area)
    {
        ...
        $statement = "select distinct s.id as id_setor, s.nome as descricao from sig_planos p
                    inner join sig_users_setor us on us.users_id = p.responsavel
                    inner join sig_setores s      on s.id = us.setores_id
                    where projeto = $projeto_atual and s.pai = $id_area ";
        $q = $this->db->query($statement);
