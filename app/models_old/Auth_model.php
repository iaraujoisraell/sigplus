...
class Auth_model extends CI_Model
{
    ...
    /*  login usado para qdo o login é pelo AD
    public function login_ad_ol($identity, $password, $remember = FALSE)
    {
       // $identity = 'uni11659';
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                ...
                if(!$user->empresa_id){
                    ...
                    $logdata = array('date' => date('Y-m-d H:i:s'), 'type' => 'LOGIN SEM SUCESSO - USUÁRIO SEM EMPRESA', 'description' => 'O usuário não conseguiu reaizar o login pois não está ligado a uma empresa.',  'userid' => $user->id, 'ip_address' => $this->input->ip_address(), 'tabela' => 'users', 'funcao' => 'login',  'empresa' => $sigplus);
                    $this->db->insert('logs', $logdata);
                    return FALSE;
                }
                //VERIFICA SE A EMPRESA DELE ESTÁ ATIVA
             ...
             //  echo $user->empresa_id;
              ...
              //  exit;
                if($status_empresa != 1){
                    $this->trigger_events('post_login_unsuccessful');
                    $this->set_error('Procure o Adminstrador do SIG PLUS!');
                    $logdata = array('date' => date('Y-m-d H:i:s'), 'type' => 'LOGIN SEM SUCESSO - EMPRESA INATIVA', 'description' => 'O usuário não conseguiu reaizar o login pois a empresa não está ativa.',  'userid' => $user->id, 'ip_address' => $this->input->ip_address(), 'tabela' => 'users', 'funcao' => 'login',  'empresa' => $sigplus);
                    $this->db->insert('logs', $logdata);
                    return FALSE;
                }
                $this->set_session($user);
                ...
                $this->trigger_events(array('post_login', 'post_login_successful'));
                $this->set_message('login_successful');
                   $logdata = array('date' => date('Y-m-d H:i:s'), 'type' => 'LOGIN COM SUCESSO ', 'description' => 'O usuário realizou o login.',  'userid' => $user->id, 'ip_address' => $this->input->ip_address(), 'tabela' => 'users', 'funcao' => 'login',  'empresa' => $this->session->userdata('empresa'));
                  $this->db->insert('logs', $logdata);
                return TRUE;
            }
            // se a senhar não for a verdadeira e errar 3X espera por 10 min
        }
