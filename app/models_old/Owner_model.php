...
FROM sig_planos p where projeto = $projeto_atual_id and p.empresa = $empresa and status = 'PENDENTE' and data_termino > NOW() ";
     ...
     **************************************************************************/
     ...
     public function getAllUsersByEmpresa(){
       ...
       $statement = "SELECT u.id as id, first_name, s.nome as setor, u.email, u.active FROM sig_users u"
               . " inner join sig_users_setor us on us.users_id = u.id "
               . " inner join sig_setores s on s.id = us.setores_id "
               . " where u.empresa_id = $empresa order by first_name asc ";
      // echo $statement; exit;
        $q = $this->db->query($statement);
