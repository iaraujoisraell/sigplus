...
class system_settings extends MY_Controller
{
    ...
    function index()
    {
        ...
        if ($this->form_validation->run() == true && $this->settings_model->updateSetting($data)) {
            ...
            $this->data['account'] = $this->settings_model->getAllAccount();
     ...
     function account()
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('account')));
        $meta = array('page_title' => lang('account'), 'bc' => $bc);
        $this->page_construct('settings/account', $meta, $this->data);
    }
    
        function add_account()
    {
        $this->form_validation->set_rules('name', lang("name_account"), 'required');
        $this->form_validation->set_rules('description', lang("description_account"));
        $this->form_validation->set_rules('balance', lang("opening_balance"), 'required|numeric');

          if ($this->form_validation->run() == true) {
            $data = array('account' => $this->input->post('name'),
            ...
            $data_trasactions = array('account' => $this->input->post('name'),
                'type' => 'Income',
                'amount' => $this->input->post('balance'),
                'status' => 'paid',
                'description' => 'Saldo Inicial',
                'date' => $date,
                'cr' => $this->input->post('balance'),
            );
            
           
            
        } elseif ($this->input->post('add_account')) {
            $this->session->set_flashdata('error', validation_errors());
            redirect("system_settings/account");
        }

        if ($this->form_validation->run() == true && $this->settings_model->addAccount($data,$data_trasactions)) { //check to see if we are creating the customer
            $this->session->set_flashdata('message', lang("account_added"));
            redirect("system_settings/account");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['page_title'] = lang("new_account");
            $this->load->view($this->theme . 'settings/add_account', $this->data);
        }
    ...
    function edit_account($id = NULL)
    {
         ...
         $cur_details = $this->settings_model->getAccountByID($id);
     //   if ($this->input->post('account') != $cur_details->account) {
     //       $this->form_validation->set_rules('account', lang("name_account"), 'is_unique[account.account]');
     //   }
        $this->form_validation->set_rules('account', lang("name_account"), 'required');
      //  $this->form_validation->set_rules('balance', lang("opening_balance"), 'required|numeric');
        ...
        if ($this->form_validation->run() == true && $this->settings_model->updateAccount($id, $data)) { //check to see if we are updateing the customer
            $this->session->set_flashdata('message', lang("account_updated"));
            redirect("system_settings/account");
        } else {
            
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['account'] = $this->settings_model->getAccountByID($id);
            $this->data['id'] = $id;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/edit_account', $this->data);
        }
    }
    ...
    function delete_account($id = NULL)
    {
        ...
        if ($this->settings_model->deleteAccount($id)) {
            echo lang("account_deleted");
        }
    }
    ...
    function account_actions()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    foreach ($_POST['val'] as $id) {
                        $this->settings_model->deleteAccount($id);
                    }
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $this->load->library('excel');
                    $this->excel->setActiveSheetIndex(0);
                    $this->excel->getActiveSheet()->setTitle(lang('accounts'));
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('account'));
                    $this->excel->getActiveSheet()->SetCellValue('B1', lang('description'));
                    $this->excel->getActiveSheet()->SetCellValue('C1', lang('opening_balance'));

                    $row = 2;
                    foreach ($_POST['val'] as $id) {
                        $sc = $this->settings_model->getAccountByID($id);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $sc->account);
                    ...
                    $this->excel->getActiveSheet()->getColumnDimension('B')->setWidth(20);
                    $this->excel->getDefaultStyle()->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
                    $filename = 'accounts_' . date('Y_m_d_H_i_s');
                    if ($this->input->post('form_action') == 'export_pdf') {
                        $styleArray = array('borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN)));
                        $this->excel->getDefaultStyle()->applyFromArray($styleArray);
                        $this->excel->getActiveSheet()->getPageSetup()->setOrientation(PHPExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE);
       ...
       function getAccounts()
    {
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select("id, account, description, balance")
            ->from("accounts")
            ->add_column("Actions", "<div class=\"text-center\"><a href='" . site_url('system_settings/edit_account/$1') . "' class='tip' title='" . lang("edit_account") . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("delete_account") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . site_url('system_settings/delete_account/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", "id");
        //->unset_column('id');
