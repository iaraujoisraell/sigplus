...
<script type="text/javascript">
     ...
     function registra_deslike_post(post, usuario) {
         ...
         function registra_comentario_post(post, usuario) {
                 $.ajax({
                type: "POST",
    ...
    <script type="text/javascript">
        function exibe_postagem_unico(post, usuario) {
              $.ajax({
                type: "POST",
                url: "themes/default/views/networking/mural/exibe_postagem_unico.php",
                data: {
                     usuario: usuario,
                      post: post
                    },
...
<?php 
    ...
    // likes de uma postagem
    /*
    $sql_likes_post = "SELECT * FROM sig_postagem_likes
                    where postagem_id = $post_id ";
    $result_likes = mysqli_query($link, $sql_likes_post); //$link->query($sql_historico);
    $count = 0;
    while($row_likes = $result_likes->fetch_assoc()) {     
           
        $nome_like = $row_likes["usuario_id"]; 
        $array_likes[]=$nome_like;
        $count++; 
    ...
    $sql_comentarios_post = "SELECT * FROM sig_post_comentarios c
                        ...
                        where id_post = $post_id order by date_comentario desc limit 3 ";
        ...
        <div class="box-footer">
          <div class="form-horizontal">
           <div class="col-sm-10">   
            ...
            <div class="col-sm-2">
                <button type="button" onclick="registra_comentario_post(<?php echo $post_id; ?>, <?php echo $usuario; ?>)"  class="btn btn-primary pull-right btn-block btn-sm">Enivar</button>
            </div>
          </div>
        </div>
