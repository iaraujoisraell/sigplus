...
$query_empresas = "SELECT distinct su.empresa_id as empresa_id, razaoSocial, emailResponsavel
        ...
        WHERE st.status != 'paid' and st.date <= NOW() and type = 'despesa' order by st.date asc ";
...
/*
 * conta quantas despesas tem a empresa
 */
...
left join `sma_sales` as sa on sa.id = st.venda_id
WHERE status != 'paid' and st.date <= NOW() and type = 'despesa' and su.empresa_id = $id_empresa order by st.date asc ";
$sql_transactions = mysqli_query($conexao,$query);
 /*
  * FIM QUERY RECEITAS
  */      
   ...
   //TABLE RECEITAS     
                            ...
                            <div class="portlet-heading">
                                <div class="portlet-title">
                                    <h4>OLÁ '.$razaoSocial. ' </h4>
                                        <h4> VOCÊ TEM '.$count_transactions.' DESPESA(S) NÃO PAGAS ATÉ A DATA DE HOJE ' .$data_atual.'</h4>
                                </div>
       ...
       $soma_valor = 0;
        while ($transactions = mysqli_fetch_array($sql_transactions,MYSQLI_ASSOC)) {
            $cont++;
           ...
           $texto .= '</tbody>
                                        ...
                                        <thead>
                                            <tr>
                                                <th colspan="2">VALOR TOTAL A PAGAR</th>
