...
class Departments extends AdminController {
    ...
    public function __construct() {
        ...
        if (!is_admin() && !has_permission_intranet('departments', '', 'view')) {
    ...
    public function delete($id) {
        ...
        if ($intranet != '') {
            redirect(base_url('gestao_corporativa/intranet_admin/index/?group=departments'));
        } else {
            redirect(admin_url('departments'));
        }
    }
    ...
    public function modal() {
        ...
        $data['department'] = $this->departments_model->get($this->input->post('id'));
        if ($this->input->post('slug') === 'colaboradores') {
            $this->load->view('admin/departments/colaboradores', $data);
        } elseif ($this->input->post('slug') === 'log') {
            $this->load->view('admin/departments/log', $data);
        }
    }

    public function table_log() {
        $this->app->get_table_data('departments_log');
    }
    
    public function get_department_staffs($departmentid) {
        $staffs = $this->departments_model->get_department_staffs($departmentid);
