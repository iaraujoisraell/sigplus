...
class Payments extends AdminController
{
    ...
    public function payment($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($success) {
                ...
                $this->invoices_model->log_invoice_activity($fatura_id, 'editou_pagamento', false, serialize($this->input->post()));
        ...
        $data['contas'] = $this->Contas_financeiras_model->get();
    ...
    public function pdf_pgto_total($invoiceid)
    {
        if (!has_permission('payments', '', 'view')
            && !has_permission('invoices', '', 'view_own')
            && get_option('allow_staff_view_invoices_assigned') == '0') {
        ...
        $payment = $this->payments_model->get_invoice_payments_pdf($invoiceid);
        ...
        $this->load->model('invoices_model');
        $invoice_data = $this->invoices_model->get($invoiceid);
       
        try {
            $paymentpdf = payment_recibo_pdf($payment, $invoice_data);
        } catch (Exception $e) {
            $message = $e->getMessage();
            echo $message;
            if (strpos($message, 'Unable to get the size of the image') !== false) {
        ...
        $paymentpdf->Output(mb_strtoupper(slug_it(_l('payment') . '-' . $payment->id)) . '.pdf', $type);
    }
