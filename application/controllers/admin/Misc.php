...
class Misc extends AdminController
{
    ...
    public function get_medicos_dropdown_template()
    {
        $name    = $this->input->post('name');
        $medicoid    = $this->input->post('medicoid');
        
        echo $this->misc_model->get_medicos_dropdown_template($name, $medicoid);
    }
    
   public function get_dados_item($item_id = '')
    {
        //$item_id    = $this->input->post('item_id');
       
        $dados =  $this->Invoice_items_model->get($item_id);
        //echo json_encode(get_currency($id));
        $valor2 = $dados->valor2;
        if(!$valor2){
            $valor2 = 0;
    ...
    public function search_fatura()
    {
        ...
        $data['result'] = $this->misc_model->perform_search_fatura($q);
        echo json_encode([
            'results' => $this->load->view('admin/search_fatura', $data, true),
            'history' => $recentSearches,
        ]);
    }
    ...
    /******************************************************************************/
     public function edit_note_atendimento($id)
    {
         
        if ($this->input->post()) {
            $success = $this->misc_model->edit_evolucao($this->input->post(), $id);
            echo json_encode([
                'success' => $success,
    ...
    public function delete_note_atendimento($id, $rel_id)
    {
        $success = $this->misc_model->delete_evolucao($id);

        if ($this->input->is_ajax_request()) {
            if ($success) {
                set_alert('success', _l('deleted', 'Registro'));
            }
           
        } 
         redirect('appointly/appointments/abrir_atendimento/'."$rel_id#$rel_id");
        
    }
    
    /*************************************************************************/
    // HISTÓRICO
    public function add_historico($rel_id, $rel_type)
    {
        if ($this->input->post()) {
            $success = $this->misc_model->add_historico($this->input->post(), $rel_type, $rel_id);
            if ($success) {
                set_alert('success', _l('added_successfully', _l('historico')));
            }
        }
        redirect($_SERVER['HTTP_REFERER']);
    }
    
    public function edit_historico($id)
    {
        if ($this->input->post()) {
            $success = $this->misc_model->edit_historico($this->input->post(), $id);
            echo json_encode([
                'success' => $success,
                'message' => _l('note_updated_successfully'),
            ]);
    ...
    {
        $success = $this->misc_model->delete_historico($id);

        if (!$this->input->is_ajax_request()) {
            if ($success) {
                set_alert('success', _l('deleted', _l('historico')));
            }
            redirect($_SERVER['HTTP_REFERER']);
        } else {
            echo json_encode(['success' => $success]);
     ...
     * NOTA FISCAL
     */
     public function add_nota_fiscal_view()
    {
         
         $data['title']      = 'Add Nota Fiscal';
         $this->load->view('admin/nota_fiscal/add_nf', $data);
     }
     
    public function add_nota_fiscal()
    {
       $post = $this->input->post();
        ...
        if ($this->input->post()) {
            ...
            $success = $this->misc_model->add_nota_fiscal($this->input->post());
            if ($success) {
                set_alert('success', _l('added_successfully', 'Nota Fiscal'));
            }
        }
        redirect($_SERVER['HTTP_REFERER']);
    }
    ...
    /*****************************************************************************
        PARAMETRIZÇÃO AGENDA
     **************************************************************************/
    
    public function add_agenda($rel_id)
    {
        
        if ($this->input->post()) {
            $success = $this->misc_model->add_agenda($this->input->post(), $rel_id);
            if ($success) {
                set_alert('success', _l('added_successfully', 'Agenda Médica'));
            }
        }
        redirect($_SERVER['HTTP_REFERER']);
    }
    ...
    /*************************************************************************/
    // PALAVRA CHAVE
    public function add_palavra($rel_id, $rel_type)
    {
        if ($this->input->post()) {
            $success = $this->misc_model->add_palavra($this->input->post(), $rel_type, $rel_id);
            if ($success) {
                set_alert('success', _l('added_successfully', _l('palavra_chave')));
            }
        }
        redirect($_SERVER['HTTP_REFERER']);
    }
    ...
    public function delete_palavra($id)
    {
        $success = $this->misc_model->delete_palavra($id);
    ...
    /*
     * REPASSE PROCEDIMENTO MÉDICO
     */
    
    public function add_repasse_multi_medico()
    {
        ...
        if ($this->input->post()) {
            ...
            foreach($medicos as $item){
                ...
                print_r($dados);
                $success = $this->misc_model->add_repasse_medico($dados);
            }
            
            if ($success) {
                set_alert('success', _l('added_successfully', _l('repasse_medico')));
    ...
    {
        if ($this->input->post()) {
            $success = $this->misc_model->add_repasse_medico($this->input->post());
            if ($success) {
                set_alert('success', _l('added_successfully', _l('repasse_medico')));
            }
        }
    ...
    public function edit_repasse($id)
    {

        if ($this->input->post()) {
            $success = $this->misc_model->edit_repasse_medico($this->input->post(), $id);
            echo json_encode([
                'success' => $success,
                'message' => _l('repasse_updated_successfully'),
            ]);
        }
    }
    
    public function delete_repasse($id)
    {
        $success = $this->misc_model->delete_repase_medico($id);

        if (!$this->input->is_ajax_request()) {
            if ($success) {
                set_alert('success', _l('deleted', _l('repasse_medico')));
            }
            redirect($_SERVER['HTTP_REFERER']);
        } else {
            echo json_encode(['success' => $success]);
     ...
     ********************* MEDICO ANAMNESE ************************************
     *************************************************************************/
    public function add_anamnese_medico($anamnese_id = null)
    {
        ...
        if ($this->input->post()) {
            ...
            if($anamnese_id){
             // edit
             $success = $this->misc_model->edit_anamnese_medico($this->input->post(), $anamnese_id);   
            }else{
             // insert
             $success = $this->misc_model->add_anamnese_medico($this->input->post());   
            }
            
            if ($success) {
                set_alert('success', _l('added_successfully', _l('anamnese')));
            }
        }
        redirect($_SERVER['HTTP_REFERER']);
    }
    ...
    //  ATUALIZA REGISTRO FATURA
    public function atualiza_registro_fatura(){
        
        
        $registros = $this->misc_model->get_registros();
        foreach($registros as $item){
           ...
           $this->misc_model->atualiza_registros_invoices($fatura, $registro);   
    ...
    public function atualiza_sala_cirurgica(){
        
        
        $salas = $this->misc_model->get_salas();
        foreach($salas as $item){
            $fatura = $item['invoiceid'];
            $sala = $item['sala'];
           
           $this->misc_model->atualiza_sala_invoices($fatura, $sala);   
    ...
    //  ATUALIZA SALA CIRURGICA
    public function atualiza_emailtemplate(){
    ...
    public function staff_user_exists()
    {
        if ($this->input->is_ajax_request()) {
            if ($this->input->post()) {
                // First we need to check if the email is the same
                ...
                if ($member_id != '') {
                    $this->db->where('staffid', $member_id);
                    $_current_email = $this->db->get(db_prefix() . 'staff')->row();
                    if ($_current_email->user == $this->input->post('user')) {
                        echo json_encode(true);
                        die();
                    }
                }
    ...
    public function atualiza_repasse_medico($medico)
    {
         $this->misc_model->atualiza_repasses_medicos($medico);
    }
    
    public function atualiza_valor_procedimento($medico)
    {
       // echo 'aqui'; exit;
         $this->misc_model->atualiza_valor_pocedimento_final($medico);
    }
