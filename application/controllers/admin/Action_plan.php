...
class Action_plan extends AdminController
{
    ...
    {
        close_setup_menu();
        $data['statuses'] = $this->action_plan_model->get_project_statuses();
        $data['title']    = _l('action_plans');
        $this->load->view('admin/action_plan/manage', $data);
    }
    
    public function plano_acao($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            $data['sequencial'] = $this->action_plan_model->get_next_sequencial();
            $data['description'] = 'n vb';
            ...
            if ($id == '') {
                ...
                $id = $this->action_plan_model->add($data);
                
                if ($id) {
                    //echo 'salvou'; exit;
                    set_alert('success', _l('added_successfully', _l('plan')));
                    redirect(admin_url('action_plan/view/' . $id));
                }
            } else {
                //if (!has_permission('projects', '', 'edit')) {
                  //  access_denied('Projects');
               // }
                //$success = $this->action_plan_model->update($data, $id);
                //if ($success) {
        ...
        if ($id == '') {
            ...
            $data['auto_select_billing_type'] = $this->action_plan_model->get_most_used_billing_type();
        } else {
            $data['project']                               = $this->action_plan_model->get($id);
            $data['project']->settings->available_features = unserialize($data['project']->settings->available_features);

            $data['project_members'] = $this->action_plan_model->get_project_members($id);
        ...
        $data['last_project_settings'] = $this->action_plan_model->get_last_project_settings();
        ...
        $data['settings'] = $this->action_plan_model->get_settings();
        $data['statuses'] = $this->action_plan_model->get_project_statuses();
        $data['staff']    = $this->staff_model->get('', ['active' => 1]);

        $data['title'] = _l('new_action_plan');
        $this->load->view('admin/action_plan/plan', $data);
    }


    public function table($clientid = '')
    {
        $this->app->get_table_data('action_plans', [
            'clientid' => $clientid,
        ]);
    }
    ...
    public function add_expense()
    {
        if ($this->input->post()) {
            ...
            if ($id) {
                ...
                echo json_encode([
                    'url'       => admin_url('action_plan/view/' . $this->input->post('project_id') . '/?group=project_expenses'),
                    'expenseid' => $id,
                ]);
                die;
            }
    ...
    public function gantt()
    {
        ...
        if (has_permission('projects', '', 'view')) {
            ...
            $data['project_members'] = $this->action_plan_model->get_distinct_projects_members();
        }

        $data['gantt_data'] = $this->action_plan_model->get_all_projects_gantt_data([
            'status' => $selected_statuses,
            'member' => $selectedMember,
        ]);

        $this->load->view('admin/action_plan/gantt', $data);
    }

    public function view($id)
    {
        //if (has_permission('projects', '', 'view') || $this->action_plan_model->is_member($id)) {
        
            //echo 'aqui'; exit;
            close_setup_menu();
            $plan = $this->action_plan_model->get($id);
            //print_r($plan); exit;
            if (!$plan) {
                blank_page(_l('project_not_found'));
            }
            
            $plan->settings->available_features = unserialize($plan->settings->available_features);
            $data['statuses']                      = $this->action_plan_model->get_project_statuses();

            //print_r($data['statuses']); exit;
            $group = !$this->input->get('group') ? 'project_overview' : $this->input->get('group');

            // Unable to load the requested file: admin/action_plan/project_tasks#.php - FIX
            ...
            $data['plan']  = $plan;
            $data['currency'] = $this->action_plan_model->get_currency($id);

            $data['plan_total_logged_time'] = $this->action_plan_model->total_logged_time($id);

            $data['staff']     = $this->staff_model->get('', ['active' => 1]);
            $percent           = $this->action_plan_model->calc_progress($id);
            $data['bodyclass'] = '';
           ...
           // );
            ...
            if ($group == 'project_overview') {
                //echo 'aqui'; exit;
                $data['members'] = $this->action_plan_model->get_plan_members($id);
                foreach ($data['members'] as $key => $member) {
                    $data['members'][$key]['total_logged_time'] = 0;
                    $member_timesheets                          = $this->tasks_model->get_unique_member_logged_task_ids($member['staff_id'], ' AND task_id IN (SELECT id FROM ' . db_prefix() . 'tasks WHERE rel_type="plan" AND rel_id="' . $this->db->escape_str($id) . '")');
                ...
                $data['plan_total_days']        = round((human_to_unix($data['plan']->deadline . ' 00:00') - human_to_unix($data['plan']->start_date . ' 00:00')) / 3600 / 24);
                $data['plan_days_left']         = $data['plan_total_days'];
                $data['plan_time_left_percent'] = 100;
                if ($data['plan']->deadline) {
                    if (human_to_unix($data['plan']->start_date . ' 00:00') < time() && human_to_unix($data['plan']->deadline . ' 00:00') > time()) {
                        $data['plan_days_left']         = round((human_to_unix($data['plan']->deadline . ' 00:00') - time()) / 3600 / 24);
                        $data['plan_time_left_percent'] = $data['plan_days_left'] / $data['plan_total_days'] * 100;
                        $data['plan_time_left_percent'] = round($data['plan_time_left_percent'], 2);
                    }
                    if (human_to_unix($data['plan']->deadline . ' 00:00') < time()) {
                        $data['plan_days_left']         = 0;
                        $data['plan_time_left_percent'] = 0;
                    }
                }
 
                $__total_where_tasks = 'rel_type = "plan" AND rel_id=' . $this->db->escape_str($id);
                if (!has_permission_intranet('tasks', '', 'view')) {
                    $__total_where_tasks .= ' AND ' . db_prefix() . 'tasks.id IN (SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid = ' . get_staff_user_id() . ')';

                    if (get_option('show_all_tasks_for_plan_member') == 1) {
                        $__total_where_tasks .= ' AND (rel_type="plan" AND rel_id IN (SELECT plan_id FROM ' . db_prefix() . 'action_plan_members WHERE staff_id=' . get_staff_user_id() . '))';
                    }
                }

                $__total_where_tasks = hooks()->apply_filters('admin_total_plan_tasks_where', $__total_where_tasks, $id);
                ...
                $data['plan_overview_chart'] = $this->action_plan_model->get_project_overview_weekly_chart_data($id, ($this->input->get('overview_chart') ? $this->input->get('overview_chart'):'this_week'));
               ...
               // $this->load->model('invoices_model');
                ...
                $data['gantt_data'] = $this->action_plan_model->get_gantt_data($id, $gantt_type, $taskStatus);
                ...
                $data['files'] = $this->action_plan_model->get_files($id);
                ...
                $data['activity'] = $this->action_plan_model->get_activity($id);
            } elseif ($group == 'project_notes') {
                $data['staff_notes'] = $this->action_plan_model->get_staff_notes($id);
                ...
                $data['tasks']                = $this->action_plan_model->get_tasks($id, 'status != ' . Tasks_model::STATUS_COMPLETE . ' AND billed=0');
                $data['timesheets_staff_ids'] = $this->action_plan_model->get_distinct_tasks_timesheets_staff($id);
            }
            
            // Discussions
            if ($this->input->get('discussion_id')) {
                $data['discussion_user_profile_image_url'] = staff_profile_image_url(get_staff_user_id());
                $data['discussion']                        = $this->action_plan_model->get_discussion($this->input->get('discussion_id'), $id);
            ...
            $other_plans       = [];
            $other_plans_where = 'id != ' . $id;

            $statuses = $this->action_plan_model->get_project_statuses();
           ...
           //print_r($other_projects_where); exit;
            //$data['other_plans'] = $this->action_plan_model->get('', $other_projects_where);
            //echo 'aqui'; exit;
            $data['title']          = $data['plan']->name;
            $data['bodyclass'] .= 'project invoices-total-manual estimates-total-manual';
            $data['plan_status'] = get_project_status_by_id($plan->status);
//            /print_r($data); exit;
            
            $this->load->view('admin/action_plan/view', $data);
        //} else {aaaaaaaaaaaaaa
    ...
    public function mark_as()
    {
        ...
        if ($this->input->is_ajax_request()) {
            if (has_permission('projects', '', 'create') || has_permission('projects', '', 'edit')) {
                ...
                $success = $this->action_plan_model->mark_as($this->input->post());
    ...
    public function file($id, $project_id)
    {
        ...
        $data['file'] = $this->action_plan_model->get_file($id, $project_id);
        ...
        $this->load->view('admin/action_plan/_file', $data);
    }

    public function update_file_data()
    {
        if ($this->input->post()) {
            $this->action_plan_model->update_file_data($this->input->post());
        }
    }
    ...
    public function add_external_file()
    {
        if ($this->input->post()) {
            ...
            $this->action_plan_model->add_external_file($data);
        }
    }

    public function download_all_files($id)
    {
        if ($this->action_plan_model->is_member($id) || has_permission('projects', '', 'view')) {
            $files = $this->action_plan_model->get_files($id);
            if (count($files) == 0) {
                set_alert('warning', _l('no_files_found'));
                redirect(admin_url('action_plan/view/' . $id . '?group=project_files'));
            }
    ...
    {
        if ($this->input->post()) {
            $this->action_plan_model->update_task_milestone($this->input->post());
        }
    }
    ...
    public function update_milestones_order()
    {
        if ($post_data = $this->input->post()) {
            $this->action_plan_model->update_milestones_order($post_data);
        }
    }

    public function pin_action($project_id)
    {
        $this->action_plan_model->pin_action($project_id);
    ...
    {
        if (has_permission('projects', '', 'edit')) {
            $this->action_plan_model->add_edit_members($this->input->post(), $project_id);
            redirect($_SERVER['HTTP_REFERER']);
        }
    }
    ...
    {
        if ($this->action_plan_model->is_member($project_id) || has_permission('projects', '', 'view')) {
            if ($this->input->is_ajax_request()) {
    ...
    public function discussion($id = '')
    {
        if ($this->input->post()) {
            ...
            if (!$this->input->post('id')) {
                $id = $this->action_plan_model->add_discussion($this->input->post());
                if ($id) {
                    $success = true;
                ...
                $id   = $data['id'];
                unset($data['id']);
                $success = $this->action_plan_model->edit_discussion($data, $id);
                if ($success) {
    ...
    {
        echo json_encode($this->action_plan_model->get_discussion_comments($id, $type));
    }

    public function add_discussion_comment($discussion_id, $type)
    {
        echo json_encode($this->action_plan_model->add_discussion_comment(
    ...
    {
        echo json_encode($this->action_plan_model->update_discussion_comment($this->input->post(null, false)));
    }

    public function delete_discussion_comment($id)
    {
        echo json_encode($this->action_plan_model->delete_discussion_comment($id));
    }
    ...
    public function delete_discussion($id)
    {
        ...
        if (has_permission('projects', '', 'delete')) {
            $success = $this->action_plan_model->delete_discussion($id);
        }
        $alert_type = 'warning';
    ...
    {
        if ($this->input->post()) {
            $this->action_plan_model->update_milestone_color($this->input->post());
        }
    }
    ...
    {
        if ($this->input->is_ajax_request()) {
            $this->action_plan_model->change_file_visibility($id, $visible);
        }
    }
    ...
    public function change_activity_visibility($id, $visible)
    {
        if (has_permission('projects', '', 'create')) {
            if ($this->input->is_ajax_request()) {
                $this->action_plan_model->change_activity_visibility($id, $visible);
            }
        }
    }
    ...
    {
        $this->action_plan_model->remove_file($id);
        redirect(admin_url('action_plan/view/' . $project_id . '?group=project_files'));
    }
    ...
    public function milestones_kanban()
    {
        ...
        $data['milestones'][] = [
          ...
          'id'                => 0,
          'total_logged_time' => $this->action_plan_model->calc_milestone_logged_time($data['project_id'], 0),
          'color'             => null,
          ];

        $_milestones = $this->action_plan_model->get_milestones($data['project_id']);

        foreach ($_milestones as $m) {
            $data['milestones'][] = $m;
        }

        echo $this->load->view('admin/action_plan/milestones_kan_ban', $data, true);
    }
    ...
    public function milestones_kanban_load_more()
    {
        ...
        $tasks = $this->action_plan_model->do_milestones_kanban_query($status, $project_id, $page, $where);
        foreach ($tasks as $task) {
            $this->load->view('admin/action_plan/_milestone_kanban_card', ['task' => $task, 'milestone' => $status]);
        }
    }
    ...
    public function milestone($id = '')
    {
        if ($this->input->post()) {
            ...
            $success = false;
            if (!$this->input->post('id')) {
                $id = $this->action_plan_model->add_milestone($this->input->post());
                if ($id) {
                ...
                $id   = $data['id'];
                unset($data['id']);
                $success = $this->action_plan_model->update_milestone($data, $id);
                if ($success) {
        ...
        redirect(admin_url('action_plan/view/' . $this->input->post('project_id') . '?group=project_milestones'));
    }

    public function delete_milestone($project_id, $id)
    {
        if (has_permission('projects', '', 'delete')) {
            if ($this->action_plan_model->delete_milestone($id)) {
                set_alert('deleted', 'project_milestone');
            }
        }
        redirect(admin_url('action_plan/view/' . $project_id . '?group=project_milestones'));
    }
    ...
    public function bulk_action_files()
    {
        ...
        if ($this->input->post()) {
            ...
            if (is_array($ids)) {
                foreach ($ids as $id) {
                    if ($hasPermissionDelete && $this->input->post('mass_delete') && $this->action_plan_model->remove_file($id)) {
                        $total_deleted++;
                    } else {
                        $this->action_plan_model->change_file_visibility($id, $fVisibility);
                    }
                }
            }
        }
    ...
    public function timesheet_task_assignees($task_id, $project_id, $staff_id = 'undefined')
    {
        ...
        foreach ($assignees as $staff) {
            ...
            if ($staff['assigneeid'] == $staff_id && $this->action_plan_model->is_member($project_id, $staff_id)) {
                $selected = ' selected';
            }
    ...
    {
        if (has_permission('projects', '', 'edit')) {
            if ($this->action_plan_model->remove_team_member($project_id, $staff_id)) {
                set_alert('success', _l('project_member_removed'));
            }
        }

        redirect(admin_url('action_plan/view/' . $project_id));
    }

    public function save_note($project_id)
    {
        if ($this->input->post()) {
            $success = $this->action_plan_model->save_note($this->input->post(null, false), $project_id);
            if ($success) {
                set_alert('success', _l('updated_successfully', _l('project_note')));
            }
            redirect(admin_url('action_plan/view/' . $project_id . '?group=project_notes'));
        }
    }
    ...
    {
        if (has_permission('projects', '', 'delete')) {
            $project = $this->action_plan_model->get($project_id);
            $success = $this->action_plan_model->delete($project_id);
            if ($success) {
    ...
    {
        if (has_permission('projects', '', 'create')) {
            $id = $this->action_plan_model->copy($project_id, $this->input->post());
            if ($id) {
    ...
    public function mass_stop_timers($project_id, $billable = 'false')
    {
        if (has_permission('invoices', '', 'create')) {
            ...
            $tasks                = $this->action_plan_model->get_tasks($project_id, $where);
    ...
    {
        if (has_permission('invoices', '', 'create')) {
            $data['billable_tasks'] = $this->action_plan_model->get_tasks($project_id, [
                'billable'     => 1,
                'billed'       => 0,
                'startdate <=' => date('Y-m-d'),
            ]);

            $data['not_billable_tasks'] = $this->action_plan_model->get_tasks($project_id, [
                'billable'    => 1,
                'billed'      => 0,
                'startdate >' => date('Y-m-d'),
            ]);
            ...
            $this->load->view('admin/action_plan/project_pre_invoice_settings', $data);
        }
    }
        ...
        if (has_permission('invoices', '', 'create')) {
            ...
            if ($tasks) {
                ...
                if ($type == 'single_line') {
                    ...
                    $timesheets     = $this->action_plan_model->get_timesheets($project_id, $tasks);
                    $added_task_ids = [];
            ...
            $this->load->view('admin/action_plan/invoice_project', $data);
        }
    }
    ...
    public function get_rel_project_data($id, $task_id = '')
    {
        if ($this->input->is_ajax_request()) {
            ...
            echo json_encode([
                ...
                'milestones'          => render_select('milestone', $this->action_plan_model->get_milestones($id), [
                    'id',
                    'name',
    ...
    public function invoice_project($project_id)
    {
        if (has_permission('invoices', '', 'create')) {
            ...
            if ($invoice_id) {
                $this->action_plan_model->log_activity($project_id, 'project_activity_invoiced_project', format_invoice_number($invoice_id));
                set_alert('success', _l('project_invoiced_successfully'));
            }
            redirect(admin_url('action_plan/view/' . $project_id . '?group=project_invoices'));
        }
    }
