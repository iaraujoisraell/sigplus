...
class Tasks extends AdminController {
    ...
    public function task($id = '') {
      ...
      /*  if (!has_permission('tasks', '', 'edit') && !has_permission('tasks', '', 'create')) {
        ...
        if ($this->input->post()) {
            ...
            $data['sequencial'] = $this->tasks_model->get_next_sequencial();
            ...
            if ($id == '') {
              ...
              /*  if (!has_permission('tasks', '', 'create')) {
                ...
                if ($id) {
                    ...
                    //print_r($files); exit;
                    $uploadedFiles = handle_task_attachments_array($id, $files);
    ...
    public function detailed_overview() {
        ...
        for ($m = 1; $m <= 12; $m++) {
            ...
            if ($record_id && $record_id != '') {
                ...
                $this->db->where('rel_id', $record_id);
                $this->db->where('rel_type', 'record');
            }
    ...
    public function add_task_comment() {
        ...
        $comment_id = false;
        if (
                $data['content'] != '' || (isset($_FILES['file']['name']) && is_array($_FILES['file']['name']) && count($_FILES['file']['name']) > 0)
        ) {
    ...
    public function unmark_complete($id) {
        if (
                $this->tasks_model->is_task_assignee(get_staff_user_id(), $id) || $this->tasks_model->is_task_creator(get_staff_user_id(), $id) || has_permission('tasks', '', 'edit')
        ) {
    ...
    public function delete_timesheet($id) {
        if (has_permission('tasks', '', 'delete') || has_permission_intranet('projects', '', 'delete') || total_rows(db_prefix() . 'taskstimers', ['staff_id' => get_staff_user_id(), 'id' => $id]) > 0) {
            $alert_type = 'warning';
            ...
            if (is_array($ids)) {
                foreach ($ids as $id) {
                    if ($this->input->post('mass_delete')) {
                        ...
                        if ($status) {
                            if (
                                    $this->tasks_model->is_task_creator(get_staff_user_id(), $id) || $is_admin || $this->tasks_model->is_task_assignee(get_staff_user_id(), $id)
                            ) {
