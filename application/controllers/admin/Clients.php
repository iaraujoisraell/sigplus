...
class Clients extends AdminController {
    ...
    public function table_search() {
        $this->app->get_table_data('clients_search');
    }
    ...
    public function client($id = '') {
        ...
        if ($this->input->post() || $this->input->is_ajax_request()) {
            if ($id == '') {
                ...
                $outside = $data['outside_clients'];
                unset($data['outside_clients']);
        ...
        if ($id == '') {
            ...
            if ($group == 'profile') {
                ...
                $notes = $this->misc_model->get_notes($id, 'customer');
                $data['user_notes'] = $this->misc_model->get_notes($id, 'customer');
            } elseif ($group == 'historico') {
                $data['user_historico'] = $this->misc_model->get_historico_ficha($id, 'customer');
            } elseif ($group == 'visitas') {
                $data['user_visitas'] = $this->clients_model->get_todos_agendamentos_paciente($id);
            } elseif ($group == 'nota_fiscal') {
                $data['attachments'] = get_all_customer_attachments($id);
                ...
                $data['ras'] = $this->Atendimentos_model->get_ra_by_id('', $id);
               
            } elseif ($group == 'ro') {
                $this->load->model('Registro_ocorrencia_model');
                $data['ros'] = $this->Registro_ocorrencia_model->get_ros_categoria_client('', $id);
               
            } elseif ($group == 'workflow') {
                $this->load->model('Workflow_model');
                $data['workflows'] = $this->Workflow_model->get_workflow_by_client_id($id);
        ...
        if ($id != '') {
            ...
            $slug_zip_folder = (
                    $client->company != '' ? $client->company : get_contact_full_name(get_primary_contact_user_id($client->userid))
                    );
    ...
    public function upload_attachment_fatura($client_id, $fatura) {
        handle_client_attachments_upload_fatura($client_id, $fatura);
    }
    ...
    public function delete_nota_avulsa($customer_id, $id) {
        // if (has_permission('customers', '', 'delete') || is_customer_admin($customer_id)) {
        $this->clients_model->delete_attachment($id);
        // }
        redirect($_SERVER['HTTP_REFERER']);
    ...
    public function client_cpf_exists() {
        if ($this->input->post()) {
