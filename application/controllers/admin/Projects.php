...
class Projects extends AdminController
{
    ...
    public function index()
    {
        ...
        $data['statuses'] = $this->projects_model->get_project_statuses();
         $data['categorias'] = $this->projects_model->get_project_category();
     ...
     * 27/09/2022
     * @WannaLuiza
     * Insere uma categoria de projeto
     */
    public function add_categoria() {

        if ($this->input->post()) {
            $id = $this->input->post('id');
            $id = $this->projects_model->add_categoria($this->input->post(), $id);
            if ($id) {
     ...
     * 27/09/2022
     * @WannaLuiza
     * Deleta uma categoria(Adicionando deleted = 1)
     */
    public function delete_categoria() {
        ...
        $this->db->update('tblprojects_category', $dados);
        redirect('admin/Projects');
    }
    ...
    {
        if (!has_permission_intranet('projects', '', 'edit') && !has_permission_intranet('projects', '', 'create')) {
            access_denied('Projects');
        }

        if ($this->input->post()) {
            $data                = $this->input->post();
            $data['sequencial'] = $this->projects_model->get_next_sequencial();
            $data['description'] = html_purify($this->input->post('description', false));
            if ($id == '') {
                if (!has_permission_intranet('projects', '', 'create')) {
                    access_denied('Projects');
                }
                ...
                if (!has_permission_intranet('projects', '', 'edit')) {
                    access_denied('Projects');
                }
    ...
    public function gantt()
    {
        ...
        if (has_permission_intranet('projects', '', 'view')) {
    ...
    {
        if (has_permission_intranet('projects', '', 'view') || $this->projects_model->is_member($id)) {
            close_setup_menu();
            ...
            if ($group == 'project_overview') {
             ...
             //  echo '1'; exit;
               ...
               // echo '2'; exit;
                ...
                //echo 'A Wanna está tentando solucionar o problema sobre as ações para o Rafael'; exit;
    ...
    public function mark_as()
    {
        ...
        if ($this->input->is_ajax_request()) {
            if (has_permission_intranet('projects', '', 'create') || has_permission_intranet('projects', '', 'edit')) {
    ...
    {
        if ($this->projects_model->is_member($id) || has_permission_intranet('projects', '', 'view')) {
    ...
    {
        if ($this->projects_model->is_member($project_id) || has_permission_intranet('projects', '', 'view')) {
            if ($this->input->is_ajax_request()) {
    ...
    {
        $success = false;
        if (has_permission_intranet('projects', '', 'delete')) {
    ...
    public function bulk_action_files()
    {
        ...
        $total_deleted       = 0;
        $hasPermissionDelete = has_permission_intranet('projects', '', 'delete');
    ...
    public function timesheet_task_assignees($task_id, $project_id, $staff_id = 'undefined')
    {
        ...
        $data                  = '';
        $has_permission_edit   = has_permission_intranet('projects', '', 'edit');
        $has_permission_create = has_permission_intranet('projects', '', 'edit');
