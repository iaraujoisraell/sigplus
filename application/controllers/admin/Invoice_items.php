...
class Invoice_items extends AdminController
{
    ...
    public function retorno_filtro_categoria()
    {
        $convenios_procedimentos = $this->input->post("convenios_procedimentos");
        // array_push($where, ' AND '. db_prefix() . 'convenio.id IN (' . implode(', ', $convenios) . ')');
        $data['categorias']            = $this->invoice_items_model->get_groups_convenio($convenios_procedimentos);
      
       $this->load->view('admin/invoice_items/filtro_categoria', $data);
    }    
    ...
    public function change_item_status($id, $status)
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->invoice_items_model->change_item_status($id, $status);
        }
    }
