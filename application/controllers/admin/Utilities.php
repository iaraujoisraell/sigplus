...
class Utilities extends AdminController
{
    ...
    public function view_event($id)
    {
        ...
        if ($data['event']->public == 1 && !is_staff_member()
            ...
            if($data['event']->titular_id){
                
                $data_plantao['event'] = $this->utilities_model->get_event_plantao($id);
                $medicos = $this->Medicos_model->get();
                $data_plantao['medicos']    = $medicos;
                $this->load->view('admin/gestao_escala/calendario/event', $data_plantao);
            }else{
    ...
    public function view_event_plantao($id)
    {
    ...
    public function get_tabela_options_nova_empresa()
    {
        
        $options =  $this->misc_model->get_options();
        foreach ($options as $data) {
            $name = $data['name'];
            $value = $data['value'];
            $autoload = $data['autoload'];
            
            $sql = "INSERT INTO tbloptions (name, value, autoload, empresa_id) VALUES ('$name','$value', '$autoload','6') ";
            $this->db->query($sql);
            ...
            $dados['name'] = $name;
            $dados['value'] = $value;
            $dados['autoload'] = $autoload;
           
            $success = $this->misc_model->add_option($dados);   
             */
        }
