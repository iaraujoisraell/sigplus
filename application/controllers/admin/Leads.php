...
class Leads extends AdminController
{
    ...
    public function index($id = '')
    {
        ...
        $data['campanhas'] = $this->Campaign_model->get_campaign()->result_array();
    ...
    public function kanban_campanha($campanha_id = '')
    {
        close_setup_menu();
        ...
        $dados_campanha = $this->Campaign_model->get_campaign($campanha_id)->row();
        $status = explode(",",$dados_campanha->status);
        $fontes = explode(",",$dados_campanha->fontes);
       // print_R($dados_campanha); exit;
         
        $data['summary']  = get_leads_summary();
        $data['statuses'] = $status; //$this->leads_model->get_status();  // status
        $data['sources']  = $fontes; //$this->leads_model->get_source();  // fontes
        $data['campanhas'] = $this->Campaign_model->get_campaign();
        $data['title']    = _l('leads');
        // in case accesed the url leads/index/ directly with id - used in search
        $data['leadid'] = $id;
        $data['campanha_id']  = $campanha_id;
        $this->load->view('admin/leads/manage_leads_campanha', $data);
    }
    
    public function switch_kanban_campanha($campanha_id = 0)
    {
        $set = 'true';
        $this->session->set_userdata([
            'leads_kanban_view' => $set,
        ]);
        redirect(admin_url('leads/kanban_campanha/'.$campanha_id));
    }
    
    public function kanban_status_campanha($campanha_id = 0)
    {
        ...
        $data['statuses']       = $status; //$this->leads_model->get_status();
        $data['campanha_id']    = $campanha_id;
       
        $data['base_currency'] = get_base_currency();
        echo $this->load->view('admin/leads/kan-ban_campanha', $data, true);
    }
    ...
    public function campanhas()
    {
        ...
        $campanhas = $this->Campaign_model->get_campaign()->result();
        
        for ($i = 0; $i < count($campanhas); $i++) {
           
            $fontes = explode(",", $campanhas[$i]->fontes);
            for ($o = 0; $o < count($fontes); $o++) {
                $fontes[$o] = $this->Campaign_model->get_fonte($fontes[$o])->row();
                $fontes[$o] = json_decode(json_encode($fontes[$o]), true);
                                
            }
            
            $campanhas[$i]->fontes = $fontes;
            
            $status = explode(",", $campanhas[$i]->status);
            for ($o = 0; $o < count($status); $o++) {
                $status[$o] = $this->Campaign_model->get_status($status[$o])->row();
                $status[$o] = json_decode(json_encode($status[$o]), true);
            }
            
            
            $campanhas[$i]->status = $status;
            
        }
       //echo 'aqui'; exit;
          
        $data["campaigns"] = $campanhas;

        $this->load->view('admin/leads/campanhas', $data);
    }
    
    public function add_campanha($id = '') {
        if ($this->input->post()) {
           
            $id = $this->input->post('id');
            $result = $this->Campaign_model->add($this->input->post(), $id);
            
            if ($result == TRUE){
                redirect('admin/leads/campanhas');
            }
        }
    }
    ...
    public function delete_camp($id)
    {
        ...
        $this->db->update('tblcampaign', $dados);
        redirect('admin/leads/campanhas');
