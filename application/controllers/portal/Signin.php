...
class Signin extends ClientsController
{
    ...
    function token($data, $client, $master)
    {
        ...
        $info = [
            ...
            "datevalid"    => date('Y-m-d H:i:s', strtotime("+1 days")),
    ...
    function login($hash = '')
    {
        ...
        if (is_array($row)) {
            ...
            $portal_access_mode = 'full';

            if (!empty($row['access_mode'])) {
                $portal_access_mode = $row['access_mode'];
            } else {
                // fallback para registros antigos sem a coluna preenchida
                $portal_access_mode = 'full';
            }

            // grava na sessão para usar depois no portal
            $this->session->set_userdata('portal_access_mode', $portal_access_mode);
            $this->session->set_userdata('portal_quick_access', ($portal_access_mode === 'quick' ? 1 : 0));
            $this->session->set_userdata('portal_master_access', ($portal_access_mode === 'master' ? 1 : 0));
    ...
    {
        $hash = $this->input->get('hash') ? $this->input->get('hash') : $hash;
        ...
        if (is_array($row)) {
            ...
            // login_2 por natureza é o acesso rápido
            $portal_access_mode = 'quick';
