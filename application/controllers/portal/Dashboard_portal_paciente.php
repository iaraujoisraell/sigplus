...
class Dashboard_portal_paciente extends ClientsController
{
    ...
    public function portal_paciente_OLD()
    {
        // echo "aqui"; exit;
        $id_client = get_client_user_id();
        ...
        $client = $this->Clients_model->get_paciente_portal($id_client);


        $view_data["info_perfil"] = $client;
        $view_data['protocolo'] = $this->session->userdata('protocolo');
        ...
        $view_data['publicacoes'] = $this->Pubs_model->get_pubs_portal_paciente();

        // echo "<pre>";
        // print_r($client); exit;

        if ($client->email_validado == '0' || $client->telefone_validado == '0') {
            // echo "aqui"; exit;
            $this->load->view('portal/dashboard/confirmar_dados.php', $view_data);
        } else {
    ...
    public function portal_paciente()
    {
        ...
        if (
            !$id_client
            or $this->session->userdata('portal_paciente') != 1
            or !$this->session->userdata('portal_paciente')
        ) {
            redirect(base_url('Authentication/logout'));
        }
        ...
        // ============================================
        // 1. VERIFICAR SE PRECISA CONFIRMAR OS DADOS
        // ============================================
        if ($client->email_validado == '0' || $client->telefone_validado == '0') {

           
            // ==========================================================
            // 2. GERAR TOKEN/CÓDIGO apenas UMA VEZ por sessão
            // ==========================================================
            if (!$this->session->userdata('ja_enviei_confirmacao')) {
                 

                // 2.1 Gerar token de e-mail
                ...
                // 2.2 Enviar e-mail automaticamente
                $this->_enviar_email_confirmacao($client->email, $token_email);
          
                // 2.3 Gerar código de SMS
                $codigo_sms = rand(100000, 999999);
                ...
                // 2.4 Enviar SMS automaticamente
                $this->_enviar_sms_confirmacao($client->telefone, $codigo_sms);

               // echo "aqui"; exit;


                // Marcar que já enviou
                $this->session->set_userdata('ja_enviei_confirmacao', true);
            }

            // Carregar tela para confirmar
            $this->load->view('portal/dashboard/confirmar_dados.php', $view_data);
            return;
        }
        ...
        $this->load->model('Atendimentos_model');
        $view_data['atendimento'] = $this->Atendimentos_model->get_ra_by_id(
            $this->session->userdata('atendimento_id')
        );

        // Se já confirmado, exibe portal
        $this->load->view('portal/dashboard/portal_paciente.php', $view_data);
    }
    ...
    private function _enviar_email_confirmacao($email, $token)
    {
        ...
        $link = base_url("portal/auth/confirmar_email/" . $token);
        ...
        $campos_email = array(
            ...
            "email_destino" => $email,
            "assunto" => 'Confirmação de Email - Portal do Paciente',
            "mensagem" => $mensagem,
            "rel_type" => 'portal_paciente',
    ...
    private function _enviar_sms_confirmacao($telefone, $codigo)
    {
        ...
        $mensagem = "Portal do Paciente: Seu código de confirmação é: $codigo";

        $campos_sms = array(
            "data_registro" => date('Y-m-d H:i:s'),
            "usuario_registro" => get_staff_user_id(),
            "phone_destino" => preg_replace("/[^0-9]/", "", $telefone),
            "assunto" => 'Confirmação de Telefone - Portal do Paciente',
            "mensagem" => $mensagem,
            "rel_type" => 'portal_paciente',            
            "empresa_id " => $this->session->userdata('empresa_id')
        );
       ...
       // print_r($campos_sms); exit;
        ...
        $this->Comunicacao_model->addSms($campos_sms);
