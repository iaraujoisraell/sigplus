...
class Atendimento extends AdminController {
    ...
    /** Busca dados do paciente no Tasy (com timeout pra não travar a página). */
    private function _buscar_info_tasy($carteirinha)
    {
        if (defined('ENVIRONMENT') && ENVIRONMENT === 'development') return null;
    ...
    public function index_old() {
        ...
        $this->Intranet_general_model->add_log(["rel_type" => "atendimento", "controller" => "Atendimento", "function" => "index", "action" => "Atendimento (Visão Geral)"]);
        ...
        $filters_ = $this->Intranet_general_model->get_filters('ra');
    ...
    public function index()
    {
        $this->Intranet_general_model->add_log([
            ...
            "function"   => "index",
            "action"     => "Atendimento (Visão Geral)"
        ]);
        ...
        $this->load->view('gestao_corporativa/atendimento/list_nova', $data);
    }
    ...
    public function buscar()
    {
        ...
        $filtros = [
            ...
            'meus_atendimentos'    => $this->input->post('meus_atendimentos') ? 1 : 0,
            'portal'               => $this->input->post('portal') ? 1 : 0,
            'data_inicio'          => trim((string) $this->input->post('data_inicio')),
            'data_fim'             => trim((string) $this->input->post('data_fim')),
        ];
        ...
        $filtersSalvar = [
            ...
            ["user_created" => get_staff_user_id(), "date_created" => date('Y-m-d'), "rel_type" => 'ra', "tab" => 0, "filter" => 'canal_atendimento_id', "value" => json_encode($filtros['canal_atendimento_id'])],
            ["user_created" => get_staff_user_id(), "date_created" => date('Y-m-d'), "rel_type" => 'ra', "tab" => 0, "filter" => 'data_inicio',          "value" => $filtros['data_inicio']],
            ["user_created" => get_staff_user_id(), "date_created" => date('Y-m-d'), "rel_type" => 'ra', "tab" => 0, "filter" => 'data_fim',             "value" => $filtros['data_fim']],
        ];

        $this->Intranet_general_model->update_filters($filtersSalvar, 'ra', 0);

        $rows = $this->Atendimentos_model->get_ra_listagem($filtros);

        $html = $this->load->view('gestao_corporativa/atendimento/partials/tabela_rows', [
            'rows'   => $rows,
    ...
    public function transcricao_nota($atendimento_id = 0)
    {
        if (!$this->input->is_ajax_request()) {
            show_404();
        }
        ...
        $texto = $this->Atendimentos_model->montar_transcricao_nota($atendimento_id);
    ...
    public function logout_without_redirect_empresa() {
        $this->Intranet_general_model->add_log(["rel_type" => "atendimento", "controller" => "Atendimento", "function" => "logout_without_redirect_empresa", "action" => "LOGOUT"]);
    ...
    public function salvar_registro_rapido($atendimento_id = 0)
    {
        if (!$this->input->post()) {
            redirect(base_url('gestao_corporativa/Atendimento/view/' . $atendimento_id . '#solicitacoes'));
        }

        $categoria_id = (int) $this->input->post('categoria_id');

        if (!$atendimento_id || !$categoria_id) {
            set_alert('danger', 'Categoria não informada.');
            redirect(base_url('gestao_corporativa/Atendimento/view/' . $atendimento_id . '#solicitacoes'));
        }
        ...
        if (empty($campos)) {
            set_alert('warning', 'Nenhum campo encontrado para esta categoria.');
            redirect(base_url('gestao_corporativa/Atendimento/view/' . $atendimento_id . '#solicitacoes'));
        }
        ...
        foreach ($campos as $campo) {
            $campos_value = [
                ...
                "campo_id"       => $campo['id'],
                "rel_id"         => $atendimento_id,
                "rel_type"       => 'ra_atendimento_rapido',
            ...
            if ($campo['type'] == 'file') {
                $fileKey = 'campo_' . $campo['name'];

                if (isset($_FILES[$fileKey]) && !empty($_FILES[$fileKey]['name'])) {
                    if (!file_exists("./assets/intranet/arquivos/ra_rapido_arquivos/campo_file/")) {
                        mkdir("./assets/intranet/arquivos/ra_rapido_arquivos/campo_file/", 0777, true);
                    }

                    $destino = "./assets/intranet/arquivos/ra_rapido_arquivos/campo_file/";
                    $nome    = str_replace(' ', '_', $_FILES[$fileKey]["name"]);
        ...
        $this->Intranet_general_model->add_log([
            "rel_type"    => "atendimento",
            "controller"  => "Atendimento",
            "function"    => "salvar_registro_rapido",
            "action"      => "Registro rápido adicionado no atendimento ($atendimento_id)",
            "rel_id"      => $atendimento_id
        ]);

        set_alert('success', 'Registro rápido salvo com sucesso!');
        redirect(base_url('gestao_corporativa/Atendimento/view/' . $atendimento_id . '#solicitacoes'));
    }
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            $this->Intranet_general_model->add_log(["rel_type" => "atendimento", "controller" => "Atendimento", "function" => "add", "action" => "Atendimento Registrado ($id)", "rel_id" => $id]);
        ...
        $this->Intranet_general_model->add_log(["rel_type" => "atendimento", "controller" => "Atendimento", "function" => "add", "action" => "Registrar atendimento"]);
    ...
    public function encerrar_atendimento($atendimento_id = '') {
        $this->Intranet_general_model->add_log(["rel_type" => "atendimento", "controller" => "Atendimento", "function" => "encerrar_atendimento", "action" => "Atendimento Encerrado ($atendimento_id)", "rel_id" => $atendimento_id]);
    ...
    public function import_client() {
        $this->Intranet_general_model->add_log(["rel_type" => "atendimento", "controller" => "Atendimento", "function" => "import_client", "action" => "Importar Cliente"]);
        //print_r($this->input->post()); exit;
        ...
        if (!$client) {
            ...
            curl_setopt_array($ch, array(
                CURLOPT_URL => 'http://189.2.65.2/sigplus/api/Tasy',
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                ...
                CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                CURLOPT_CUSTOMREQUEST => 'POST',
                CURLOPT_POSTFIELDS => '{ "carteirinha": "' . $this->input->post('numero_carteirinha') . '", "import": "1" }',
            ...
            if ($response) {
                ...
                $msg = 'Carteirinha encontrada e importada!';
                 $this->Intranet_general_model->add_log(["rel_type" => "client", "controller" => "Atendimento", "function" => "import_client", "action" => "Cliente Importado", "rel_id" => $client->userid]);
            } else {
                $alert = 'danger';
    ...
    public function pdf($atendimento_id = '') {
        $this->Intranet_general_model->add_log(["rel_type" => "atendimento", "controller" => "Atendimento", "function" => "pdf", "action" => "Vizualizar PDF Atendimento", "rel_id" => $atendimento_id]);
        $registros_rapidos = $this->Atendimentos_model->get_values_by_categoria_id($atendimento_id);
        $data['registros_rapidos'] = $registros_rapidos;
        ...
        $this->load->model('Intranet_general_model');
        $data['notes'] = $this->Intranet_general_model->get_notes('atendimento', $atendimento_id);
