...
class Cdc extends AdminController {
    ...
    public function index($id = '') {
        ...
        $view_data['categorias'] = $this->Cdc_model->get_categorias();
...
//print_r($view_data); exit;
    ...
    public function salvar() {
        ...
        $categoria = $this->Categorias_campos_model->get_categoria($this->input->post('categoria_id'), 'cdc');
        $string = $categoria->codigoKey;
        $palavras_para_substituir = array('<<<d>>>', '<<<m>>>', '<<<Y>>>', '<<<sequencial>>>', '<<<titulo>>>', '<<<titulo_c>>>', '<<<name>>>', '<<<numero_versao>>>');
        $substituicoes = array(date('d'), date('m'), date('Y'), $data['sequence_view'], $data['titulo'], $categoria->titulo, get_departamento_nome($data['setor_id'], true), $data['numero_versao']);
...
//ECHO $value; EXIT;
            ...
            if ($this->input->post('immediately') != 1) {
                ...
                foreach ($fluxos as $f) {
                    $dados['status'] = 0;
                    $dados['staff_id'] = $this->input->post('flow_' . $f['id']);
                    $dados['fluxo_id'] = $f['id'];
...
// $this->verifica_comunicado_pendente();
        ...
        $ci = $this->Cdc_model->get_documento($id);
...
// Saída do PDF com marca d'água para o navegador
    ...
    public function create_pdf($id) {
        ...
        $html = $this->load->view('gestao_corporativa/cdc/pdf.php', $data, true);
