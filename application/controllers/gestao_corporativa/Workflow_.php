...
class Workflow extends AdminController
{
    ...
    public function add($registro_atendimento_id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($categoria->aprovacao == 1) {
                ...
                $info['date_prazo'] = date('d/m/Y', strtotime("+" . $categoria->prazo . " days", strtotime(date('Y-m-d H:i:s'))));
    ...
    public function pdf($id)
    {
        ...
        foreach ($internals as $internal) {
            ...
            foreach ($campos as $campo) {
                if ($campo['value']) {
                    $campo['staffid'] = $internal['staffid'];
                    $campos_internal[] = $campo;
                }
            }
        }
        $data['campos_internal'] = $campos_internal;

        $campos_external = [];
        $externals = $this->Workflow_model->get_resquest_external($workflow->id);
        foreach ($externals as $external) {
            $campos = $this->Categorias_campos_model->get_values($external['id'], 'external_request_workflow');
            foreach ($campos as $campo) {
                if ($campo['value']) {
                    $campos_external[] = $campo;
                }
            }
        }
    ...
    public function modal()
    {
        ...
        if ($this->input->post('slug') === 'cancel') {
    ...
    public function back()
    {
        ...
        if ($this->db->update('tbl_intranet_workflow_fluxo_andamento', $dados)) {
            ...
            $this->db->where('id', $_GET['fluxo_andamento_id_old']);
    ...
    public function take_responsability()
    {
        ...
        if ($this->input->post('id')) {
            ...
            echo json_encode(array("alert" => "success", "message" => 'O objetivo está em aberto.'), true);
        } else {
