...
class Categorias_campos extends AdminController
{
    ...
    public function modal()
    {
        ...
        if ($this->input->post('slug') === 'update_categoria') {
        ...
        } elseif ($this->input->post('slug') === 'filters') {
            ...
            $this->load->view('gestao_corporativa/categorias_campos/filters', $data);
        }
    }
    ...
    function add_tipo()
    {
       ...
       //print_r($FORM); exit;
        ...
        if ($rel_type == 'cdc') {
            ...
            $palavras_para_substituir = array('[DIA]', '[MÊS]', '[ANO]', '[SEQUÊNCIAL]', '[TÍTULO CDC]', '[TÍTULO CATEGORIA]', '[DEPARTAMENTO(SÍGLA)]', '[VERSÃO]', '[DESCRIÇÃO CATEGORIA]');
            $substituicoes = array('<<<d>>>', '<<<m>>>', '<<<Y>>>', '<<<sequencial>>>', '<<<titulo>>>', '<<<titulo_c>>>', '<<<name>>>', '<<<numero_versao>>>', '<<<desc_categoria>>>');
            $info['codigoKey'] = str_replace($palavras_para_substituir, $substituicoes, $string);

            $info['staff_responsavel'] = $FORM['staff_responsavel'];
            $info['exclusivo_responsavel'] = $FORM['exclusivo_responsavel'];
            $info['permite_pessoas'] = $FORM['permite_pessoas'];            
        }
        ...
        if($FORM['portal_paciente'] == '1'){
            $info['portal_paciente'] = 1;
        }else{
            $info['portal_paciente'] = 0;
        }
       ...
       //  print_r($info); exit;
        ...
        if ($this->input->post('id')) {
            ...
            $atuantes = '';
            foreach ($info['atuantes'] as $at) {

                $atuantes .= $at . ',';
            }

            $info['atuantes'] = substr($atuantes, 0, -1);
    ...
    public function retorno_categoria_campos()
    {
        ...
        if ($rel_type == 'cdc') {
            ...
            $info_categoria = $this->Cdc_model->get_categoria($categoria_id);
            $data['staff_responsavel'] = $info_categoria['staff_responsavel'];

            $sql_staff_departments = "SELECT * from tblstaff_departments td where staffid = " . get_staff_user_id();
            $staff_departments = $this->db->query($sql_staff_departments)->result_array();

//            print_r($staff_departments); exit;
/*
            foreach ($staff_departments as $staff_department) {
                if ($staff_department['departmentid'] == $info_categoria['responsavel']) {
                    $data['staffs'] = $this->Staff_model->get();
                    break;
                }else{
                    $data['staffs'] = array();
    ...
    public function get_campos_in_options_filter()
    {
        ...
        $data['campos_'] = $this->Categorias_campos_model->get_categoria_campos($categoria_id, 0, false, '', '', '', array('select', 'multiselect', 'date', 'setores', 'funcionarios', 'list'));
        $data['campos'] = $this->Categorias_campos_model->get_categoria_campos($categoria_id, 0, false);
