...
class Feed extends AdminController {
    ...
    public function load_newsfeed() {
        ...
        $data['exibe_menu_topo'] = 1;
        $data['response'] = $response;

        $this->load->model('departments_model');
        $data['departments'] = $this->departments_model->get();

        $this->load->view('gestao_corporativa/intranet/feed', $data);
        // echo $response; 
    }
    ...
    public function init_post_likes() {
        ...
        if (!$this->newsfeed_model->user_liked_post($id)) {
            ...
            $_likes .= '<button type="button" class="btn btn-danger btn-icon" onclick="unlike_post(' . $id . ')"> <i class="fa fa-heart"></i></button>';
        }
        $_likes .= '</div>';
        ...
        $data['likes'] = $_likes;
        $this->load->view('gestao_corporativa/intranet/retorno_likes_feed', $data);
    ...
    public function add_post() {
        $conteudo = $_POST['conteudo_textarea'];
        if ($this->input->post()) {
            $data['conteudo_textarea'] = $conteudo;

            $postid = $this->newsfeed_model->add($data);
            redirect(admin_url(''));
        }
    }
    ...
    public function add_comment() {
        //$content = $this->input->post('coment_post_'.$postid);
        //$postid = $this->input->post('postid');

        $data['postid'] = $this->input->post('postid');
        $data['content'] = $this->input->post('texto');
        $page = $this->input->post('page');
        //print_R($data); exit;   
        $comment_id = $this->newsfeed_model->add_comment($data);
        ...
        if ($total_comments > 0) {
            ...
            $comments = $this->newsfeed_model->get_post_comments($data['postid'], $page);
            ...
            if ($total_comments > $this->newsfeed_model->post_comments_limit && !$page) {
    ...
    public function delete_post($postid) {
        ...
        $this->newsfeed_model->delete_post($postid);
        redirect(admin_url(''));
    }
