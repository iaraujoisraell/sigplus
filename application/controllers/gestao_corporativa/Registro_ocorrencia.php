...
class Registro_ocorrencia extends AdminController
{
    ...
    public function delete_atuante()
    {
        ...
        if ($this->db->update('tbl_intranet_registro_ocorrencia_atuantes', $dados)) {
    ...
    public function add($registro_atendimento_id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            //ADICIONANDO A CATEGORIA NO CORPO DA MENSAGEM            
            $sql = "select * from tbl_intranet_categorias where id = " . $this->input->post('categoria_id');
            $info_categoria = $this->db->query($sql)->row_array();
            $action_info['conteudo_email'] .= " <br><strong>" . $info_categoria['titulo'] . "</strong>";
            $action_info['conteudo_sms'] .= " - " . $info_categoria['titulo'];
    ...
    public function atualizar_etapa_2()
    {
        ...
        if ($info['staff_id'] != '') {
            ...
            $sql = "select * from tbl_intranet_categorias where id = $categoria";
            $info_categoria = $this->db->query($sql)->row_array();
            ...
            $action_info_ = get_actions($action);
            ...
            //  print_r($staffs_department); exit;

            $action_info_['rel_type'] = 'r.o';
            $action_info_['rel_id'] = $data['id'];

            //ADICIONANDO A CATEGORIA NO CORPO DA MENSAGEM
            $action_info_['conteudo_email'] .= " <br><strong>" . $info_categoria['titulo'] . "</strong>";
            $action_info_['conteudo_sms'] .= " - " . $info_categoria['titulo'];

            $action_info_['link_sigplus'] = 'gestao_corporativa/Registro_ocorrencia/registro/' . base64_encode(openssl_encrypt($data['id'], "aes-256-cbc", 'sigplus', 0, 'sigplus'));
            foreach ($staffs_department as $staff) {
                $action_info_['email_destino'] = $staff['email'];
                $action_info_['staff_destino'] = $staff['staffid'];
                $action_info_['phone_destino'] = $staff['phonenumber'];

                //  print_r($action_info_);
                //exit;

                $this->Comunicacao_model->send_sigplus_email_sms($action, $action_info_);
            }
    ...
    public function modal()
    {
        if ($this->input->post('slug') == 'log_analise') {
        ...
        } else if ($this->input->post('slug') == 'trocar_categoria') {
            $data = $this->input->post();
            ...
            $this->load->view('gestao_corporativa/registro_ocorrencia/modal_trocar_categoria', $data);
        }
    }
    ...
    public function trocar_categoria()
    {
        $data = $this->input->post();
      ...
      if($data['ro_id']){
        ...
        if ($this->db->query("UPDATE tbl_intranet_categorias_campo_values a LEFT JOIN tbl_intranet_categorias_campo b ON b.id = a.campo_id SET a.deleted = 1
                            WHERE a.rel_type = 'r.o' AND a.rel_id = " . $data['ro_id'] . " AND b.preenchido_por = '0'")) {
            ...
            foreach ($campos as $campo) {
                ...
                $result =   $this->Registro_ocorrencia_model->add_campo_value($campos_value);
                if($result == false){
                    break;
                }
                //print_r($campos_value);
            ...
            if($result == true){
                set_alert('success', 'R.O. Atualizado com sucesso');
            }else{
                set_alert('danger', 'Erro ao Atualizar');
            }
        }

        redirect(base_url('gestao_corporativa/Registro_ocorrencia/registro/'.$data['ro_id']));
    }
    ...
    public function retorno_categoria_campos()
    {
        ...
        if ($this->input->post('types')) {
            $types = explode(',', $this->input->post('types'));
        }
        $campos = $this->Categorias_campos_model->get_categoria_campos($categoria_id, 0, false, '', '', '', $types);
        //echo 'jsj'; exit;
        ...
        $data['just_campos'] = $this->input->post("just_campos");
        $data['by_id'] = $this->input->post("by_id");
        ...
        //print_r($campos); exit;
        if (count($campos) > 0) {
            $this->load->view('gestao_corporativa/categorias_campos/retorno_categoria_troca', $data);
        }
    }
}
