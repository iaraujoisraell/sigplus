...
class App
{
    ...
    public function get_table_data_intranet($table, $params = [])
    {
        $params = hooks()->apply_filters('table_params', $params, $table);
        ...
        $path = VIEWPATH . 'gestao_corporativa/intranet/tables/' . $table . EXT;
        ...
        if (!file_exists($path)) {
            ...
            $myPrefixedPath = VIEWPATH . 'gestao_corporativa/intranet/tables/my_' . $table . EXT;
            if (file_exists($myPrefixedPath)) {
                $path = $myPrefixedPath;
            }
        }
    ...
    public function get_option($name)
    {
        ...
        if (!isset($this->options[$name])) {
            ...
            $this->ci->db->where('empresa_id', $empresa_id);
    ...
    protected function init()
    {
        ...
        if ($this->ci->db->field_exists('autoload', db_prefix() . 'options')) {
            ...
            if($empresa_id){
                ...
                $options = $this->ci->db->select('name, value')
                ->where('autoload', 1)
                ->get(db_prefix() . 'options_padrao')->result_array();
            }
