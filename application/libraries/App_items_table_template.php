...
abstract class App_items_table_template
{
    ...
    */
    ...
    protected function medicos_html($item, $width)
    {
        $itemHTML = '';
        $invoice_id = $item['rel_id'];
        // valor da fatura
        $dados_fatura = $this->ci->invoices_model->get($invoice_id);
        $subtotal = $dados_fatura->subtotal;
        $discount_total = $dados_fatura->discount_total; // Valor do desconto
        $valor_fatura = $dados_fatura->total;
        
        //se tiver valor de desconto, calcula a porcentagem.
        if($discount_total > 0){
            $porc_desc = ($discount_total * 100)/$subtotal;
            $porc_desc = substr("$porc_desc", 0, 7);
            $porc_desc = $porc_desc/100;
        }else{
            $porc_desc = 1;
        }
        
        /*
        * Verifica na excessão o médico para esse procedimento
        */
        $tabela_procedimento_medico_row = $this->ci->invoices_model->get_medico_procedimento_invoice_item($item['item_id'], $item['medicoid']);
        ...
        if($regra_procedimento == 1){
         foreach ($tabela_procedimento_medico_row as $procedimentos) {
                    ...
                    $qty                 =  $item['qty'];
                    $valor_total_procedimento = $valor_procedimento * $qty;
                   // se o repasse do médico for com %
                   if($tipo == 2){
                       $valor_repasse = $procedimentos['valor'];
                       $valor_repasse = $valor_repasse/100;
                       $valor_comissao = ($valor_total_procedimento * $valor_repasse);
                       if($discount_total > 0){
                        $valor_desc = ($valor_comissao * $porc_desc);
                        $valor_comissao = $valor_comissao - $valor_desc;
                       // $valor_comissao = substr("$valor_comissao", 0, 4);
                       }
                   }else{
                       // se o valor da comissao do médico for valor fixo
                       $valor_comissao = ($procedimentos['valor']);
                       if($discount_total > 0){
                        $valor_desc = ($valor_comissao * $porc_desc);
                        $valor_comissao = $valor_comissao - $valor_desc;
                       // $valor_comissao = substr("$valor_comissao", 0, 4);
                   ...
                   $valor_comissao = $valor_comissao * $qty;
         }
         
        }else{
         //se não tiver tabela especial, entra na regra padrão da categoria do ITEM
         /*
          * Caso não encontre segue a regra padrão: 30% Exame e 35% consulta
          */
            
           $invoices_itens_group = $this->ci->invoices_model->get_group_item_procedimento_invoice_item($item['item_id']);
           $comissao = $invoices_itens_group->comissao;
         
           //$comissao            =  $item['comissao'];
           $valor_procedimento  =  $item['rate'];
           $qty                 =  $item['qty'];
           $valor_total_procedimento = $valor_procedimento * $qty;
           $valor_comissao      = ($valor_total_procedimento * $comissao)/ 100;
           $valor_comissao = $valor_comissao * $qty;
        

        }
       
            $comissao = app_format_money(($valor_comissao), $this->transaction->currency_name, $this->exclude_currency());
    
            $itemHTML .= '<td align="right" width="' . $width . '%">';
       
                $itemHTML .= $item['medico']; //.' ('.app_format_money(($valor_comissao), $this->transaction->currency_name, $this->exclude_currency()).')'; // $valor_comissao = $valor_comissao * $qty;
     ...
     * @return string
     */
    public function desconto_heading()
    {
        return 'Desconto';
    }
    ...
    public function set_headings($alias = '')
    {
        ...
        $this->headings['tax']    = _l($langFrom . '_table_medico_heading', '', false);
