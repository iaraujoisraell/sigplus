...
class App_items_table extends App_items_table_template
{
    ...
    public function items()
    {
        ...
        foreach ($this->items as $item) {
            ...
            // Table data number
            $itemHTML .= '<td' . $this->td_attributes() . ' align="center" width="5%">' .$item['item_id'] . '</td>';

            $itemHTML .= '<td class="description" align="left;" width="25%">';
            
            $medico_id = $item['medicoid'];
            $item_id = $item['item_id'];
             $tabela_procedimento_medico_row = $this->ci->invoices_model->get_medico_procedimento_invoice_item($item_id, $medico_id);
            ...
            if (!empty($item['description'])) {
                ...
                . $this->period_merge_field($item['description']) .' ['. $item['categoria'].'] '
                . '</strong></span>';
                ...
                if($regra_procedimento == 0 && $medico_id != 28){
                $itemHTML .= '<br> <label class ="btn btn-danger"> PROCEDIMENTO NÃO PARAMETRIZADO PARA O MÉDICO </label>';
                }
            }
            ...
            $itemHTML .= '<td align="right" width="' . $regularItemWidth . '%">' . app_format_money($item['desconto_valor'], $this->transaction->currency_name, $this->exclude_currency()) . '<br> ('.$item['destino_desconto']. ')</td>';
            }else{
            $itemHTML .= '<td align="right" width="' . $regularItemWidth . '%">' . app_format_money($item['desconto_valor'], $this->transaction->currency_name, $this->exclude_currency()) . '</td>';    
            }
            /**
             * Items table taxes HTML custom function because it's too general for all features/options
             * @var string
             */
            $itemHTML .= $this->medicos_html($item, $regularItemWidth);
            /*
            $porc_comissao =  $item['comissao'];
            $valor_total_item = $item['qty'] * $item['rate'];
            $valor_total_medico = ($valor_total_item * $porc_comissao)/100;
            $comissao = app_format_money(( $valor_total_medico), $this->transaction->currency_name, $this->exclude_currency());
            $itemHTML .= '<td align="right" width="' . $regularItemWidth . '">' . $item['medico'] . '('.$comissao.')'. '</td>';
            */
            /**
    ...
    public function html_headings()
    {
        ...
        $html .= '<th align="right">' . $this->desconto_heading() . '</th>';
    ...
    public function pdf_headings()
    {
        ...
        $tblhtml .= '<th width="25%" align="left">' . $this->item_heading() . '</th>';
        ...
        $tblhtml .= '<th width="' . $regularItemWidth . '%" align="right">' . $this->desconto_heading() . '</th>';
