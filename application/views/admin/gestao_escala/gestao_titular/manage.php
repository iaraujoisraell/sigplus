 ...
 padding: 6px 12px;
  ...
  <div class="content">
        ...
        <div class="panel_s">
          ...
          <section class="content-header">
            ...
            <?php  if(has_permission('items','','delete')){ ?>
             <a href="#" data-toggle="modal" data-table=".table-gestao_titular" data-target="#quebrar_horario_bulk_actions" class="hide bulk-actions-btn table-btn"><?php echo 'QUEBRAR HORÁRIO'; ?></a>
            ...
            <?php } ?>
             <!-- EXCLUIR plantão -->
              <?php if(has_permission('items','','delete')){ ?>
            <a href="#"  data-toggle="modal" data-table=".table-gestao_titular" data-target="#deletar_plantao_bulk_actions" class="hide bulk-actions-btn table-btn  btn-info "><?php echo 'DELETAR HORÁRIO'; ?></a>
            ...
            <?php if(has_permission('items','','delete')){ ?>
             <a href="#" data-toggle="modal" data-table=".table-gestao_titular" data-target="#trocar_horario_bulk_actions" class="hide bulk-actions-btn table-btn"><?php echo 'TROCAR HORÁRIO'; ?></a>
             ...
             <!-- mudar TITULAR de plantão -->
              <?php if(has_permission('items','','delete')){ ?>
            <a href="#"  data-toggle="modal" data-table=".table-gestao_titular" data-target="#trocar_titular_bulk_actions" class="hide bulk-actions-btn table-btn  btn-info "><?php echo 'TROCAR TITULAR'; ?></a>
             ...
             <!-- mudar SUBSTITUTO de plantão -->
              <?php if(has_permission('items','','delete')){ ?>
            <a href="#"  data-toggle="modal" data-table=".table-gestao_titular" data-target="#trocar_substituto_bulk_actions" class="hide bulk-actions-btn table-btn  btn-info "><?php echo 'ADD SUBSTITUTO'; ?></a>
                       ...
                       <div class="modal-body">
                         <?php if(has_permission('leads','','delete')){ ?>
                           ...
                           <div class="form-group">
                            ...
                            echo render_select('substituto_troca_id',$titulares,array('medicoid',array('nome_profissional',  'especialidade')),'Substituto',$selected);
                            ?>
                           </div>

                           <label>  Dt Início</label>
                            <input type="date" name="data_inicio" id="data_inicio"  value="<?php echo date('Y-m-d'); ?>" class="form-control" > 
                            <br>
                            <label> Dt Término</label>
                            <input type="date" name="data_termino" id="data_termino"  value="" class="form-control" >
                            <br>
                            <!--
                            <label>Unidade</label>
                            <select name="unidade_id"  id="unidade_substituto" required="true" class="form-control" onchange="setores_substituto(this.value);"> 
                                <option selected disabled>Selecione</option>
                                <?php foreach ($unidades_hospitalares as $unidade) { ?>
                                    <option value="<?php echo $unidade['id'] ?>"><?php echo $unidade['razao_social']; ?></option>
                                <?php } ?>
                            </select>
                            
                            <div id="setores_unidades_substituto">

                            </div>
                            
                            <div class="form-group">
                            <label>Horários</label>
                            <select name="horario_substituto" id="horario_substituto"   class="form-control">
                           ...
                           <select  name="dia_semana_substituto" id="dia_semana_substituto"   class="form-control">
            ...
            <?php echo form_open('admin/Gestao_plantao/pdf'); ?>  
              <div class="col-md-12"> 
                  ...
                  <div class="form-group">
                            <label>Horários</label>
                            <select name="horario_id" id="horario_id"  onchange="procedimentos_table_reload();" class="form-control">
                                <option checked="true" value="">Selecione</option>
                                <?php foreach ($horarios as $plano){
              ...
              <div class="col-md-12">
                  ...
                  <div class="form-group">
                       <label>Ordernar por</label>
                        <select name="ordernar_por" id="ordernar_por" onchange="procedimentos_table_reload();"class="form-control">
                            <option value="1">Unidade / Setor / Titular</option>
    ...
    if(has_permission('items','','delete')) {
      $table_data[] = '<span class="hide"> - </span><div class="checkbox mass_select_all_wrap"><input type="checkbox" id="mass_select_all" data-to-table="gestao_titular"><label></label></div>';
    ...
    render_datatable($table_data,'gestao_titular'); ?>
  </div>
        </div>
      </div>
    </div>
...
<script>
    ...
    function troca_horario_bulk_action(event) {
	  ...
	  var horario_value = horario_troca.options[horario_troca.selectedIndex].value;
         ...
         // $(event).addClass('disabled');
          setTimeout(function() {
            $.post(admin_url + 'gestao_titular/bulk_action_trocar_escala', data).done(function() {
    ...
    function quebra_horario_bulk_action(event) {
	  ...
	  var horario_value2 = horario_quebra2.options[horario_quebra2.selectedIndex].value;
         ...
         // $(event).addClass('disabled');
          setTimeout(function() {
            $.post(admin_url + 'gestao_titular/bulk_action_quebra_escala', data).done(function() {
             // window.location.reload();
             procedimentos_table_reload();
            }).fail(function(data) {
              alert_float('danger', data.responseText);
