               ...
               <!--  <select class="selectpicker"
                  ...
                  <?php foreach($medicos as $medico){
                     ...
                     if(isset($info)){
                        ...
                        if(is_array($profissionais_escalados)) {
                         foreach($profissionais_escalados as $prof){
                           if($prof == $medico['medicoid']){
                             $selected = ' selected';
                           }
                         }
                       }
                     ...
                     ?>
                     <option value="<?php echo $medico['medicoid']; ?>"<?php echo $selected; ?>><?php echo $medico['nome_profissional'].' ['.$medico['cpf'].']'; ?></option>
                  <?php } ?>
                  </select> -->
                 
                 <select style="height: 400px;" name="segunda[]" multiple="true" class="form-control">
                        <option value="">Selecione o(s) Médico(s)</option>  
                        <?php  $profissionais_escalados = $this->Unidades_hospitalares_model->get_escala_fixa_by_config_setor_id($config_id, 'segunda');?>
                        <?php foreach($medicos as $medico){ 
                            $selected = '';
                            if(is_array($profissionais_escalados)) {
                             foreach($profissionais_escalados as $profissional){
                               if($profissional['medico_id'] == $medico['medicoid']){
                                 $selected = 'selected';
                               }
                             }
                           }else{
                ...
                <?php if ($info->terca > 0) { ?>
                 ...
                 <?php //for($seg = 1; $seg <= $info->terca; $seg++){ ?>                 
                       ...
                       <select style="height: 400px;" name = "terca[]" multiple="true" class="form-control">
                        ...
                        <?php  $profissionais_escalados = $this->Unidades_hospitalares_model->get_escala_fixa_by_config_setor_id($config_id, 'terca');?>
                        <?php foreach($medicos as $medico){ 
                            $selected = '';
                            if(is_array($profissionais_escalados)) {
                             foreach($profissionais_escalados as $profissional){
                ...
                <?php if ($info->quarta > 0) { ?>
                 ...
                 <?php //for($seg = 1; $seg <= $info->quarta; $seg++){ ?>                 
                       ...
                       <select style="height: 400px;" name = "quarta[]" multiple="true" class="form-control">
                         <option value="">Selecione um Médico</option>       
                        <?php  $profissionais_escalados = $this->Unidades_hospitalares_model->get_escala_fixa_by_config_setor_id($config_id, 'quarta');?>
                        <?php foreach($medicos as $medico){ 
                            $selected = '';
                            if(is_array($profissionais_escalados)) {
                             foreach($profissionais_escalados as $profissional){
                ...
                <?php if ($info->quinta > 0) { ?>
                 ...
                 <?php //for($seg = 1; $seg <= $info->segunda; $seg++){ ?>                 
                       <select style="height: 400px;" name = "quinta[]" multiple="true" class="form-control">
                         <option value="">Selecione um Médico</option>
                        <?php  $profissionais_escalados = $this->Unidades_hospitalares_model->get_escala_fixa_by_config_setor_id($config_id, 'quinta');?>
                        <?php foreach($medicos as $medico){ 
                            $selected = '';
                            if(is_array($profissionais_escalados)) {
                             foreach($profissionais_escalados as $profissional){
                ...
                <?php if ($info->sexta > 0) { ?>
                 ...
                 <?php //for($seg = 1; $seg <= $info->sexta; $seg++){ ?>                 
                       <select style="height: 400px;" name = "sexta[]" multiple="true" class="form-control">
                        <option value="">Selecione um Médico</option>        
                        <?php  $profissionais_escalados = $this->Unidades_hospitalares_model->get_escala_fixa_by_config_setor_id($config_id, 'sexta');?>
                        <?php foreach($medicos as $medico){ 
                            $selected = '';
                            if(is_array($profissionais_escalados)) {
                             foreach($profissionais_escalados as $profissional){
                ...
                <?php if ($info->sabado > 0) { ?>
                 ...
                 <?php //for($seg = 1; $seg <= $info->sabado; $seg++){ ?>                 
                       <select style="height: 400px;" name = "sabado[]" multiple="true" class="form-control">
                        <option value="">Selecione um Médico</option>      
                        <?php  $profissionais_escalados = $this->Unidades_hospitalares_model->get_escala_fixa_by_config_setor_id($config_id, 'sabado');?>
                        <?php foreach($medicos as $medico){ 
                            $selected = '';
                            if(is_array($profissionais_escalados)) {
                             foreach($profissionais_escalados as $profissional){
                ...
                <?php if ($info->domingo > 0) { ?>
                 ...
                 <?php //for($seg = 1; $seg <= $info->domingo; $seg++){ ?>                 
                       <select style="height: 400px;" name = "domingo[]" multiple="true" class="form-control">
                           <option value="">Selecione um Médico</option>     
                        <?php  $profissionais_escalados = $this->Unidades_hospitalares_model->get_escala_fixa_by_config_setor_id($config_id, 'domingo');?>
                        <?php foreach($medicos as $medico){ 
                            $selected = '';
                            if(is_array($profissionais_escalados)) {
                             foreach($profissionais_escalados as $profissional){
