...
<div class="modal-header task-single-header" data-task-single-id="<?php echo $task->id; ?>" data-status="<?php echo $task->status; ?>">
   ...
   <?php echo _l('task_is_private'); ?>
      <?php if(has_permission_intranet('tasks','','edit')) { ?> -
   ...
   <div class="row">
      <div class="col-md-8 task-single-col-left">
         ...
         <?php if(!empty($task->rel_id)){
            ...
            if($task->rel_type == 'project' && $task->milestone != 0){
             ...
             if(has_permission_intranet('tasks','','edit') && count($milestones) > 1){ ?>
         ...
         <?php if($task->status != Tasks_model::STATUS_COMPLETE && ($task->current_user_is_assigned || has_permission_intranet('tasks','','edit') || $task->current_user_is_creator)){ ?>
         ...
         <?php } else if($task->status == Tasks_model::STATUS_COMPLETE && ($task->current_user_is_assigned || has_permission_intranet('tasks','','edit') || $task->current_user_is_creator)){ ?>
         <p class="no-margin pull-left" style="<?php echo 'margin-'.(is_rtl() ? 'left' : 'right').':5px !important'; ?>">
         ...
         <?php } ?>
         <?php if(has_permission_intranet('tasks','','create') && count($task->timesheets) > 0){ ?>
            ...
            <div class="table-responsive">
               <table class="table">
                  ...
                  <tbody>
                     ...
                     <?php if(has_permission_intranet('tasks','','edit') || has_permission_intranet('tasks','','create') || has_permission_intranet('tasks','','delete') || $timesheet['staff_id'] == get_staff_user_id()){
                        $timers_found = true; ?>
                     <tr>
                        <td>
                        ...
                        <td>
                           <?php
                              if(!$task->billed){
                                   if(has_permission_intranet('tasks','','delete') || (has_permission_intranet('projects','','delete') && $task->rel_type == 'project') || $timesheet['staff_id'] == get_staff_user_id()){
                              ...
                              <div class="form-group">
                                 ...
                                 <select name="single_timesheet_staff_id" class="selectpicker" data-width="100%">
                                    <?php foreach($task->assignees as $assignee){
                                       if((!has_permission_intranet('tasks','','create') && !has_permission_intranet('tasks','','edit') && $assignee['assigneeid'] != get_staff_user_id()) || ($task->rel_type == 'project' && !has_permission_intranet('projects','','edit') && $assignee['assigneeid'] != get_staff_user_id())){continue;}
                                       $selected = '';
         ...
         <?php if(has_permission_intranet('tasks','','edit')){ ?><a href="#" onclick="edit_task_inline_description(this,<?php echo $task->id; ?>); return false;" class="pull-left mtop10 mleft5 font-medium-xs"><i class="fa fa-pencil-square-o"></i></a>
         <?php } ?>
         <div class="clearfix"></div>
         ...
         <div class="clearfix"></div>
         <hr />
         <h4 class="th font-medium mbot15 pull-left"><?php echo _l('task_add_edit_ob'); ?></h4>
         <div class="clearfix"></div>
         <?php if(!empty($task->objetivo)){
            echo '<div class="tc-content"><div id="task_view_description">' .check_for_links($task->objetivo) .'</div></div>';
            } else {
            echo '<div class="no-margin tc-content task-no-description" id="task_view_description"><span class="text-muted">' . _l('task_no_ob') . '</span></div>';
            } ?>
         <div class="clearfix"></div>
         <hr />
         <h4 class="th font-medium mbot15 pull-left"><?php echo _l('task_add_edit_what'); ?></h4>
         <div class="clearfix"></div>
         <?php if(!empty($task->porque)){
            echo '<div class="tc-content"><div id="task_view_description">' .check_for_links($task->porque) .'</div></div>';
            } else {
            echo '<div class="no-margin tc-content task-no-description" id="task_view_description"><span class="text-muted">' . _l('task_no_what') . '</span></div>';
            } ?>
         <div class="clearfix"></div>
         <hr />
         <h4 class="th font-medium mbot15 pull-left"><?php echo _l('task_add_edit_where'); ?></h4>
         <div class="clearfix"></div>
         <?php if(!empty($task->onde)){
            echo '<div class="tc-content"><div id="task_view_description">' .check_for_links($task->onde) .'</div></div>';
            } else {
            echo '<div class="no-margin tc-content task-no-description" id="task_view_description"><span class="text-muted">' . _l('task_no_where') . '</span></div>';
            } ?>
         <div class="clearfix"></div>
         <hr />
         <h4 class="th font-medium mbot15 pull-left"><?php echo _l('task_add_edit_how'); ?></h4>
         <div class="clearfix"></div>
         <?php if(!empty($task->como)){
            echo '<div class="tc-content"><div id="task_view_description">' .check_for_links($task->como) .'</div></div>';
            } else {
            echo '<div class="no-margin tc-content task-no-description" id="task_view_description"><span class="text-muted">' . _l('task_no_how') . '</span></div>';
            } ?>
         <div class="clearfix"></div>
         <hr />
         <h4 class="th font-medium mbot15 pull-left"><?php echo _l('task_add_edit_cost'); ?></h4>
         <div class="clearfix"></div>
         <?php if(!empty($task->custo)){
            echo '<div class="tc-content"><div id="task_view_description">' .check_for_links($task->custo) .'</div></div>';
            } else {
            echo '<div class="no-margin tc-content task-no-description" id="task_view_description"><span class="text-muted">' . _l('task_no_cost') . '</span></div>';
            } ?>
         <div class="clearfix"></div>
         <hr />
         <h4 class="th font-medium mbot15 pull-left"><?php echo _l('task_add_edit_justify'); ?></h4>
         <div class="clearfix"></div>
         <?php if(!empty($task->justificativa)){
            echo '<div class="tc-content"><div id="task_view_description">' .check_for_links($task->justificativa) .'</div></div>';
            } else {
            echo '<div class="no-margin tc-content task-no-description" id="task_view_description"><span class="text-muted">' . _l('task_no_justify') . '</span></div>';
            } ?>
         <div class="clearfix"></div>
         <hr />
         <h4 class="th font-medium mbot15 pull-left"><?php echo _l('task_add_edit_indicators'); ?></h4>
         <div class="clearfix"></div>
         <?php if(!empty($task->indicadores)){
            echo '<div class="tc-content"><div id="task_view_description">' .check_for_links($task->indicadores) .'</div></div>';
            } else {
            echo '<div class="no-margin tc-content task-no-description" id="task_view_description"><span class="text-muted">' . _l('task_no_indicators') . '</span></div>';
            } ?>
         <div class="clearfix"></div>
         <hr />
         <h4 class="th font-medium mbot15 pull-left"><?php echo _l('task_add_edit_goal'); ?></h4>
         <div class="clearfix"></div>
         <?php if(!empty($task->meta)){
            echo '<div class="tc-content"><div id="task_view_description">' .check_for_links($task->meta) .'</div></div>';
            } else {
            echo '<div class="no-margin tc-content task-no-description" id="task_view_description"><span class="text-muted">' . _l('task_no_goal') . '</span></div>';
            } ?>
         <div class="clearfix"></div>
         <hr />
      ...
      <div class="col-md-4 task-single-col-right">
         <div class="pull-right mbot10 task-single-menu task-menu-options">
            ...
            <?php if(has_permission_intranet('tasks','','delete') || has_permission_intranet('tasks','','edit') || has_permission_intranet('tasks','','create')){ ?>
         ...
         <div class="task-info task-status task-info-status">
            <h5>
               ...
               <?php if($task->current_user_is_assigned || $task->current_user_is_creator || has_permission_intranet('tasks','','edit')) { ?>
         ...
         <div class="task-info task-single-inline-wrap task-info-start-date">
            <h5><i class="fa task-info-icon fa-fw fa-lg fa-calendar-plus-o pull-left fa-margin"></i>
               ...
               <?php if(has_permission_intranet('tasks','','edit') && $task->status !=5) { ?>
         ...
         <div class="task-info task-info-priority">
            <h5>
               ...
               <?php if(has_permission_intranet('tasks','','edit') && $task->status != Tasks_model::STATUS_COMPLETE) { ?>
         ...
         <?php if($task->current_user_is_creator || has_permission_intranet('tasks','','edit')){ ?>
         ...
         <?php } ?>
         <?php if(has_permission_intranet('tasks','','create') || has_permission_intranet('tasks','','edit')){ ?>
         ...
         <?php if(has_permission_intranet('tasks','','edit') || has_permission_intranet('tasks','','create')){ ?>
         ...
         <div class="task_users_wrapper">
            <?php
               ...
               foreach ($task->assignees as $assignee) {
                $_remove_assigne = '';
                if(has_permission_intranet('tasks','','edit') || has_permission_intranet('tasks','','create')){
