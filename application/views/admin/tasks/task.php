...
?>" id="_task_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"<?php
     if ($this->input->get('opened_from_lead_id')) {
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <?php if (isset($task)) { ?>
                            <div class="pull-right mbot10 task-single-menu task-menu-options">
                                <div class="content-menu hide">
                                    <ul>
                                        <?php if (has_permission_intranet('tasks', '', 'create')) { ?>
                                            <?php
                                            $copy_template = "";
                                        ...
                                        <?php } ?>
                                        <?php if (has_permission_intranet('tasks', '', 'delete')) { ?>
                                            <li>
                                ...
                                <?php if (has_permission_intranet('tasks', '', 'delete') || has_permission_intranet('tasks', '', 'create')) { ?>
                        ...
                        <div class="checkbox checkbox-primary no-mtop checkbox-inline task-add-edit-public">
                            <input type="checkbox" id="task_is_public" name="is_public" <?php
                            if (isset($task)) {
                                if ($task->is_public == 1) {
                                    echo 'checked';
                                }
                        ...
                        <div class="checkbox checkbox-primary checkbox-inline task-add-edit-billable">
                            ...
                            <?php
                            if ((isset($task) && $task->billable == 1) || (!isset($task) && get_option('task_biillable_checked_on_creation') == 1)) {
                                echo ' checked';
                            }
                            ?>>
                            <label for="task_is_billable"><?php echo _l('task_billable'); ?></label>
                        </div>
                        <div class="task-visible-to-customer checkbox checkbox-inline checkbox-primary<?php
                        if ((isset($task) && $task->rel_type != 'project') || !isset($task) || (isset($task) && $task->rel_type == 'project' && total_rows(db_prefix() . 'project_settings', array('project_id' => $task->rel_id, 'name' => 'view_tasks', 'value' => 0)) > 0)) {
                            echo ' hide';
                        }
                        ?>">
                            <input type="checkbox" id="task_visible_to_client" name="visible_to_client" <?php
                            if (isset($task)) {
                                        ...
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <div class="input-group">
                                                    <input type="file" data-target="assets/intranet/arquivos/task_arquivos/" extension="<?php echo str_replace('.', '', get_option('allowed_files')); ?>" filesize="<?php echo file_upload_max_size(); ?>" class="form-control" name="attachments[0]">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <?php echo render_textarea('objetivo', _l('task_add_edit_ob'), (isset($task) ? $task->objetivo : ''), array('rows' => 3, 'placeholder' => _l('task_add_ob')), array(), 'no-mbot', 'tinymce-task'); ?>
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <?php echo render_textarea('description', _l('task_add_edit_description') . '?', (isset($task) ? $task->description : ''), array('rows' => 3, 'placeholder' => _l('task_add_description')), array(), 'no-mbot', 'tinymce-task'); ?>
                            </div>
                            <div class="col-md-6">
                                <?php echo render_textarea('porque', _l('task_add_edit_what') . '?', (isset($task) ? $task->porque : ''), array('rows' => 3, 'placeholder' => _l('task_add_what')), array(), 'no-mbot', 'tinymce-task'); ?>
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <?php echo render_textarea('onde', _l('task_add_edit_where') . '?', (isset($task) ? $task->onde : ''), array('rows' => 3, 'placeholder' => _l('task_add_where')), array(), 'no-mbot', 'tinymce-task'); ?>
                            </div> 
                            <div class="col-md-6">
                                <?php echo render_textarea('como', _l('task_add_edit_how') . '?', (isset($task) ? $task->como : ''), array('rows' => 3, 'placeholder' => _l('task_add_how')), array(), 'no-mbot', 'tinymce-task'); ?>
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <?php if ($rel_type == 'record') { ?>
                                    <script>
                                        var data = new Date("2022-12-02");
                                        document.getElementById('calendario').max = data.toISOString().split("T")[0];
                                        document.getElementById('calendario').max = data.toISOString().split("T")[0];
                                    </script>
                                <?php } ?>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <?php echo render_textarea('custo', _l('task_add_edit_cost'), (isset($task) ? $task->custo : ''), array('rows' => 3, 'placeholder' => _l('task_add_cost')), array(), 'no-mbot', 'tinymce-task'); ?>
                            </div>
                            <div class="col-md-6">
                                <?php echo render_textarea('justificativa', _l('task_add_edit_justify'), (isset($task) ? $task->justificativa : ''), array('rows' => 3, 'placeholder' => _l('task_add_edit_justify')), array(), 'no-mbot', 'tinymce-task'); ?>
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <?php echo render_textarea('indicadores', _l('task_add_edit_indicators'), (isset($task) ? $task->indicadores : ''), array('rows' => 3, 'placeholder' => _l('task_add_indicators')), array(), 'no-mbot', 'tinymce-task'); ?>
                            </div>
                            <div class="col-md-6">
                                <?php echo render_textarea('meta', _l('task_add_edit_goal'), (isset($task) ? $task->meta : ''), array('rows' => 3, 'placeholder' => _l('task_add_goal')), array(), 'no-mbot', 'tinymce-task'); ?>
                            </div>
                        ...
                        <div class="task-hours<?php
                        if (isset($task) && $task->rel_type == 'project' && total_rows(db_prefix() . 'projects', array('id' => $task->rel_id, 'billing_type' => 3)) == 0) {
                            echo ' hide';
                        }
                        ?>">
                            ...
                            <div class="form-group">
                                ...
                                <select name="milestone" id="milestone" class="selectpicker" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                                    ...
                                    <?php foreach ($milestones as $milestone) { ?>
                                        <option value="<?php echo $milestone['id']; ?>" <?php
                                        if (isset($task) && $task->milestone == $milestone['id']) {
                                            echo 'selected';
                                        }
                        ...
                        <div class="recurring_custom <?php
                        if ((isset($task) && $task->custom_recurring != 1) || (!isset($task))) {
                            echo 'hide';
                        }
                        ?>">
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <select name="repeat_type_custom" id="repeat_type_custom" class="selectpicker" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                                        ...
                                        if (isset($task) && $task->custom_recurring == 1 && $task->recurring_type == 'day') {
                                            echo 'selected';
                                        }
                                        ?>><?php echo _l('task_recurring_days'); ?></option>
                                        <option value="week" <?php
                                        if (isset($task) && $task->custom_recurring == 1 && $task->recurring_type == 'week') {
                                            echo 'selected';
                                        }
                                        ?>><?php echo _l('task_recurring_weeks'); ?></option>
                                        <option value="month" <?php
                                        if (isset($task) && $task->custom_recurring == 1 && $task->recurring_type == 'month') {
                                            echo 'selected';
                                        }
                                        ?>><?php echo _l('task_recurring_months'); ?></option>
                                        <option value="year" <?php
                                        if (isset($task) && $task->custom_recurring == 1 && $task->recurring_type == 'year') {
                                            echo 'selected';
                                        }
                        ...
                        <div id="cycles_wrapper" class="<?php
                        if (!isset($task) || (isset($task) && $task->recurring == 0)) {
                            echo ' hide';
                        }
                        ?>">
                            ...
                            <div class="form-group recurring-cycles">
                                <label for="cycles"><?php echo _l('recurring_total_cycles'); ?>
                                    <?php
                                    if (isset($task) && $task->total_cycles > 0) {
                                ...
                                <div class="input-group">
                                    ...
                                    ?> name="cycles" id="cycles" value="<?php echo $value; ?>" <?php
                                           if (isset($task) && $task->total_cycles > 0) {
                        ...
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="priority" class="selectpicker" id="priority" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                                        <?php foreach (get_tasks_priorities() as $priority) { ?>
                                            <option value="<?php echo $priority['id']; ?>"<?php
                                            if (isset($task) && $task->priority == $priority['id'] || !isset($task) && get_option('default_task_priority') == $priority['id']) {
                                                echo ' selected';
                                            }
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="repeat_every" class="control-label">Repetir(c/e)</label>
                                    <select name="repeat_every" id="repeat_every" class="selectpicker" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                                        ...
                                        if (isset($task) && $task->repeat_every == 1 && $task->recurring_type == 'week') {
                                            echo 'selected';
                                        }
                                        ?>><?php echo _l('week'); ?></option>
                                        <option value="2-week" <?php
                                        if (isset($task) && $task->repeat_every == 2 && $task->recurring_type == 'week') {
                                            echo 'selected';
                                        }
                                        ?>>2 <?php echo _l('weeks'); ?></option>
                                        <option value="1-month" <?php
                                        if (isset($task) && $task->repeat_every == 1 && $task->recurring_type == 'month') {
                                            echo 'selected';
                                        }
                                        ?>>1 <?php echo _l('month'); ?></option>
                                        <option value="2-month" <?php
                                        if (isset($task) && $task->repeat_every == 2 && $task->recurring_type == 'month') {
                                            echo 'selected';
                                        }
                                        ?>>2 <?php echo _l('months'); ?></option>
                                        <option value="3-month" <?php
                                        if (isset($task) && $task->repeat_every == 3 && $task->recurring_type == 'month') {
                                            echo 'selected';
                                        }
                                        ?>>3 <?php echo _l('months'); ?></option>
                                        <option value="6-month" <?php
                                        if (isset($task) && $task->repeat_every == 6 && $task->recurring_type == 'month') {
                                            echo 'selected';
                                        }
                                        ?>>6 <?php echo _l('months'); ?></option>
                                        <option value="1-year" <?php
                                        if (isset($task) && $task->repeat_every == 1 && $task->recurring_type == 'year') {
                                            echo 'selected';
                                        }
                                        ?>>1 <?php echo _l('year'); ?></option>
                                        <option value="custom" <?php
                                        if (isset($task) && $task->custom_recurring == 1) {
                                            echo 'selected';
                                        }
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="rel_type" class="selectpicker" id="rel_type" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                                        <?php if (!$this->input->get('rel_type')) { ?>
                                            <option value=""></option>
                                        <?php } ?>
                                        <?php if (!$this->input->get('rel_type') || $rel_type == 'record') { ?>
                                            <option value="record" 
                                            <?php
                                        ...
                                        <?php } ?>
                                        <?php if (!$this->input->get('rel_type') || $rel_type == 'project') { ?>
                                            <option value="project" style="display: "
                                                    <?php
                                                    if (isset($task) || $this->input->get('rel_type')) {
                                                        if ($rel_type == 'project') {
                                                            echo 'selected';
                                        ...
                                        <?php } ?>
                                        <?php if (!$this->input->get('rel_type') || $rel_type == 'invoice') { ?>
                                        ...
                                        <?php } ?>
                                        <?php if (!$this->input->get('rel_type') || $rel_type == 'customer') { ?>
                                        ...
                                        <?php } ?>
                                        <?php if (!$this->input->get('rel_type') || $rel_type == 'estimate') { ?>
                                            <option value="estimate" <?php
                                            if (isset($task) || $this->input->get('rel_type')) {
                                        ...
                                        <?php } ?>
                                        <?php if (!$this->input->get('rel_type') || $rel_type == 'contract') { ?>
                                        ...
                                        <?php } ?>
                                        <!-- comment -->
                                        <?php if (!$this->input->get('rel_type') || $rel_type == 'ticket') { ?>
                                        ...
                                        <?php } ?>
                                        <?php if (!$this->input->get('rel_type') || $rel_type == 'expense') { ?>
                                        ...
                                        <?php } ?>
                                        <?php if (!$this->input->get('rel_type') || $rel_type == 'lead') { ?>
                                        ...
                                        <?php } ?>
                                        <?php if (!$this->input->get('rel_type') || $rel_type == 'proposal') { ?>
                                ...
                                ?>" id="rel_id_wrapper">
                                    ...
                                    <div id="rel_id_select">
                                        <select name="rel_id" id="rel_id" class="ajax-sesarch" 
                                                <?php if ($rel_type != 'record') { ?> data-live-search="true" <?php } ?>data-width="100%"  data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                                                    <?php
                                                    if ($rel_id != '' && $rel_type != '') {
                        ...
                        <?php
                        if (isset($task) && $task->status == Tasks_model::STATUS_COMPLETE && (has_permission('create') || has_permission('edit'))) {
                            echo render_datetime_input('datefinished', 'task_finished', _dt($task->datefinished));
                        }
                        ?>
                        <div class="form-group checklist-templates-wrapper<?php
                        if (count($checklistTemplates) == 0 || isset($task)) {
                            echo ' hide';
                        }
                        ?>">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-info " onclick="files();"><?php echo _l('submit'); ?></button>
                <button type="submit" class="btn btn-info hide" id="submit"><?php echo _l('submit'); ?></button>
            </div>
        </div>
    </div>
    <?php echo form_close(); ?>
        ...
        function files() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    var novoInputFile = document.createElement("input");
                    novoInputFile.type = "text";
                    novoInputFile.name = "files"; // Defina um nome se necessário
                    novoInputFile.value = response;
                    // Adiciona o novo input ao formulário
                    formulario.appendChild(novoInputFile);

                    // Restaure os inputs de arquivo removidos (opcional)
                    // for (var i = 0; i < inputsFile.length; i++) {
                    //     formulario.appendChild(inputsFile[i]);
                    // }

                    // Descomente esta seção se você precisar de lógica adicional para manipular a resposta
