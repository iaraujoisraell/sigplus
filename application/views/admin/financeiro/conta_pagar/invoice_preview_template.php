...
if((credits_can_be_applied_to_invoice($invoice->status) && $credits_available > 0)) { ?>
   ...
   <div class="panel_s">
      <div class="panel-body">
         ...
         <div class="horizontal-scrollable-tabs preview-tabs-top">
            ...
            <div class="horizontal-tabs">
               <ul class="nav nav-tabs nav-tabs-horizontal mbot15" role="tablist">
                  ...
                  <li role="presentation">
                     <a href="#invoice_nota_fical" aria-controler="invoice_nota_fical" role="tab" data-toggle="tab">
                     <?php echo 'Nota Fiscal'; ?> <span class="badge"><?php echo count($notas_fiscais); ?></span>
                     </a>
                  </li>
                  <?php //} ?>
                  
                   <?php if(has_permission('invoices','','producao_aba')){ ?>
                   <li role="presentation">
                     <a href="#tab_producao" aria-controler="tab_producao" role="tab" data-toggle="tab">
                     <?php echo 'Faturamento'; ?> 
                     </a>
                  </li>
                   <?php } ?>
                  
                  <?php if(has_permission('invoices','','desconto_aba')){ ?>
                   <li role="presentation">
                     <a href="#tab_desconto" aria-controler="tab_desconto" role="tab" data-toggle="tab">
                     <?php echo 'Desconto'; ?> 
                     </a>
                  </li>
                   <?php } ?>
                  ...
                  <?php } ?>
                  <li role="presentation">
                     <a href="#tab_cadastro_cliente" aria-controls="tab_child_invoices" role="tab" data-toggle="tab">
                     <?php echo 'Cadastro Cliente'; ?>
                     </a>
                  </li> 
                  <li role="presentation">
            ...
            <div class="col-md-9 _buttons">
               ...
               <div class="pull-right">
                  ...
                  <?php if(has_permission('payments','','create') && abs($invoice->total) > 0){ ?>
                    <?php
                  //  echo $invoice->status.'<br>';
                  //  echo Invoices_model::STATUS_PAID ;
                    //if($invoice->status != 2){ 
                    // $invoice->status == Invoices_model::STATUS_PAID || ?>
                    <a href="#" onclick="record_payment(<?php echo $invoice->id; ?>); return false;"  class="mleft10 pull-right btn btn-success<?php if($invoice->status == Invoices_model::STATUS_CANCELLED){echo ' disabled';} ?>">
         ...
         <div class="tab-content">
            ...
            <?php } ?>
             <?php //if(count($notas_fiscais) > 0) { ?>
            <div class="tab-pane" role="tabpanel" id="invoice_nota_fical">
               <?php include_once(APPPATH . 'views/admin/invoices/invoice_nota_fiscal_table.php'); ?>
            </div>
            <?php //} ?>
            <?php if(count($applied_credits) > 0){ ?>
            ...
            <?php } ?>
             ...
             <!-- CADASTRO USUÁRIO -->
            <div role="tabpanel" class="tab-pane" id="tab_cadastro_cliente">
               <?php
               //print_r($invoice);
               //echo '<br>'.$invoice->clientid; 
               $dados_cliente = $this->clients_model->get($invoice->clientid); ?>
               <?php echo form_open(admin_url('invoices/atualiza_cadastro_cliente/'.$invoice->id.'/'.$invoice->clientid)); ?>
              
               <div class="col-md-6">
                    <div class="form-gruoup">
                        <label for="note" class="control-label"><?php echo 'E-mail'; ?></label>
                        <input type="email" class="form-control" name="email" value="<?php echo $dados_cliente->email2 ?>">
                    </div>
                </div>
                <div class="col-md-6">
                  <label for="note" class="control-label"><?php echo 'Celular / Whatsapp'; ?></label>
                  <input type="tel" placeholder="(99)99999-9999" class="form-control" name="phonenumber" onkeyup="mascara( this, mtel );" maxlength="15" value="<?php echo $dados_cliente->phonenumber ?>">
                </div>
                <div class="col-md-6">
                  <label for="note" class="control-label"><?php echo 'Contato 2'; ?></label>
                  <input type="tel" placeholder="(99)99999-9999" class="form-control" name="phonenumber2" onkeyup="mascara( this, mtel );" maxlength="15" value="<?php echo $dados_cliente->phonenumber2 ?>">
                </div>
                <div class="col-md-6">
                  <label for="note" class="control-label"><?php echo 'CPF'; ?></label>
                  <input type="text" placeholder="99999999999" class="form-control" name="vat" required="true" minlength="11" maxlength="14" value="<?php echo $dados_cliente->vat ?>">
                </div>
               ...
               <div class="text-right">
                  <button type="submit" class="btn btn-info mtop15 mbot15"><?php echo 'Atualizar cadastro'; ?></button>
               </div>
               <?php echo form_close(); ?>
               <hr />
             ...
             <!-- CADASTRO USUÁRIO -->
            <div role="tabpanel" class="tab-pane" id="tab_desconto">
               <?php
               $dados_cliente = $this->clients_model->get($invoice->clientid); ?>
               <?php echo form_open(admin_url('invoices/calcularDesconto/')); ?>
               <?php
               $desconto_maximo_empresa = get_option('desconto_maximo_empresa');
               $permitir_desconto_medico = get_option('permitir_desconto_medico');
               
               $total_invoice = $invoice->total;
               $vl_desc_empresa = ($total_invoice * $desconto_maximo_empresa)/100;
               ?>
                <input type="hidden" value="<?php echo $invoice->id; ?>" name="invoiceid">
                <input type="hidden" value="<?php echo $invoice->total; ?>" name="vl_total_fatura">
                <input type="hidden" value="<?php echo $invoice->discount_total; ?>" name="discount_total">
                <input type="hidden" value="<?php echo $vl_desc_empresa; ?>" name="desconto_maximo_empresa">
                
                <div class="row">
                    <div class="col-md-6">
                        <i class="fa fa-info-circle"></i> O desconto será distribuído entre os procedimentos, proporcionalmente. 
                    </div>
                    <div class="col-md-6">
                        <i class="fa fa-money"></i> Valor Máximo de desconto para a empresa: <?php echo app_format_money($vl_desc_empresa, $recurring->currency_name); ?>
                    </div>
                </div>    
                <br>
                <div class="row">   
                   <div class="col-md-6">
                        <div class="form-gruoup">
                            <div class="col-md-8">
                                <label for="note" class="control-label"><?php echo 'Valor Desconto'; ?></label>
                                <input type="text" data-toggle="tooltip" data-title="<?php echo _l('numbers_not_formatted_while_editing'); ?>" onKeyPress="return(moeda(this,'.',',',event))" maxlength="7"  required="true" class="form-control pull-left" name="valor_desconto_empresa" id="valor_desconto_empresa">
                            </div>
                            <div class="col-md-4">
                                <label for="note" class="control-label"><?php echo 'Tipo'; ?></label>
                                <select name="tipo_desconto_empresa" id="tipo_desconto_empresa"  class="form-control">
                    ...
                    <div class="col-md-6">
                      <label for="note" class="control-label"><?php echo 'Aplicar para'; ?></label>
                      <select name="destino_desconto_empresa" id="destino_desconto_empresa"  class="form-control">
              ...
              <br>
                <?php if($permitir_desconto_medico == 1){ ?>
                    <div class="row">   
                   <div class="col-md-6">
                        <div class="form-gruoup">
                            ...
                            <div class="col-md-8">
                                <label for="note" class="control-label"><?php echo 'Valor Desconto'; ?></label>
                                <input type="text" data-toggle="tooltip" data-title="<?php echo _l('numbers_not_formatted_while_editing'); ?>" value="0" class="form-control pull-left" name="valor_processa_desconto" id="valor_processa_desconto">
                            </div>
                            <div class="col-md-4">
                                <label for="note" class="control-label"><?php echo 'Tipo'; ?></label>
                ...
                <div class="row">
                    <div class="col-md-12">
                       <div class="text-right">
                          <button type="submit" class="btn btn-info mtop15 mbot15"><?php echo 'Aplicar Desconto'; ?></button>
                       </div>
                    </div>       
                </div>
               <?php echo form_close(); ?>
                ...
                <div class="table-responsive">
                  <table class="table table-bordered table-hover no-mtop">
                     <thead>
                        <th><span class="bold"><?php echo 'Desconto'; ?> #</span></th>
                        <th><span class="bold"><?php echo 'Log'; ?></span></th>
                        <th><span class="bold"><?php echo 'Usuário'; ?></span></th>
                     </thead>
                     <tbody>
                        <?php
                        //
                        foreach($descontos_fatura as $desconto) { ?>
                        <tr>
                           <td>
                              <?php echo format_money($desconto['valor_empresa'],'R$'); ?>
                           </td>
                           <td><?php echo _d($desconto['data_log']); ?></td>
                           <td><?php 
                                $nome = $desconto['firstname'];
                                $snome = $desconto['lastname'];
                                $nome_log = $nome.' '.$snome;
