...
<h4 class="customer-profile-group-heading"><?php echo 'Formas de Pagamento'; ?></h4>
      ...
      <div class="col-md-12">
        <div class="panel_s">
         ...
         <?php if(has_permission('items','','create')){ ?>
           <div class="_buttons">
            <a href="#" class="btn btn-info pull-left"  data-toggle="modal" data-target="#add_forma_modal" data-id=""><?php echo 'Add Forma Pgto'; ?></a>    
        ...
        render_datatable($table_data,'forma_pgto'); ?>
      </div>
        </div>
      </div>
    </div>
</div>
<?php $this->load->view('admin/financeiro/cadastros/groups/novo_forma_pgto'); ?>
...
<script>
  ...
  $(function(){
    ...
    var CustomersServerParams = {};
       ...
       var tAPI = initDataTable('.table-forma_pgto', admin_url+'financeiro/table_forma_pgto', [0], [0], CustomersServerParams, [1, 'desc']);
   ...
   function procedimentos_table_reload() {
     ...
     // });
       CustomersServerParams['categorias'] = '[name="categorias"]';
       CustomersServerParams['natureza'] = '[name="natureza"]'; 
     if ($.fn.DataTable.isDataTable('.table-forma_pgto')) {
       $('.table-forma_pgto').DataTable().destroy();
     }
      var tAPI = initDataTable('.table-forma_pgto', admin_url+'financeiro/table_forma_pgto', [0], [0], CustomersServerParams, [1, 'desc']);
   // tAPI.ajax.reload();
  ...
  function items_bulk_action(event) {
    if (confirm_delete()) {
      ...
      var rows = $('.table-forma_pgto').find('tbody tr');
      $.each(rows, function() {
