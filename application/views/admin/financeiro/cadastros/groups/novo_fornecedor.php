...
<div class="modal fade" id="add_fornecedor_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel">
                    <span class="edit-title"><?php echo 'Editar Fornecedor'; ?></span>
                    <span class="add-title"><?php echo 'Add Fornecedor'; ?></span>
                </h4>
            </div>
            <?php echo form_open('admin/financeiro/manage_fornecedores',array('id'=>'invoice_item_form')); ?>
            <?php echo form_hidden('id'); ?>
           
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        <?php echo render_input('company','Fornecedor'); ?>
                        <?php echo render_input('vat','CNPJ'); ?>
    ...
    $.post(url, data).done(function (response) {
        ...
        if (response.success == true) {
            ...
            if ($("body").find('.accounting-template').length > 0) {
                ...
                $('.table-fornecedores').DataTable().ajax.reload(null, false);
            }
            alert_float('success', response.message);
        }
        $('#add_fornecedor_modal').modal('hide');
    }).fail(function (data) {
...
function init_item_js() {
    ...
    $("body").on('show.bs.modal', '#add_fornecedor_modal', function (event) {

        $('.affect-warning').addClass('hide');

        var $itemModal = $('#add_fornecedor_modal');
        ...
        if (typeof (id) !== 'undefined') {
            ...
            requestGetJSON('financeiro/get_fornecedor_by_id/' + id).done(function (response) {
    ...
    $("body").on("hidden.bs.modal", '#add_fornecedor_modal', function (event) {
