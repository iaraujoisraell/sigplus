...
<div class="modal fade" id="add_banco_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">
                    ...
                    <span class="add-title"><?php echo 'Add Banco'; ?></span>
                </h4>
            </div>
            <?php echo form_open('admin/financeiro/manage_bancos',array('id'=>'invoice_item_form')); ?>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <?php echo render_input('agencia','Agência'); ?>
                        <?php echo render_input('numero_conta','Conta'); ?>
                </div>
        </div>
    </div>
    ...
    $.post(url, data).done(function (response) {
        ...
        if (response.success == true) {
            ...
            if ($("body").find('.accounting-template').length > 0) {
                ...
                $('.table-bancos').DataTable().ajax.reload(null, false);
            }
            alert_float('success', response.message);
        }
        $('#add_banco_modal').modal('hide');
    }).fail(function (data) {
...
function init_item_js() {
    ...
    $("body").on('show.bs.modal', '#add_banco_modal', function (event) {

        $('.affect-warning').addClass('hide');

        var $itemModal = $('#add_banco_modal');
        ...
        if (typeof (id) !== 'undefined') {
            ...
            requestGetJSON('financeiro/get_banco_by_id/' + id).done(function (response) {
                $itemModal.find('input[name="id"]').val(response.id);
                $itemModal.find('input[name="banco"]').val(response.banco);
                $itemModal.find('input[name="agencia"]').val(response.agencia);
                $itemModal.find('input[name="numero_conta"]').val(response.numero_conta);
    ...
    $("body").on("hidden.bs.modal", '#add_banco_modal', function (event) {
