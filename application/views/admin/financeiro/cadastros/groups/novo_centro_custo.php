...
<div class="modal fade" id="add_centro_custo_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel">
                    <span class="edit-title"><?php echo 'Editar Centro de Custo'; ?></span>
                    <span class="add-title"><?php echo 'Add Centro de Custo'; ?></span>
                </h4>
            </div>
            <?php echo form_open('admin/financeiro/manage_centro_custo',array('id'=>'invoice_item_form')); ?>
            <?php echo form_hidden('id'); ?>
           
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        <?php echo render_select('cc_tipo_id',$tipos,array('id','descricao'),'Tipo'); ?>
                        <?php echo render_input('descricao','Centro de Custo'); ?>
                        
                        <label>Status</label>
                        <select name="ativo" id="ativo" class="form-control">
                            <option checked="true" value="1">ATIVO</option>
    ...
    $.post(url, data).done(function (response) {
        ...
        if (response.success == true) {
            ...
            if ($("body").find('.accounting-template').length > 0) {
                ...
                $('.table-centro_custo').DataTable().ajax.reload(null, false);
            }
            alert_float('success', response.message);
        }
        $('#add_centro_custo_modal').modal('hide');
    }).fail(function (data) {
...
function init_item_js() {
    ...
    $("body").on('show.bs.modal', '#add_centro_custo_modal', function (event) {

        $('.affect-warning').addClass('hide');

        var $itemModal = $('#add_centro_custo_modal');
        ...
        if (typeof (id) !== 'undefined') {
            ...
            requestGetJSON('financeiro/get_centro_custo_by_id/' + id).done(function (response) {
                $itemModal.find('input[name="id"]').val(response.id);
                $itemModal.find('input[name="descricao"]').val(response.descricao);
              
                $itemModal.find('#cc_tipo_id').selectpicker('val', response.cc_tipo_id);
    ...
    $("body").on("hidden.bs.modal", '#add_centro_custo_modal', function (event) {
