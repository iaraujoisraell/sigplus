...
$aColumns = array_merge($aColumns, [
    db_prefix() . 'fin_payment_modes.name as name',

    db_prefix() . 'fin_payment_modes.active as active'
    ]);

$sIndexColumn = 'id';
$sTable       = db_prefix() . 'fin_payment_modes';
...
array_push($where, 'AND '.db_prefix() . 'fin_payment_modes.deleted = 0' );
array_push($where, ' AND '.db_prefix() . 'fin_payment_modes.empresa_id = '.$empresa_id );
//$where   = [];
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == '1') {
           ...
           // $_data = '<div class="checkbox"><input type="checkbox" value="' . $aRow['id'] . '"><label></label></div>';
            ...
            $_data = '<a href="#" data-toggle="modal" data-target="#add_forma_modal" data-id="' . $aRow['id'] . '">' . $aRow['name'] . '</a>';
            $_data .= '<div class="row-options">';

            if (has_permission('items', '', 'edit')) {
                $_data .= '<a href="#" data-toggle="modal" data-target="#add_forma_modal" data-id="' . $aRow['id'] . '">' . _l('edit') . '</a>';
            }

            if (has_permission('items', '', 'delete')) {
                $_data .= ' | <a href="' . admin_url('financeiro/delete_modo_pagamento/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
            }
            $_data .= '</div>';
