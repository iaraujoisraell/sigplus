...
$aColumns = array_merge($aColumns, [
    db_prefix() . 'fin_fornecedores.company as company',
    'vat',
    db_prefix() . 'fin_fornecedores.phonenumber'
    ]);

$sIndexColumn = 'id';
$sTable       = db_prefix() . 'fin_fornecedores';
...
array_push($where, 'AND '.db_prefix() . 'fin_fornecedores.deleted = 0' );
array_push($where, ' AND '.db_prefix() . 'fin_fornecedores.empresa_id = '.$empresa_id );
//$where   = [];
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == '1') {
           ...
           // $_data = '<div class="checkbox"><input type="checkbox" value="' . $aRow['id'] . '"><label></label></div>';
            ...
            $_data = '<a href="#" data-toggle="modal" data-target="#plano_conta_modal" data-id="' . $aRow['id'] . '">' . $aRow['company'] . '</a>';
            $_data .= '<div class="row-options">';

            if (has_permission('items', '', 'edit')) {
                $_data .= '<a href="#" data-toggle="modal" data-target="#add_fornecedor_modal" data-id="' . $aRow['id'] . '">' . _l('edit') . '</a>';
            }

            if (has_permission('items', '', 'delete')) {
                $_data .= ' | <a href="' . admin_url('financeiro/delete_fornecedores/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
            }
            $_data .= '</div>';
