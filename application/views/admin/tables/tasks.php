...
$hasPermissionEdit   = has_permission_intranet('tasks', '', 'edit');
$hasPermissionDelete = has_permission_intranet('tasks', '', 'delete');
...
array_push($where, ' AND ' . db_prefix() . 'tasks.deleted=0');
array_push($where, ' AND ' . db_prefix() . 'tasks.empresa_id= '.$empresa_id);

//array_push($where, 'AND (tbltasks.id in (SELECT taskid FROM tbltask_assigned WHERE taskid=tbltasks.id AND staffid= ' . get_staff_user_id() . ') or
 //   tbltasks.id in (SELECT taskid FROM tbltask_followers WHERE taskid=tbltasks.id AND staffid= ' . get_staff_user_id() . '))');
...
foreach ($rResult as $aRow) {
    ...
    $row[] = '<p style="color: grey;">'.$aRow['objetivo'].'</p>';
    $row[] = '<p style="color: grey;">'.$aRow['description'].'</p>';
    $row[] = '<p style="color: grey;">'.$aRow['porque'].'</p>';
    $row[] = '<p style="color: grey;">'.$aRow['onde'].'</p>';
    $row[] = '<p style="color: grey;">'.$aRow['como'].'</p>';
    ...
    $canChangeStatus = ($aRow['current_user_is_creator'] != '0' || $aRow['current_user_is_assigned'] || has_permission_intranet('tasks', '', 'edit'));
    ...
    if (has_permission_intranet('tasks', '', 'edit') && $aRow['status'] != Tasks_model::STATUS_COMPLETE) {
