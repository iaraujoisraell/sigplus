...
$aColumns = array_merge($aColumns, [
    db_prefix() . '_intranet_eventos.titulo as titulo',
    db_prefix() . '_intranet_eventos.inicio as inicio',
    db_prefix() . '_intranet_eventos.fim as fim',
    db_prefix() . '_intranet_eventos.cor as cor'
    ]);
$sIndexColumn = 'id';
$sTable       = db_prefix() . '_intranet_eventos';
...
array_push($where, 'AND '.db_prefix() . '_intranet_eventos.deleted = 0' );
array_push($where, ' AND '.db_prefix() . '_intranet_eventos.empresa_id = '.$empresa_id );
       ...
       $url = $link->url;
        if ($aColumns[$i] == '1') {
           ...
           // $_data = '<div class="checkbox"><input type="checkbox" value="' . $aRow['id'] . '"><label></label></div>';
            ...
            $_data = '<a href="#" data-toggle="modal" data-target="#add_banco_modal" style="color:'.$aRow['cor'].'" data-id="' . $aRow['id'] . '">' . $aRow['titulo'] . '</a>';
            $_data .= '<div class="row-options">';

            if (has_permission('items', '', 'edit')) {
                $_data .= '<a data-toggle="modal" data-target="#add_data'.$aRow['id'].'">' . _l('edit') . '</a>';
            }

            if (has_permission('items', '', 'delete')) {
                $_data .= ' | <a href="' . base_url('gestao_corporativa/Intranet/delete_data'.'?id='. $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
            }
        ...
        $aRow['inicio'] = date("d-m-Y", strtotime($aRow['inicio']));
