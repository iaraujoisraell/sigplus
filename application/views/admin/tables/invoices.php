...
$aColumns = [
    'number',
    db_prefix() .'appointly_appointments.codigo_sinconecta as codigo_sinconecta',
   // db_prefix() .'appointly_appointments.finished as finished_id',
   // db_prefix() .'appointly_appointments.finished_data as finished_data_Atendimento',    
    'total',
   // 'total_tax',
   // 'YEAR(tblinvoices.date) as year',
    ...
    db_prefix() . 'appointly_appointments.finished as finished',
    ];
...
$join = [
    ...
    'LEFT JOIN ' . db_prefix() . 'appointly_appointments  ON ' . db_prefix() . 'appointly_appointments.id = ' . db_prefix() . 'invoices.atendimento_id',
];
...
// convenio
$convenios    = $this->ci->Convenios_model->get_particular();
$convenioIds = [];
foreach ($convenios as $convenio) {
    if ($this->ci->input->post('convenio_' . $convenio['id'])) {
        array_push($convenioIds, $convenio['id']);
    }
}
if (count($convenioIds) > 0) {
    array_push($filter,  'AND ' . db_prefix() . 'invoices.convenio IN (' . implode(', ', $convenioIds) . ')');
}
// medico
$medicos    = $this->ci->Medicos_model->get();
$medicosIds = [];
foreach($medicos as $medico){
    if ($this->ci->input->post('medicos_' . $medico['medicoid'])) {
        array_push($medicosIds, $medico['medicoid']);
    }
}
if (count($medicosIds) > 0) {
    array_push($filter,  'AND ' . db_prefix() . 'invoices.medicoid IN (' . implode(', ', $medicosIds) . ')');
}
...
if (count($yearArray) > 0) {
    array_push($where, 'AND YEAR(tblinvoices.date) IN (' . implode(', ', $yearArray) . ')');
}
...
if ($faturaid != '') {
    array_push($where, 'AND ' . db_prefix() . 'invoices.id=' . $this->ci->db->escape_str($faturaid));
}
...
if (!has_permission('invoices', '', 'view')) {
 ...
 array_push($where, ' AND ' . db_prefix() . 'convenio.particular = 1');
 
$empresa_id = $this->ci->session->userdata('empresa_id');
array_push($where, ' AND '.db_prefix() . 'invoices.empresa_id = '.$empresa_id );
...
foreach ($rResult as $aRow) {
    ...
    $sinconecta = '';
    // if( $aRow['finished_id'] == 1){
        //$sinconecta .= '<br /><span class="label label-success inline-block mtop4"> ' . $aRow['finished_id'] . '</span>';
    // }
    $registro = $aRow['registro'];
    if($registro){
        $row[] = $aRow['registro'];
    }else{
        $row[] = $aRow['codigo_sinconecta'];
    }
   ...
   // $row[] = _d($aRow['duedate']);
    ...
    $finished = '<label class="label label-warning"> AGUARDANDO </label>';
    if($aRow['finished'] == 1){
        $row[] = '<label class="label label-success"> ATENDIDO </label>';
    }else{
        $row[] = $finished;
    }
