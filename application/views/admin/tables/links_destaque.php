...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
       ...
       //$url = $link->url;
        if ($aColumns[$i] == '1') {
           ...
           // $_data = '<div class="checkbox"><input type="checkbox" value="' . $aRow['id'] . '"><label></label></div>';
            ...
            if (has_permission('items', '', 'see')) {
                $_data .= ' | <a href="'.$aRow['url'].'" target="_blank">Ver</a>';
            }

            if (has_permission('items', '', 'delete')) {
                $_data .= ' | <a href="' . base_url('gestao_corporativa/intra/Links_destaque/delete_link'.'?id='. $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
            }
        ...
        //$this->ci->load->model('Link_destaque_model');
        
        //$categoria = $this->ci->Link_destaque_model->get_categoria($aRow['categoria'])->row();
        //$titulo = $categoria->titulo;
        //$aRow['categoria'] = $titulo;
        //$aRow['categoria'] = 'aba';
    }
