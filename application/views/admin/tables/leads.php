...
$campanha_id = 0;
if($this->ci->input->post('campanha_id')){
    $campanha_id = $this->ci->input->post('campanha_id');
}else{
    $campanhas = $this->ci->Campaign_model->get_campaign()->result_array();
    
    $campanha_id = "";
        $total_camp = count($campanhas);
        $cont = 1;
...
if (has_permission('leads', '', 'view') && $this->ci->input->post('campanha_id')) {
    array_push($where, 'AND campanha_id in (' . $this->ci->db->escape_str($campanha_id).' )');
}
...
array_push($where, 'AND '.db_prefix() . 'leads.deleted = 0' );
array_push($where, ' AND '.db_prefix() . 'leads.empresa_id = '.$empresa_id );
...
foreach ($rResult as $aRow) {
    ...
    $row[] = ($aRow['phonenumber'] != '' ? '<a target="_blank" href="https://api.whatsapp.com/send?phone=55' . $aRow['phonenumber'] . '">' . $aRow['phonenumber'] . '</a>' : '');
