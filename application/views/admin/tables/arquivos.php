...
$aColumns = array_merge($aColumns, [
    db_prefix() . '_intranet_arquivo.titulo as titulo',
    db_prefix() . '_intranet_arquivo.dt_created as date',
    db_prefix() . '_intranet_arquivo.file as file',
    db_prefix() . '_intranet_arquivo.file as para',
    db_prefix() . '_intranet_arquivo.file as lido'
    ]);
$sIndexColumn = 'id';
$sTable       = db_prefix() . '_intranet_arquivo';
...
array_push($where, 'AND '.db_prefix() . '_intranet_arquivo.deleted = 0' );
array_push($where, ' AND '.db_prefix() . '_intranet_arquivo.empresa_id = '.$empresa_id );
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == '1') {
           ...
           // $_data = '<div class="checkbox"><input type="checkbox" value="' . $aRow['id'] . '"><label></label></div>';
            ...
            if (has_permission('items', '', 'see')) {
                $_data .= ' | <a href="' . base_url('assets/intranet/img/docs/'. $aRow['file']) . '" target="_blank">Ver</a>';
            }

            if (has_permission('items', '', 'delete')) {
                $_data .= ' | <a href="' . base_url('gestao_corporativa/intra/Arquivos/delete_arquivo'.'?id='. $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
            }
        ...
        $enviados = $this->ci->Arquivo_model->get_arquivo_send($aRow['id'])->result();
        if($enviados){
