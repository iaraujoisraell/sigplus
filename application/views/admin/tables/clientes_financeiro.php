...
$sIndexColumn = 'id';
$sTable       = db_prefix() . 'fin_clients_financeiro';
...
//array_push($where, ' AND '.db_prefix() . 'clients.paciente = 2' );
array_push($where, ' AND '.db_prefix() . 'fin_clients_financeiro.deleted = 0' );
array_push($where, ' AND '.db_prefix() . 'fin_clients_financeiro.empresa_id = '.$empresa_id );
//$where   = [];
        ...
        if ($aColumns[$i] == '1') {
           ...
           // $_data = '<div class="checkbox"><input type="checkbox" value="' . $aRow['id'] . '"><label></label></div>';
            ...
            if (has_permission('items', '', 'edit')) {
                $_data .= '<a href="#" data-toggle="modal" data-target="#add_cliente_modal" data-id="' . $aRow['id'] . '">' . _l('edit') . '</a>';
            }

            if (has_permission('items', '', 'delete')) {
                $_data .= ' | <a href="' . admin_url('financeiro/delete_clientes/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
            }
            $_data .= '</div>';
