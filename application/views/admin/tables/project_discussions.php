...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'subject') {
            ...
            if (has_permission_intranet('projects', '', 'edit') || has_permission_intranet('projects', '', 'delete')) {
                ...
                if (has_permission_intranet('projects', '', 'delete')) {
                     $_data .= (has_permission_intranet('projects', '', 'edit') ? ' | ' : '') . '<a href="#" onclick="delete_project_discussion(' . $aRow['id'] . '); return false;" class="text-danger">'._l('delete').'</a>';
                }
                $_data .= '</div>';
            }
