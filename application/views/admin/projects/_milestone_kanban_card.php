...
<li data-task-id="<?php echo $task['id']; ?>" class="task<?php if(has_permission_intranet('tasks','','create') || has_permission_intranet('tasks','','edit')){echo ' sortable';} ?><?php if($task['current_user_is_assigned']){echo ' current-user-task';} if((!empty($task['duedate']) && $task['duedate'] < date('Y-m-d')) && $task['status'] != Tasks_model::STATUS_COMPLETE){ echo ' overdue-task'; } ?>">
