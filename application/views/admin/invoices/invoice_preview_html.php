   ...
   <?php if($invoice->medicoid != 0 ){ ?>
    ...
    <?php if($invoice->centrocustoid > 0){ ?>
      <p class="no-mbot">
        ...
        <?php echo get_centro_custo_full_name($invoice->centrocustoid); ?>
      </p>
    <?php } ?>  
         ...
         <tbody>
            ...
            <?php if(count($invoice->payments) > 0 && get_option('show_total_paid_on_invoice') == 1) { ?>
               <tr>
                  ...
                  <td>
                     <?php echo '-' . app_format_money(sum_from_table(db_prefix().'invoicepaymentrecords',array('field'=>'amount','where'=>array('invoiceid'=>$invoice->id, 'deleted'=>0))), $invoice->currency_name); ?>
                  </td>
               </tr>
            <?php } ?>
