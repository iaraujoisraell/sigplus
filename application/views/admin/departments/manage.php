            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="panel_s">
                        ...
                        <?php } else { ?>
                            <h4 class="customer-profile-group-heading"><?php echo 'Departamentos'; ?></h4>
                        <?php } ?>
                        <?php if (is_admin() or has_permission_intranet('departments', '', 'create')) { ?>
                            <div class="_buttons">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <?php echo render_input('abreviado', 'Sigla do Departamento'); ?>
                            </div>
                            <div class="col-md-6">
                                <?php
                                ...
                                ?>
                                <?php
                                echo render_select('setor_pai', $departments, array('departmentid', 'name'), 'Departamento subordinado a');
                                ?>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="select-placeholder form-group" app-field-wrapper="gestor_staff">
                                    <label for="gestor_staff" class="control-label">Responsável do Departamento</label>
                                    <select id="gestor_staff" name="gestor_staff" class="selectpicker" data-width="100%" data-none-selected-text="Nada selecionado" data-live-search="true">
                                        <option value=""></option>
                            ...
                            <div class="col-md-6">
                                <div class="select-placeholder form-group" app-field-wrapper="aprovador">
                                    <label for="aprovador" class="control-label">Aprovador do Departamento</label>
                                    <select id="aprovador" name="aprovador" class="selectpicker" data-width="100%" data-none-selected-text="Nada selecionado" data-live-search="true">
                                        <option value=""></option>
                                        <?php foreach ($staffs as $staff) { ?>
                                            <option value="<?php echo $staff['staffid']; ?>"><?php echo $staff['firstname'] . ' ' . $staff['lastname']; ?></option>
                                        <?php } ?>
                        ...
                        <?php echo render_input('password', 'dept_email_password', '', 'password', array('class' => 'stealthy')); ?>
...
<?php init_tail(); ?>
<script>
    function Colaboradores(el) {
        $("#modal_wrapper").load("<?php echo base_url(); ?>admin/Departments/modal", {
            slug: 'colaboradores',
            id: el
        }, function () {
...
<?php } ?>
        initDataTable('.table-departments', '<?php echo base_url(); ?>' + 'admin/Departments', [4], [4], Params, [1, 'asc']);
    ...
    function edit_department(invoker, id) {
        ...
        //var aprovador = $(invoker).data('aprovador');
        //document.getElementById('aprovador').value = aprovador;
        $(input_enc_selector).prop('checked', true);
        $('#additional').append(hidden_input('id', id));
        $('select[name="aprovador"]').val($(invoker).data('aprovador'));
        $('select[name="gestor_staff"]').val($(invoker).data('gestor_staff'));
        $('select[name="setor_pai"]').val($(invoker).data('setor_pai'));
        $('.selectpicker').selectpicker('refresh');
        $('#department input[name="name"]').val($(invoker).data('name'));
        $('#department input[name="abreviado"]').val($(invoker).data('abreviado'));
