...
<div class="modal fade" id="unidade_hospitalar_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <?php echo form_open('admin/unidade_hospitalar/manage',array('id'=>'invoice_item_form')); ?>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <?php echo render_select('convenio_id',$convenios,array('id','name'),'convenios'); ?>
                        
                        <?php echo render_select('group_id',$items_groups,array('id','name'),'categoria'); ?>
                        
                        <?php echo render_select('codigo_tuss', $items_tuss, array('codigo_tuss',array('codigo_tuss',  'nome_tuss')),'codigo_tuss'); ?>
                        
                        <?php echo render_input('description','invoice_item_add_edit_description'); ?>
                        
                        
                        <?php //echo render_input('codigo_tuss','codigo_tuss'); ?>
                        <?php
                       //$selected = '';
                        //echo render_select('codigo_tuss',$items_tuss,array('codigo_tuss',array('codigo_tuss',  'nome_tuss')),'codigo_tuss',$selected);
                        ?>
                        ...
                        <label for="rate" class="control-label">
                            <?php echo 'Valor Unitário '. _l($base_currency->name); ?></label>
    ...
    $("body").on('show.bs.modal', '#sales_item_modal', function (event) {
        ...
        if (typeof (id) !== 'undefined') {
            ...
            requestGetJSON('invoice_items/get_item_by_id/' + id).done(function (response) {
                $itemModal.find('input[name="item_id"]').val(response.item_id);
                $itemModal.find('input[name="description"]').val(response.description);
                //$itemModal.find('input[name="codigo_tuss"]').val(response.codigo_tuss);
                ...
                $itemModal.find('#codigo_tuss').selectpicker('val', response.codigo_tuss);
                $itemModal.find('#group_id').selectpicker('val', response.group_id);
                $itemModal.find('#convenio_id').selectpicker('val', response.convenio_id);
