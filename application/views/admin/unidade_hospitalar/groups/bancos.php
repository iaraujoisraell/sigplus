...
<h4 class="customer-profile-group-heading"><?php echo 'Bancos'; ?></h4>
      ...
      <div class="col-md-12">
        <div class="panel_s">
         ...
         <?php if(has_permission('items','','create')){ ?>
           <div class="_buttons">
            <a href="#" class="btn btn-info pull-left"  data-toggle="modal" data-target="#add_banco_modal" data-id=""><?php echo 'Add Banco'; ?></a>    
        ...
        render_datatable($table_data,'bancos'); ?>
      </div>
        </div>
      </div>
    </div>
</div>
<?php $this->load->view('admin/financeiro/cadastros/groups/novo_banco'); ?>
...
<script>
  ...
  $(function(){
    ...
    var CustomersServerParams = {};
       ...
       var tAPI = initDataTable('.table-bancos', admin_url+'financeiro/table_banco', [0], [0], CustomersServerParams, [1, 'desc']);
   ...
   function procedimentos_table_reload() {
     ...
     // });
       CustomersServerParams['categorias'] = '[name="categorias"]';
       CustomersServerParams['natureza'] = '[name="natureza"]'; 
     if ($.fn.DataTable.isDataTable('.table-bancos')) {
       $('.table-bancos').DataTable().destroy();
     }
      var tAPI = initDataTable('.table-bancos', admin_url+'financeiro/table_banco', [0], [0], CustomersServerParams, [1, 'desc']);
   // tAPI.ajax.reload();
  ...
  function items_bulk_action(event) {
    if (confirm_delete()) {
      ...
      var rows = $('.table-bancos').find('tbody tr');
      $.each(rows, function() {
