<div class="table-responsive">
   <table class="table table-bordered prifile_type no-margin">
      <thead>
         <tr>
            <th>Feature</th>
            <th>Capabilities</th>
      ...
      <tbody>
         ...
         foreach(get_available_staff_permissions_intranet($funcData) as $feature => $permission) { ?>
               ...
               <?php foreach ($permission['capabilities'] as $capability => $name) {
                  ...
                  if((isset($is_admin) && $is_admin) ||
                   ...
                   (
                      ...
                      &&
                        ((isset($member)
                         && staff_can_intranet(($capability == 'view' ? 'view_own' : 'view'), $feature, $member->staffid))
                        ||
                        (isset($role)
                         && has_role_permission_intranet($role->id, ($capability == 'view' ? 'view_own' : 'view'), $feature))
                      )
                   )
                  ){
                   $disabled = ' disabled ';
                  } else if((isset($member) && staff_can_intranet($capability, $feature, $member->staffid)) || isset($role) && has_role_permission_intranet($role->id, $capability, $feature)){
                  $checked = ' checked ';
                  }
                  ?>
               <div class="checkbox">
                  <input
                     ...
                     name="permissions_intranet[<?php echo $feature; ?>][]"
