...
$info     = '';
if ($proposal->assigned != 0 && get_option('show_sale_agent_on_invoices') == 1) {
    $info .= _l('sale_agent_string') . ': ' . get_staff_full_name($proposal->assigned) . '<br />';
}

if ($proposal->medicoid > 0) {
    $info .= _l('medico') . ': ' . get_medico_full_name($proposal->medicoid) . '<br />';
}

if ($proposal->convenio > 0) {
    $info .= _l('convenio') . ': ' . get_convenio_full_name($proposal->convenio) . '<br />';
}
