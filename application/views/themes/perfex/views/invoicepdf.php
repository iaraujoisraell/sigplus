...
if ($invoice->medicoid > 0) {
    $invoice_info .= _l('medico') . ': ' . get_medico_full_name($invoice->medicoid) . '<br />';
}

if ($invoice->convenio > 0) {
    $invoice_info .= _l('convenio') . ': ' . get_convenio_full_name($invoice->convenio) . '<br />';
}

if ($invoice->centrocustoid > 0) {
    $invoice_info .= 'Centro de Custo' . ': ' . get_centro_custo_full_name($invoice->centrocustoid) . '<br />';
}
...
if (count($invoice->payments) > 0 && get_option('show_transactions_on_invoice_pdf') == 1) {
    ...
    $tblhtml = '<table width="100%" bgcolor="#fff" cellspacing="0" cellpadding="5" border="0">
        ...
        <th width="10%;" style="' . $border . '">Pgto#</th>
        <th width="30%;" style="' . $border . '"> Conta Financeira</th>
        
        <th width="20%;" style="' . $border . '">' . _l('invoice_payments_table_mode_heading') . '</th>
        <th width="15%;" style="' . $border . '">' . _l('invoice_payments_table_date_heading') . '</th>
        <th width="15%;" style="' . $border . '">' . _l('invoice_payments_table_amount_heading') . '</th>
        <th width="10%;" style="' . $border . '"> NF</th>     
    </tr>';
    $tblhtml .= '<tbody>';
