...
if ($estimate->medicoid > 0) {
    $estimate_info .= _l('medico') . ': ' . get_medico_full_name($estimate->medicoid) . '<br />';
}

if ($estimate->convenio > 0) {
    $estimate_info .= _l('convenio') . ': ' . get_convenio_full_name($estimate->convenio) . '<br />';
}
