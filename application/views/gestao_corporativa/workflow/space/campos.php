...
<input type="hidden" id="fluxo_id" value="<?php echo $fluxo->id; ?>" />
<div class="row">
    <div class="col-md-12">
        <a href="#" class="btn btn-info mtop15 mbot10" id="teste" onclick="slideToggle('.usernote2<?php echo $tipo->id; ?>');
                                return false;"><?php echo 'ADD Campo' ?></a>
        <div class="clearfix"></div>
        <div class="usernote2<?php echo $tipo->id; ?> hide row" id="form">
...
<script>
    ...
    $(function () {
        ...
        CustomersServerParams['fluxo_id'] = '[id="fluxo_id"]';
        var tAPI = initDataTable('.table-campos', '<?php echo base_url(); ?>' + 'gestao_corporativa/Workflow/table_campos', [0], [0], CustomersServerParams, [1, 'desc']);
    });

    function delete_campo(id) {
        $.ajax({
            type: "POST",
            url: "<?php echo base_url('gestao_corporativa/Workflow/delete_campo'); ?>",
            data: {
                id: id, categoria_id: '<?php echo $tipo->id ?>', preenchido_por: '<?php echo $fluxo->id; ?>'
            },
    ...
    function add_campo() {
        ...
        var preenchido_por = '<?php echo $fluxo->id; ?>';
        $.ajax({
            type: "POST",
            url: "<?php echo base_url('gestao_corporativa/Workflow/save_campo'); ?>",
            data: {
                nome: nome, tam_coluna: tam_coluna, type: type, categoria_id: '<?php echo $tipo->id ?>', options: values, obrigatorio: obrigatorio, preenchido_por: preenchido_por
            },
            success: function (data) {
                reload_campos();
                slideToggle('.usernote2<?php echo $tipo->id; ?>');
                $('#form').html(data);
    ...
    function subir(ordem) {
        $.ajax({
            type: "POST",
            url: "<?php echo base_url('gestao_corporativa/Workflow/subir'); ?>",
            data: {
                ordem: ordem, categoria_id: '<?php echo $tipo->id ?>', preenchido_por: '<?php echo $fluxo->id; ?>'
            },
            success: function (data) {
                reload_campos();
    ...
    function descer(ordem) {
        $.ajax({
            type: "POST",
            url: "<?php echo base_url('gestao_corporativa/Workflow/descer'); ?>",
            data: {
                ordem: ordem, categoria_id: '<?php echo $tipo->id ?>', preenchido_por: '<?php echo $fluxo->id; ?>'
            },
            success: function (data) {
