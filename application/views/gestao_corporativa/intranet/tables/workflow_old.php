    ...
    if ($this->ci->input->post('setor') || $this->ci->input->post('assumidos') || $this->ci->input->post('setor_finalizados')) {
        if (!$this->ci->input->post('setor_finalizados')) {
            if ($aRow['atribuido_a'] != 0) {
                ...
                if ($aRow['staff_responsavel'] == get_staff_user_id()) {
                    ...
                    $assumido .= '<span data-toggle="tooltip" title="Opções do colaborador responsável"><i class="fa fa-caret-down" aria-hidden="true"></i></span>';
                    $assumido .= '</a>';
