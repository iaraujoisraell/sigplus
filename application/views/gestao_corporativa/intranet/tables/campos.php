...
if ($rel_type == 'internal_request_workflow' || $rel_type == 'external_request_workflow') {
    ...
    array_push($where, "AND " . db_prefix() . "_intranet_categorias_campo.rel_type != 'external_request_workflow'");
    array_push($where, "AND " . db_prefix() . "_intranet_categorias_campo.rel_type != 'internal_request_workflow'");
}
...
if ($rel_type == 'workflow' || $rel_type == 'r.o' || $rel_type == 'cdc') {
...
array_push($where, "AND (" . db_prefix() . "_intranet_categorias_campo.rel_type = '$rel_type' or  rel_type is null)");
...
foreach ($rResult as $aRow) {
    ...
    if ($rel_type != 'internal_request_workflow' and $rel_type != 'external_request_workflow') {
        ...
        //. '<div class="col-md-6"><a class="btn btn-info btn-xs w-100"  style="margin: 0; " onclick="subir(' . $aRow['ordem'] . ','."'".$preenchido_por."'".');"><i class="fa fa-arrow-up"></i></a></div> '
        //. '<div class="col-md-6"> <a class="btn btn-warning btn-xs w-100"  style="margin: 0;" onclick="descer(' . $aRow['ordem'] . ','."'".$preenchido_por."'".');"><i class="fa fa-arrow-down"></i></a></div></div>';
    }
    $row[] = '<p style="color: ' . $aRow['color'] . ';">' . $aRow['titulo'] . '</p><input type="hidden" value="' . $aRow['id'] . '" id="campo_ordem">';
