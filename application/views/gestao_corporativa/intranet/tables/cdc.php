...
if ($this->ci->input->post('loading')) {
    ...
    array_push($additionalSelect, "tbl_intranet_categorias_loading.staff_id as staff_id");
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, $additionalSelect, $group_by, "", $orderby, get_staff_user_id());
...
foreach ($rResult as $aRow) {
    ...
    if ($this->ci->input->post('categoria_id')) {
        ...
        $btn .= '<button class="btn btn-xs btn-default" title="Revisar Pessoas no Fluxo" onclick="revisar_pessoa('. $aRow['id'].')"><icon class="fa fa-user"></icon></button>';
    ...
    if ($this->ci->input->post('categoria_id')) {
        ...
        if ($aRow['pub'] == 1) {
            ...
            $titulo .= '<span class="label label-sm label-info">AGUARDANDO APROVAÇÃO</span>';
        }
    }
    $row[] = $titulo;

    if ($this->ci->input->post('loading')) {
        $responsability = strtoupper($aRow['responsabilidade']). '('. get_staff_full_name($aRow['staff_id']). ')' ;
