...
foreach ($rResult as $aRow) {
    ...
    $req = '';
    foreach ($ro as $r){
    $req .= '<a   href="' . base_url('gestao_corporativa/Registro_ocorrencia/registro/' . base64_encode(openssl_encrypt($r['id'], "aes-256-cbc", 'sigplus', 0, 'sigplus'))) . '" target="_blank">RO #' . $r['id'] . '</a><br>';
    }
    foreach ($workflow as $wf){
    $req .= '<a   target="_blank" href="' . base_url() . 'gestao_corporativa/workflow/pdf/' . $wf['id'] . '">WF #' . $wf['id'] . '</a><br>';
    }
