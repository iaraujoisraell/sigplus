...
if ($this->ci->input->post('portal')) {
  ...
  // $portais = $this->ci->input->post('portal'); 
  // print_r($portais); exit;
...
if ($this->ci->input->post('id')) {
    ...
    array_push($where, " AND tbl_intranet_registro_atendimento.id in(SELECT registro_atendimento_id from tbl_intranet_workflow WHERE id = $id_)");
}

if ($this->ci->input->post('categoria_id')) {
    $categoria_id = $this->ci->input->post('categoria_id');

    for($i = 0; $i < count($categoria_id); $i++){
        if($categoria_id[$i] == 9){
            $categoria_id[$i] .= ',0';
        }
    }
  
    $categoria_id = implode(',', $categoria_id);  // print_r($categoria_id); exit;
...
if ($this->ci->input->post('canal_atendimento_id')) {
    ...
    for($i = 0; $i < count($canal_atendimento_id); $i++){
        if($canal_atendimento_id[$i] == 10){
            $canal_atendimento_id[$i] = 0;
        }
    }
    
  //  print_r($canal_atendimento_id); exit;
  
    $canal_atendimento_id = implode(',', $canal_atendimento_id);  // print_r($canal_atendimento_id); exit;
