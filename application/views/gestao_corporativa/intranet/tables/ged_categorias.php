...
array_push($where, "AND " . db_prefix() . "_intranet_categorias.rel_type = 'ged'");
...
foreach ($rResult as $aRow) {
    ...
    $row[] = '<a class="btn btn-default btn-icon" data-toggle="modal" onclick="Campos('.$aRow['id'].');" ><i class="fa fa-list"></i> Visualizar</a>';
    $options = '';
    if (has_permission_intranet('registro_ocorrencia', '', 'edit_categoria') || is_admin()) {
    $options = '<a class="btn btn-info btn-xs mleft" onclick="Update_categoria('.$aRow['id'].');"><i class="fa fa-pencil"></i></a>';
    }
    if (has_permission_intranet('registro_ocorrencia', '', 'delete_categoria') || is_admin()) {
    $options .= '<a class="btn btn-danger btn-xs mleft5 _delete" data-toggle="tooltip" onclick="delete_categoria('.$aRow['id'].');"><i class="fa fa-trash"></i></a>';
    }
    $row[] = $options;
