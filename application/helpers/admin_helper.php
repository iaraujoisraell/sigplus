...
function init_head_menu_admin($aside = true)
{
    $CI = &get_instance();
    $CI->load->view('admin/includes/head');
    $CI->load->view('admin/includes/header', ['startedTimers' => $CI->misc_model->get_staff_started_timers()]);
...
function init_head_intranet_novo($aside = true)
{
    $CI = &get_instance();
    $CI->load->view('admin/includes/intranet/head');
    $CI->load->view('admin/includes/intranet/header');
    $CI->load->view('admin/includes/intranet/setup_menu');
    if ($aside == true) {
        //$CI->load->view('admin/includes/intranet/aside');
     //   $CI->load->view('gestao_corporativa/intranet/asidebar.php');
    }
}

function init_head_intranet($aside = true)
{
    $CI = &get_instance();
    $CI->load->view('admin/includes/intranet/head_intranet');
    
    //$CI->load->view('admin/includes/header_intranet');   
}

function init_head_intranet_versao1($aside = true)
{
    $CI = &get_instance();
    $CI->load->view('admin/includes/head_intranet_versai1');
    //$CI->load->view('admin/includes/header_intranet');   
}

function init_head_intranet_admin($aside = true)
{
    $CI = &get_instance();
    $CI->load->view('gestao_corporativa/intranet/includes/head_intranet');
    $CI->load->view('gestao_corporativa/intranet/includes/header_intranet');
   
}
function init_head_intranet2($aside = true)
{
    $CI = &get_instance();
    $CI->load->view('admin/includes/head_intranet');
    $CI->load->view('admin/includes/header_intranet');
...
function init_tail_intranet()
{
    $CI = &get_instance();
    $CI->load->view('admin/includes/scripts_intranet');
}

function init_tail_procedimentos()
{
    $CI = &get_instance();
    $CI->load->view('admin/includes/scripts_procedimentos');
}
/**
 * Get admin url
...
function staff_can_intranet($capability, $feature = null, $staff_id = '')
{
    $staff_id = empty($staff_id) ? get_staff_user_id() : $staff_id;
    ...
    if (!$permissions) {
        ...
        $permissions = $CI->staff_model->get_staff_permissions_intranet($staff_id);
    }
    ...
    if (!$feature) {
        ...
        return hooks()->apply_filters('staff_can_intranet', $retVal, $capability, $feature, $staff_id);
    }

    foreach ($permissions as $permission) {
        if ($feature == $permission['feature']
            && $capability == $permission['capability']) {
            return hooks()->apply_filters('staff_can_intranet', true, $capability, $feature, $staff_id);
        }
    }

    return hooks()->apply_filters('staff_can_intranet', false, $capability, $feature, $staff_id);
}
 ...
 * @return boolean
 */
function has_role_permission_intranet($type_id, $capability, $feature)
{
    $CI          = &get_instance();
    $permissions = $CI->Profile_type_intranet_model->get($type_id)->permissions;
...
function has_permission_intranet($permission, $staffid = '', $can = '')
{
    return staff_can_intranet($can, $permission, $staffid);
}
/**
 * @since 1.0.0
