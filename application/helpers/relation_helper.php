...
function get_relation_data($type, $rel_id = '')
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        $where_clients = '';
        if ($q) {
            $where_clients .= '(company LIKE "%' . $CI->db->escape_like_str($q) . '%" ESCAPE \'!\' OR CONCAT(firstname, " ", lastname) LIKE "%' . $CI->db->escape_like_str($q) . '%" ESCAPE \'!\' OR ' . db_prefix() . 'contacts.email LIKE "%' . $CI->db->escape_like_str($q) . '%" ESCAPE \'!\') AND ' . db_prefix() . 'clients.active = 1';
        }
        ...
        if ($rel_id != '') {
            ...
            $data = $CI->Registro_ocorrencia_model->get_ro_by_id($rel_id);
        } else {
            $CI->load->model('Registro_ocorrencia_model');
            $data   = $CI->Registro_ocorrencia_model->get();
        }
    } elseif ($type == 'staff') {
        if ($rel_id != '') {
    ...
    }elseif ($type == 'medicos' || $type == 'medico') {
        // Tasks only have relation with custom fields when searching on top
        if ($rel_id != '') {
            $data = $CI->Medicos_model->get($rel_id);
        }else{
            $data = $CI->Medicos_model->get();
        }
    }
...
function get_relation_values($relation, $type)
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        $link = base_url('gestao_corporativa/registro_ocorrencia/registro/' . $id);
    }   elseif ($type == 'medico' || $type == 'medicos') {
        if (is_array($relation)) {
            $id   = $relation['medicoid'];
            $name = $relation['nome_profissional'];
        } else {
            $id   = $relation->medicoid;
            $name = $relation->nome_profissional;
        }
...
function init_relation_options($data, $type, $rel_id = '')
{
    ...
    $has_permission_projects_view  = has_permission_intranet('projects', '', 'view');
