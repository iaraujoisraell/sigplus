...
{
    return hooks()->apply_filters('ajax_on_total_items', 5000);
}
 ...
 * @return array
 */
function get_fin_items_by_type($type, $id)
{
   
    $CI = &get_instance();
    $CI->db->select('*');
    $CI->db->from(db_prefix() . 'fin_itemable');
...
{
    $CI = &get_instance();
    $CI->db->select(db_prefix() . 'itemable.* ,' . db_prefix() . 'medicos.nome_profissional as medico, ' . db_prefix() . 'medicos.comissao as comissao,'.db_prefix() . 'items_groups.name as categoria');
    $CI->db->from(db_prefix() . 'itemable');
    $CI->db->join(db_prefix() . 'medicos', ''    . db_prefix() . 'medicos.medicoid = '    . db_prefix() . 'itemable.medicoid', 'left');
    
    $CI->db->join(db_prefix() . 'items', ''    . db_prefix() . 'items.id = '    . db_prefix() . 'itemable.item_id', 'left');
    $CI->db->join(db_prefix() . 'items_groups', ''    . db_prefix() . 'items_groups.id = '    . db_prefix() . 'items.group_id', 'left');
 ...
 */
function add_new_sales_item_post_conta_receber($item, $rel_id, $rel_type)
{
    $custom_fields = false;
    ...
    $valor_procedimento = $item['rate'];
    
    $qty = $item['qty'];
    $fat_total = ($valor_procedimento * $qty);
    $rate_desconto = $fat_total;
    ...
    $datos_items = [
                    ...
                    'rel_id'           => $rel_id,
                    'rate_desconto'         => $rate_desconto,
                    'rel_type'              => $rel_type,
                    'unit'                  => $item['unit']
                ];
    
    $CI->db->insert(db_prefix() . 'fin_itemable', $datos_items);
    $id = $CI->db->insert_id();
...
function add_new_sales_item_post($item, $rel_id, $rel_type,$destino_desconto)
{
    $custom_fields = false;
    ...
    $porcentagem_deconto = ($desconto_valor * 100)/$valor_procedimento;
    $porcentagem_deconto = substr($porcentagem_deconto, 0, 5);
    $qty = $item['qty'];
    $fat_total = ($valor_procedimento * $qty);
    $rate_desconto = $fat_total-$desconto_valor;
    
  
    if($destino_desconto){
        $destino_desconto = $item['destino_desconto'];
    }else{
        $destino_desconto = 'EMPRESA';
        }
    ...
    $datos_items = [
                    ...
                    'qty'              => $item['qty'],
                    'rate'             => number_format($item['rate'], get_decimal_places(), '.', ''),
                    'desconto_valor'   => number_format($item['desconto_valor'], get_decimal_places(), '.', ''),
                    'rel_id'           => $rel_id,
                    'desconto_porcentagem'  => $porcentagem_deconto,
                    'rate_desconto'         => $rate_desconto,
                    'rel_type'              => $rel_type,
                    'unit'                  => $item['unit'],
                    'medicoid'              => $item['medicoid'],
                    'item_id'               => $item['item_id'],
                    'destino_desconto'      => $destino_desconto
                ];
   
    $CI->db->insert(db_prefix() . 'itemable', $datos_items);
    $id = $CI->db->insert_id();
...
function update_sales_item_post($item_id, $data, $field = '')
{
    ...
    if ($field !== '') {
        ...
        } elseif ($field == 'desconto_porcentagem') {
            $valor_procedimento = $data['rate'];
            $desconto_valor = $data['desconto_valor'];
            $porcentagem_deconto = ($desconto_valor * 100)/$valor_procedimento;
            $porcentagem_deconto = substr($porcentagem_deconto, 0, 5);
            $update[$field] = $porcentagem_deconto;
        } elseif ($field == 'rate_desconto') {
            $valor_procedimento = $data['rate'];
            $qty = $data['qty'];
            $desconto_valor = $data['desconto_valor'];
            $fat_total = ($valor_procedimento * $qty);
            $rate_desconto = $fat_total-$desconto_valor;
            $update[$field] = $rate_desconto;
        } elseif ($field == 'destino_desconto') {
            $update[$field] = $data['destino_desconto'];
        } else {
