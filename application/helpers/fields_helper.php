 ...
 * @return string
 */
function render_input($name, $label = '', $value = '', $type = 'text', $input_attrs = [], $form_group_attr = [], $form_group_class = '', $input_class = '', $obrigatorio = false)
{
    $input            = '';
    $_form_group_attr = '';
    $_input_attrs     = '';
    if($obrigatorio == true){
         $asterisk = '<small class="req text-danger">* </small>';
    }
    ...
    if ($label != '') {
        $input .= '<label for="' . $name . '" class="control-label">' . $asterisk. _l($label, '', false) . '</label>';
    }
...
function render_date_input_readonly($name, $label = '', $value = '', $input_attrs = [], $form_group_attr = [], $form_group_class = '', $input_class = '')
{
    $input            = '';
    $_form_group_attr = '';
    $_input_attrs     = '';
    ...
    $input .= '<div class="input-group date">';
    $input .= '<input readonly="true" type="text" id="' . $name . '" name="' . $name . '" class="form-control ' . $input_class . '" ' . $_input_attrs . ' value="' . set_value($name, $value) . '" autocomplete="off">';
    $input .= '<div class="input-group-addon">
    <i class="fa fa-calendar calendar-icon"></i>
</div>';
    $input .= '</div>';
 ...
 * @return string
 */
function render_textarea($name, $label = '', $value = '', $textarea_attrs = [], $form_group_attr = [], $form_group_class = '', $textarea_class = '', $obrigatorio = false)
{
    if($obrigatorio == true){
         $asterisk = '<small class="req text-danger">* </small>';
    }
    ...
    if ($label != '') {
        $textarea .= '<label for="' . $name . '" class="control-label">' . $asterisk . _l($label, '', false) . '</label>';
    }
...
function render_select_required_true($name, $options, $option_attrs = [], $label = '', $selected = '', $select_attrs = [], $form_group_attr = [], $form_group_class = '', $select_class = '', $include_blank = true)
{
    $callback_translate = '';
    ...
    $select .= '<select id="' . $name . '" name="' . $name . '" class="selectpicker' . $select_class . '" ' . $_select_attrs . ' data-live-search="true" required="true">';
...
function render_select($name, $options, $option_attrs = [], $label = '', $selected = '', $select_attrs = [], $form_group_attr = [], $form_group_class = '', $select_class = '', $include_blank = true, $obrigatorio = false, $style_label = '')
{
    if($obrigatorio == true){
         $asterisk = '<small class="req text-danger">* </small>';
    }
    ...
    $select .= '<div class="select-placeholder form-group' . $form_group_class . '" ' . $_form_group_attr . '>';
    if ($label != '') {
        $select .= '<label for="' . $name . '" class="control-label" '.$style_label.'>'. $asterisk . _l($label, '', false) . '</label>';
    }
...
if (!function_exists('render_form_builder_field')) {
    ...
    function render_form_builder_field($field)
    {
        ...
        if ($type == 'header' || $type == 'paragraph') {
            ...
            echo '<div class="form-group" style="padding: 020px 020px 020px 020px; margin: 12px 12px 12px 0px; border-radius: 5px 5px 5px 5px; background-color: #f5f8fb; font-size: 16px;" data-type="' . $type . '" data-name="' . $field->name . '" data-required="' . (isset($field->required) ? true : 'false') . '">';
            echo '<label class="control-label" style="padding: 020px 020px 020px 020px; margin: 12px 12px 12px 0px; border-radius: 5px 5px 5px 5px; background-color: #f5f8fb; font-size: 16px;" for="' . $field->name . '">' . (isset($field->required) ? ' <span class="text-danger">* </span> ': '') . $field->label . '' . (isset($field->description) ? ' <i class="fa fa-question-circle" data-toggle="tooltip" data-title="' . $field->description . '" data-placement="' . (is_rtl(true) ? 'left' : 'right') . '"></i>' : '') . '</label>';
         ...
         <input' . (isset($field->required) ? ' required="true"': '') . ' placeholder="' . (isset($field->placeholder) ? $field->placeholder : '') . '" type="text"' . (isset($field->value) ? ' value="' . $field->value . '"' : '') . ' name="' . $field->name . '" id="' . $field->name . '" class="' . (isset($field->className) ? $field->className : '') . '" />
             <span class="input-group-addon"><i></i></span>
                ...
                echo '<input style="padding: 020px 020px 020px 020px; margin: 12px 12px 12px 0px; border-radius: 5px 5px 5px 5px; background-color: #f5f8fb; font-size: 16px;" ' . (isset($field->required) ? ' required="true"': '') . (isset($field->placeholder) ? ' placeholder="' . $field->placeholder . '"' : '') . ' type="' . $ftype . '" name="' . $field->name . '" id="' . $field->name . '" class="' . (isset($field->className) ? $field->className : '') . '" value="' . (isset($field->value) ? $field->value : '') . '"' . ($field->type == 'file' ? ' accept="' . get_form_accepted_mimes() . '" filesize="' . file_upload_max_size() . '"' : '') . '>';
            } elseif ($type == 'textarea') {
                echo '<textarea style="padding: 020px 020px 020px 020px; margin: 12px 12px 12px 0px; border-radius: 5px 5px 5px 5px; background-color: #f5f8fb; font-size: 16px;"' . (isset($field->required) ? ' required="true"': '') . ' id="' . $field->name . '" name="' . $field->name . '" rows="' . (isset($field->rows) ? $field->rows : '4') . '" class="' . (isset($field->className) ? $field->className : '') . '" placeholder="' . (isset($field->placeholder) ? $field->placeholder : '') . '">' . (isset($field->value) ? $field->value : '') . '</textarea>';
            } elseif ($type == 'date') {
                echo '<input style="padding: 020px 020px 020px 020px; margin: 12px 12px 12px 0px; border-radius: 5px 5px 5px 5px; background-color: #f5f8fb; font-size: 16px;"' . (isset($field->required) ? ' required="true"': '') . ' placeholder="' . (isset($field->placeholder) ? $field->placeholder : '') . '" type="text" class="' . (isset($field->className) ? $field->className : '') . ' datepicker" name="' . $field->name . '" id="' . $field->name . '" value="' . (isset($field->value) ? _d($field->value) : '') . '">';
            } elseif ($type == 'datetime-local') {
                echo '<input style="padding: 020px 020px 020px 020px; margin: 12px 12px 12px 0px; border-radius: 5px 5px 5px 5px; background-color: #f5f8fb; font-size: 16px;"' . (isset($field->required) ? ' required="true"': '') . ' placeholder="' . (isset($field->placeholder) ? $field->placeholder : '') . '" type="text" class="' . (isset($field->className) ? $field->className : '') . ' datetimepicker" name="' . $field->name . '" id="' . $field->name . '" value="' . (isset($field->value) ? _dt($field->value) : '') . '">';
            } elseif ($type == 'select') {
                echo '<select style="padding: 020px 020px 020px 020px; margin: 12px 12px 12px 0px; border-radius: 5px 5px 5px 5px; background-color: #f5f8fb; font-size: 16px;"' . (isset($field->required) ? ' required="true"': '') . '' . (isset($field->multiple) ? ' multiple="true"' : '') . ' class="' . (isset($field->className) ? $field->className : '') . '" name="' . $field->name . (isset($field->multiple) ? '[]' : '') . '" id="' . $field->name . '"' . (isset($field->values) && count($field->values) > 10 ? 'data-live-search="true"': '') . 'data-none-selected-text="' . (isset($field->placeholder) ? $field->placeholder : '') . '">';
                $values = [];
                ...
                if (isset($field->values) && count($field->values) > 0) {
                    ...
                    foreach ($field->values as $checkbox) {
                        ...
                        echo '<input style="padding: 020px 020px 020px 020px; margin: 12px 12px 12px 0px; border-radius: 5px 5px 5px 5px; background-color: #f5f8fb; font-size: 16px;"' . (isset($field->required) ? ' required="true"': '') . ' class="' . (isset($field->className) ? $field->className : '') . '" type="checkbox" id="chk_' . $field->name . '_' . $i . '" value="' . $checkbox->value . '" name="' . $field->name . '[]"' . (isset($checkbox->selected) ? ' checked' : '') . '>';
...
if (!function_exists('render_formulario_builder_field')) {
    /**
     * Used for customer forms eq. leads form, builded from the form builder plugin
     * @param  object $field field from database
     * @return mixed
     */
    function render_formulario_builder_field($field)
    {
        ...
        echo '<div class="' . $classNameCol . ' articles_list">';
        if ($type == 'header' || $type == 'paragraph') {
            echo '<' . $field['subtype'] . ' class="' . (isset($field['className']) ? $field['className'] : '') . '">' . check_for_links(nl2br($field['label'])) .  '</' . $field['subtype'] . '>';
        } else {
            echo '<div class="form-group" style="padding: 020px 020px 020px 020px; margin: 12px 12px 12px 0px; border-radius: 5px 5px 5px 5px; background-color: #f5f8fb; font-size: 16px;" data-type="' . $type . '" data-name="' . $field['name'] . '" data-required="' . (isset($field['required']) ? true : 'false') . '">';
         ...
         <input' . (isset($field['required']) ? ' required="true"': '') . ' placeholder="' . (isset($field['placeholder']) ? $field['placeholder'] : '') . '" type="text"' . (isset($field->value) ? ' value="' . $field['value'] . '"' : '') . ' name="' . $field['name'] . '" id="' . $field['name'] . '" class="' . (isset($field['className']) ? $field['className'] : '') . '" />
             <span class="input-group-addon"><i></i></span>
                ...
                $ftype = isset($field['subtype']) ? $field['subtype'] : $type;
                echo '<input style="padding: 020px 020px 020px 020px; margin: 12px 12px 12px 0px; border-radius: 5px 5px 5px 5px;  font-size: 16px;"' . (isset($field['required']) ? ' required="true"': '') . (isset($field['placeholder']) ? ' placeholder="' . $field['placeholder'] . '"' : '') . ' type="' . $ftype . '" name="' . $field['name'] . '" id="' . $field['name'] . '" class="' . (isset($field['className']) ? $field['className'] : '') . '" value="' . (isset($field['value']) ? $field['value'] : '') . '"' . ($field['type'] == 'file' ? ' accept="' . get_form_accepted_mimes() . '" filesize="' . file_upload_max_size() . '"' : '') . '>';
            } elseif ($type == 'textarea') {
                echo '<textarea style="padding: 020px 020px 020px 020px; margin: 12px 12px 12px 0px; border-radius: 5px 5px 5px 5px;  font-size: 16px;"' . (isset($field['required']) ? ' required="true"': '') . ' id="' . $field['name'] . '" name="' . $field['name'] . '" rows="' . (isset($field->rows) ? $field->rows : '4') . '" class="' . (isset($field['className']) ? $field['className'] : '') . '" placeholder="' . (isset($field['placeholder']) ? $field['placeholder'] : '') . '">' . (isset($field['value']) ? $field['value'] : '') . '</textarea>';
            } elseif ($type == 'date') {
                echo '<input style="padding: 020px 020px 020px 020px; margin: 12px 12px 12px 0px; border-radius: 5px 5px 5px 5px;  font-size: 16px;"' . (isset($field['required']) ? ' required="true"': '') . ' placeholder="' . (isset($field['placeholder']) ? $field['placeholder'] : '') . '" type="date" class="' . (isset($field['className']) ? $field['className'] : '') . ' " name="' . $field['name'] . '" id="' . $field['name'] . '" value="' . (isset($field['value']) ? _d($field['value']) : '') . '">';
            } elseif ($type == 'datetime') {
                echo '<input style="padding: 020px 020px 020px 020px; margin: 12px 12px 12px 0px; border-radius: 5px 5px 5px 5px;  font-size: 16px;"' . (isset($field['required']) ? ' required="true"': '') . ' placeholder="' . (isset($field['placeholder']) ? $field['placeholder'] : '') . '" type="text" class="' . (isset($field['className']) ? $field['className'] : '') . ' datetimepicker" name="' . $field['name'] . '" id="' . $field['name'] . '" value="' . (isset($field['value']) ? _dt($field['value']) : '') . '">';
            } elseif ($type == 'select') {
                $dados_itens = $field['dados_itens'];
               
                
                $pergunta_id = $dados_itens[0]['pergunta_id'];
                echo '<select ' . (isset($field['required']) ? ' required="true"': '') . '' . ' class="' . $className . '" name="' . $field['name'] .  '" id="' . $field['name'] . '"' . (isset($field['values']) && count($field['values']) > 10 ? 'data-live-search="true"': '') . 'data-none-selected-text="' . (isset($field['placeholder']) ? $field['placeholder'] : '') . '">';
                $values = [];
                
                if (isset($dados_itens) && count($dados_itens) > 0) {
                    foreach ($dados_itens as $option) {
                        echo '<option value="' . $option['name'] . '" ' . (isset($option->selected) ? ' selected' : '') . '>' . $option['name'] . '</option>';
                    }
                }
                echo '</select>';
            } elseif ($type == 'caixa_selecao') {
                $dados_itens = $field['dados_itens'];
                $values = [];
                if (isset($dados_itens) && count($dados_itens) > 0) {
                    $i = 0;
                    echo '<div class="chk">';
                    foreach ($dados_itens as $checkbox) {
                        echo '<div class="checkbox' . ((isset($field->inline) && $field->inline == 'true') || (isset($field['className']) && strpos($field['className'], 'form-inline-checkbox') !== false) ? ' checkbox-inline' : '') . '">';
                        echo '<input ' . (isset($field['required']) ? ' required="true"': '') . '  type="checkbox" id="chk_' . $field['name'] . '_' . $i . '" value="' . $checkbox['name'] . '" name="' . $field['name'] . '[]"' . (isset($checkbox->selected) ? ' checked' : '') . '>';
                        echo '<label for="chk_' . $field['name'] . '_' . $i . '">';
                        echo $checkbox['name'];
                        echo '</label>';
                        echo '</div>';
                ...
                if (isset($dados_itens) && count($dados_itens) > 0) {
                 ...
                 //   echo '<div class="chk">';
                    foreach ($dados_itens as $checkbox) {
                        ...
                        echo '<div class="' . ((isset($field->inline) && $field->inline == 'true') || (isset($field['className']) && strpos($field['className'], 'form-inline-checkbox') !== false) ? ' checkbox-inline' : '') . '">';
                        echo '<input style="padding: 020px 020px 020px 020px; margin: 12px 12px 12px 0px; border-radius: 5px 5px 5px 5px; background-color: #f5f8fb; font-size: 16px;"' . (isset($field['required']) ? ' required="true"': '') . '  type="radio" id="' . $field['name'] . '" value="' . $checkbox['name'] . '"  name="' . $field['name'] . '"'  . '>';
                        echo '<label for="chk_' . $checkbox['name'] . '_' . $i . '">';
                        ?>
