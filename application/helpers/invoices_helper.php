...
function get_fin_invoice_total_left_to_pay($id, $invoice_total = null)
{
    $CI = & get_instance();
    ...
    if ($invoice_total === null) {
        $CI->db->select('total')
        ->where('id', $id);
        $invoice_total = $CI->db->get(db_prefix() . 'fin_invoices')->row()->total;
    }
    ...
    $payments = $CI->Financeiro_model->get_invoice_payments($id);
...
function update_fin_invoice_status($id, $force_update = false, $prevent_logging = false)
{
    $CI = & get_instance();
    
    $CI->load->model('Financeiro_model');
    $invoice = $CI->Financeiro_model->get_conta_receber($id);
    ...
    // AJUSTA O VALOR DO TOTAL
    // TOTAL = SUBTOTAL - IMPOSTO - GLOSA
    $taxas = $invoice->total_tax;
    if(!$taxas){
        $taxas = 0;
    }
    
    //DESCONTO
    $desconto = $invoice->discount_total;
    if(!$desconto){
        $desconto = 0;
    }
    
    // VALOR TOTAL GLOSADO ANTES
    $glosas_antes = $CI->Financeiro_model->get_glosas_invoice($id, 1);
    $totalGlosas_antes = [];
    foreach ($glosas_antes as $payment) {
        array_push($totalGlosas_antes, $payment['amount']);
    }
    $totalGlosas_antes = array_sum($totalGlosas_antes);
    if(!$totalGlosas_antes){
        $totalGlosas_antes = 0;
    }
    
    // VALOR TOTAL GLOSADO DEPOIS DO IMPOSTO
    $glosas_depois = $CI->Financeiro_model->get_glosas_invoice($id, 2);
    $totalGlosas_depois = [];
    foreach ($glosas_depois as $payment) {
        array_push($totalGlosas_depois, $payment['amount']);
    }
    $totalGlosas_depois = array_sum($totalGlosas_depois);
    if(!$totalGlosas_depois){
        $totalGlosas_depois = 0;
    }
    
    $total_glosas = 0;
    $total_glosas = $totalGlosas_antes + $totalGlosas_depois;
    $data_fat['glosa_pos'] = $totalGlosas_depois;
    $data_fat['glosa_antes'] = $totalGlosas_antes;
    
   
    $data_fat['total'] = $invoice->subtotal - $taxas - $total_glosas - $desconto;
    $CI->db->where('id', $id);
    $CI->db->update(db_prefix() . 'fin_invoices', $data_fat);
            
    
    
    
    $payments = $CI->Financeiro_model->get_pagamentos_invoice($id);
    $totalPayments = [];
    $status        = invoices_model::STATUS_UNPAID;
    ...
    $altera_status =  $CI->Financeiro_model->update_status_invoice($id, $status);
