 ...
 * @return string
 */
function format_members_by_ids_and_names($ids, $names, $hidden_export_table = true, $image_class = 'staff-profile-image-small', $intranet = false) {
    $outputAssignees = '';
    $exportAssignees = '';
    ...
    foreach ($assignees as $key => $assigned) {
        ...
        if ($assigned != '') {
            if($intranet == false){
                $caminho = admin_url('profile/' . $assignee_id);
            } else {
                $caminho = admin_url('staff/member/'.$assignee_id.'?intranet=intranet');
            }
            $outputAssignees .= '<a href="' . $caminho . '" target="_blanck">' .
...
function init_relation_tasks_table($table_attributes = []) {
    ...
    _l('task_add_edit_what'),
    _l('task_add_edit_where'),
    _l('task_add_edit_how'),
    ];
    ...
    if (has_permission('tasks', '', 'create')) {
        ...
        if ($table_attributes['data-new-rel-type'] != 'project') {
           // echo "<a href='#' class='btn btn-info pull-left mbot25 mright5 new-task-relation" . $disabled . "' onclick=\"new_task_from_relation('$table_name'); return false;\" data-rel-id='" . $table_attributes['data-new-rel-id'] . "' data-rel-type='" . $table_attributes['data-new-rel-type'] . "'> Nova Ação</a>";
        }
    }
    ...
    if ($table_attributes['data-new-rel-type'] == 'project') {
        ...
        echo '</div>';
    } elseif ($table_attributes['data-new-rel-type'] == 'record') {
        echo "<a href='" . admin_url('tasks/detailed_overview?record_id=' . $table_attributes['data-new-rel-id']) . "' class='btn btn-success pull-right mbot25'>" . _l('detailed_overview') . '</a>';
        echo "<a href='" . admin_url('tasks/list_tasks?record_id=' . $table_attributes['data-new-rel-id'] . '&kanban=true') . "' class='btn btn-default pull-right mbot25 mright5 hidden-xs'>" . _l('view_kanban') . '</a>';
        echo '<div class="clearfix"></div>';
        echo $CI->load->view('admin/tasks/_bulk_actions', ['table' => '.table-rel-tasks'], true);
        echo $CI->load->view('admin/tasks/_summary', ['rel_id' => $table_attributes['data-new-rel-id'], 'rel_type' => 'record', 'table' => $table_name], true);
        echo '<a href="#" data-toggle="modal" data-target="#tasks_bulk_actions" class="hide bulk-actions-btn table-btn" data-table=".table-rel-tasks">' . _l('bulk_actions') . '</a>';
    }
    echo "<div class='clearfix'></div>";
