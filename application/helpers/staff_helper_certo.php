...
function get_available_staff_permissions_intranet($data = []) {
    ...
    $corePermissions = [
        ...
        'departments' => [
            'name' => _l('departments').' - ÁREA ADMINISTRATIVA',
            'capabilities' => $withoutViewOwnPermissionsArray,
        ],
        // --- APLICADO
        'tipos_usuarios' => [
        ...
        'workflow_settings' => [
            ...
            'capabilities' => [
                ...
                'duplicate_categoria' => 'Duplicar Categorias',
            ],
        ],
        'ro_settings' => [
...
function staff_profile_image_caminho($id, $type = 'small') {
    ...
    //return $url;
    if (!file_exists('uploads/staff_profile_images/' . $id . '/' . $type . '_' . $result->profile_image)) {
        $url = base_url('assets/images/user-placeholder.jpg');
    }
 ...
 * @return string Firstname and Lastname
 */
function get_departamento_nome($departmentid = '', $abrv = false) {
    $CI = & get_instance();
    ...
    $department = $CI->db->select('name, abreviado')->from(db_prefix() . 'departments')->get()->row();
    if($abrv == true) {

        return html_escape($department ? $department->abreviado : '');
    } else {
