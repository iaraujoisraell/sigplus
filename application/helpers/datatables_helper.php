 ...
 * @return array
 */
function data_tables_init($aColumns, $sIndexColumn, $sTable, $join = [], $where = [], $additionalSelect = [], $sGroupBy = '', $searchAs = [], $orderBy = '', $staff = '')
{
    //echo 'aqui'; exit;
    $CI = &get_instance();
    ...
    $sWhere
    " . $where . "
    $sGroupBy $sOrder $sLimit
    ";
    if ($staff == '1490') {
       // echo 'QUERY AGRUPADA: ' . date('H:i:s') . '<br>';
       //echo $sQuery; exit;  
    }
   // echo $sQuery; exit;
    $rResult = $CI->db->query($sQuery)->result_array();
    if ($staff == '878') {
       // echo '<br>QUERY REALIZADA: ' . date('H:i:s') . '<br>';
    }
    ...
    if ($staff == '878') {
       // echo 'QUERY ORDENADA: ' . date('H:i:s') . '<br>';
    }
    ...
    if ($staff == '878') {
        //echo 'segunda query realizada: ' . date('H:i:s') . '<br>'; exit;
    } 
...
function data_tables_init_escala_fixa($aColumns, $sIndexColumn, $sTable, $join = [], $where = [], $additionalSelect = [], $sGroupBy = '', $searchAs = [], $orderBy = '')
{
    $CI = &get_instance();
    $__post = $CI->input->post();
    $havingCount = '';
...
function data_tables_init_calendario($aColumns, $sIndexColumn, $sTable, $join = [], $where = [], $additionalSelect = [], $sGroupBy = '', $searchAs = [], $reportfrom)
{
    $CI = &get_instance();
    $__post = $CI->input->post();
    $havingCount = '';
    ...
    return [
        'rResult' => $rResult,
        'output' => $output,
        'data_selecionada' => $reportfrom,
    ];
}

function data_tables_init_order($aColumns, $sIndexColumn, $sTable, $join = [], $where = [], $additionalSelect = [], $sGroupBy = '', $sOrder = '', $searchAs = [])
{
    $CI = &get_instance();
    $__post = $CI->input->post();
    $havingCount = '';
