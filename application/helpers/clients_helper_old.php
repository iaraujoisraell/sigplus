...
function get_medico_profile_tabs() {

    return get_instance()->app_tabs->get_medico_profile_tabs();
}

function get_financeiro_profile_tabs() {

    return get_instance()->app_tabs->get_financeiro_profile_tabs();
}

function get_intranet_profile_tabs() {
    return get_instance()->app_tabs->get_intranet_profile_tabs();
}

function get_unidade_hospitalar_tabs() {

    return get_instance()->app_tabs->get_unidade_hospitalar_tabs();
}
...
function app_init_customer_profile_tabs() {
    ...
    if ($_SESSION['intranet'] == true) {
        $CI->app_tabs->add_customer_profile_tab('ra', [
            'name' => 'Registros de atendimento',
            'icon' => 'fa fa-calendar',
            'view' => 'admin/clients/groups/ra',
            'position' => 10,
        ]);
        $CI->app_tabs->add_customer_profile_tab('workflow', [
            'name' => 'Workflow',
            'icon' => 'fa fa-spinner',
            'view' => 'admin/clients/groups/workflow',
            'position' => 15,
        ]);
        $CI->app_tabs->add_customer_profile_tab('ro', [
            'name' => 'Registros de Ocorrência',
            'icon' => 'fa fa-exclamation',
            'view' => 'admin/clients/groups/ro',
            'position' => 20,
        ]);
        $CI->app_tabs->add_customer_profile_tab('sms', [
            'name' => 'SMS',
            'icon' => 'fa fa-commenting',
            'view' => 'admin/clients/groups/sms',
            'position' => 25,
        ]);
        $CI->app_tabs->add_customer_profile_tab('email', [
            'name' => 'Emails',
            'icon' => 'fa fa-envelope-o',
            'view' => 'admin/clients/groups/email',
            'position' => 30,
        ]);
    }

    if (!$_SESSION['intranet']) {
        $CI->app_tabs->add_customer_profile_tab('visitas', [
            'name' => 'Atendimentos',
            'icon' => 'fa fa-calendar',
            'view' => 'admin/clients/groups/visitas',
            'position' => 10,
        ]);
        $CI->app_tabs->add_customer_profile_tab('historico', [
            'name' => _l('historico'),
            'icon' => 'fa fa-file-text-o',
            'view' => 'admin/clients/groups/anamnese',
            'position' => 15,
        ]);
        ...
        $CI->app_tabs->add_customer_profile_tab('palavras_chaves', [
            'name' => _l('palavras_chaves'),
            'icon' => 'fa fa-list',
            'view' => 'admin/clients/groups/palavras_chaves',
            'position' => 25,
        ]);
        ...
        $CI->app_tabs->add_customer_profile_tab('nota_fiscal', [
            'name' => 'Nota Fiscal',
            'icon' => 'fa fa-paperclip',
            'view' => 'admin/clients/groups/nota_fiscal',
            'position' => 85,
        ]);
    ...
    $CI->app_tabs->add_customer_profile_tab('anotacoes', [
        'name' => _l('anotacoes'),
    ...
    /*
      ...
      $CI->app_tabs->add_customer_profile_tab('propostas', [
     ...
     * MENU INTRANET
     */
    if (has_permission_intranet('portal', '', 'view') || is_admin()) {
        $CI->app_tabs->add_intranet_profile_tab('portal', [
            'name' => 'Portal Cliente',
            'icon' => 'fa fa-user-o',
            'view' => 'gestao_corporativa/intranet/cadastros/groups/portal',
            'position' => 10,
        ]);
    }

    if (has_permission_intranet('company', '', 'view') || is_admin()) {
        $CI->app_tabs->add_intranet_profile_tab('company', [
            'name' => 'Empresa',
            'icon' => 'fa fa-building-o',
            'view' => 'gestao_corporativa/intranet/cadastros/groups/company',
            'position' => 20,
        ]);
    }

    if (has_permission_intranet('publicacoes', '', 'view') || is_admin()) {
        $CI->app_tabs->add_intranet_profile_tab('publicacoes', [
            'name' => 'Publicações',
            'icon' => 'fa fa-list-ol',
            'view' => 'gestao_corporativa/intranet/cadastros/groups/publicacao',
            'position' => 30,
        ]);
    }
    /*
      //if (!has_permission_intranet('publicacoes', '', 'view') || is_admin()) {
      $CI->app_tabs->add_intranet_profile_tab('arquivos', [
      'name'     => 'Arquivos  --- ATENCAO',
      'icon'     => 'fa fa-users',
      'view'     => 'gestao_corporativa/intranet/cadastros/groups/arquivo',
      'visible'  => true,
      'position' => 40,
      ]);
      //}
     * */
    if (has_permission_intranet('lisks_destaque', '', 'view') || is_admin()) {
        $CI->app_tabs->add_intranet_profile_tab('links_destaque', [
            'name' => 'Links Destaque',
            'icon' => 'fa fa-link',
            'view' => 'gestao_corporativa/intranet/cadastros/groups/link_destaque',
            'position' => 50,
        ]);
    }
    if (has_permission_intranet('lisks_externos', '', 'view') || is_admin()) {
        $CI->app_tabs->add_intranet_profile_tab('links', [
            'name' => 'Links Externos',
            'icon' => 'fa fa-list',
            'view' => 'gestao_corporativa/intranet/cadastros/groups/link',
            'position' => 60,
        ]);
    }
    if (has_permission_intranet('eventos', '', 'view') || is_admin()) {
        $CI->app_tabs->add_intranet_profile_tab('eventos', [
            'name' => 'Eventos',
            'icon' => 'fa fa-calendar',
            'view' => 'gestao_corporativa/intranet/cadastros/groups/evento',
            'position' => 70,
        ]);
    }
    if (has_permission_intranet('menus_paginas', '', 'view') || is_admin()) {
        $CI->app_tabs->add_intranet_profile_tab('menu', [
            'name' => 'Menus / Paginas',
            'icon' => 'fa fa-list-ul',
            'view' => 'gestao_corporativa/intranet/cadastros/groups/menu',
            'position' => 80,
        ]);
    }
    if (has_permission_intranet('grupos', '', 'view') || is_admin()) {
        $CI->app_tabs->add_intranet_profile_tab('grupos', [
            'name' => 'Grupos',
            'icon' => 'fa fa-users',
            'view' => 'gestao_corporativa/intranet/cadastros/groups/grupo',
            'position' => 90,
        ]);
    }
    /*
      //if (has_permission_intranet('publicacoes', '', 'view') || is_admin()) {
      $CI->app_tabs->add_intranet_profile_tab('gestao_arquivos', [
      'name'     => 'Gestão de Arquivos  --- ATENCAO',
      'icon'     => 'fa fa-folder',
      'view'     => 'gestao_corporativa/intranet/cadastros/groups/gestao_arquivos',
      'position' => 100,
      ]);
      //}
      //if (has_permission_intranet('publicacoes', '', 'view') || is_admin()) {
      $CI->app_tabs->add_intranet_profile_tab('login', [
      'name'     => 'Tela de Login --- ATENCAO',
      'icon'     => 'fa fa-sign-in',
      'view'     => 'gestao_corporativa/intranet/cadastros/groups/login',
      'position' => 110,
      ]);
      //}
     * 
     */
    if (has_permission_intranet('staff', '', 'view') || is_admin()) {
        $CI->app_tabs->add_intranet_profile_tab('staffs', [
            'name' => 'Colaboradores',
            'icon' => 'fa fa-users',
            'view' => 'admin/staff/manage',
            'position' => 120,
        ]);
    }

    if (has_permission_intranet('departments', '', 'view') || is_admin()) {
        $CI->app_tabs->add_intranet_profile_tab('departments', [
            'name' => 'Departamentos',
            'icon' => 'fa fa-briefcase',
            'view' => 'admin/departments/manage',
            'position' => 140,
        ]);
    }
    if (has_permission_intranet('tipos_usuarios', '', 'view') || is_admin()) {
        $CI->app_tabs->add_intranet_profile_tab('profile_type', [
            'name' => 'Tipos de Usuários',
            'icon' => 'fa fa-user',
            'view' => 'gestao_corporativa/intranet/cadastros/groups/profile_type',
            'position' => 150,
        ]);
    }
    if (has_permission_intranet('workflow_settings', '', 'view') || is_admin()) {
        $CI->app_tabs->add_intranet_profile_tab('workflow_settings', [
            'name' => 'Workflow - Configurações',
            'icon' => 'fa fa-spinner',
            'view' => 'gestao_corporativa/intranet/cadastros/groups/workflow_settings',
            'position' => 160,
        ]);
    }

    if (has_permission_intranet('ro_settings', '', 'view') || is_admin()) {
        $CI->app_tabs->add_intranet_profile_tab('ro_settings', [
            'name' => 'RO - Configurações',
            'icon' => 'fa fa-exclamation',
            'view' => 'gestao_corporativa/intranet/cadastros/groups/ro_settings',
            'position' => 170,
        ]);
    }

    if (has_permission_intranet('atendimento_settings', '', 'view') || is_admin()) {
        $CI->app_tabs->add_intranet_profile_tab('ra_settings', [
            'name' => 'RA - Configurações',
            'icon' => 'fa fa-list-alt',
            'view' => 'gestao_corporativa/intranet/cadastros/groups/ra_settings',
            'position' => 180,
        ]);
    }
    ...
    if ($empresa_id == 2) {
            $CI->app_tabs->add_intranet_profile_tab('documento_settings', [
                'name' => 'Documentos - Configurações',
                'icon' => 'fa fa-folder',
                'view' => 'gestao_corporativa/intranet/cadastros/groups/documento_settings',
                'position' => 190,
            ]);
        
    } else {
        if (has_permission_intranet('cdc_settings', '', 'view') || is_admin()) {
            $CI->app_tabs->add_intranet_profile_tab('cdc_settings', [
                'name' => 'Controle Documental - Configurações',
                'icon' => 'fa fa-folder-o',
                'view' => 'gestao_corporativa/intranet/cadastros/groups/cdc_settings',
                'position' => 185,
            ]);
        }
    }
    ...
    if (has_permission_intranet('announcements', '', 'view') || is_admin()) {
        $CI->app_tabs->add_intranet_profile_tab('announcements', [
            'name' => 'Anúncios',
            'icon' => 'fa fa-bullhorn',
            'view' => 'admin/announcements/manage',
            'position' => 200,
        ]);
    }
    /*
    ...
    $CI->app_tabs->add_medico_profile_tab('profile_medico', [
        'name' => _l('client_add_edit_profile'),
        'icon' => 'fa fa-user-circle',
        'view' => 'admin/medicos/groups/profile',
        //    'visible'  => (has_permission('expenses', '', 'view') || has_permission('expenses', '', 'view_own')),
        'position' => 205,
    ]);

    $CI->app_tabs->add_medico_profile_tab('procedimentos', [
        'name' => _l('procedimento_repasse_medico'),
        'icon' => 'fa fa-list',
        'view' => 'admin/medicos/groups/procedimentos',
        'position' => 210,
    ]);

    $CI->app_tabs->add_medico_profile_tab('agenda', [
        'name' => _l('agenda'),
        'icon' => 'fa fa-calendar',
        'view' => 'admin/medicos/groups/agenda',
        //  'visible'  => (has_permission('invoices', '', 'view') || has_permission('invoices', '', 'view_own') || (get_option('allow_staff_view_invoices_assigned') == 1 && staff_has_assigned_invoices())),
        'position' => 215,
    ]);

    $CI->app_tabs->add_medico_profile_tab('anamnese', [
        'name' => 'Anamnese',
        'icon' => 'fa fa-heartbeat',
        'view' => 'admin/medicos/groups/anamnese',
        //  'visible'  => (has_permission('invoices', '', 'view') || has_permission('invoices', '', 'view_own') || (get_option('allow_staff_view_invoices_assigned') == 1 && staff_has_assigned_invoices())),
        'position' => 220,
    ]);

    $CI->app_tabs->add_medico_profile_tab('anotacoes_medico', [
        'name' => _l('anotacoes'),
        'icon' => 'fa fa-sticky-note-o',
        'view' => 'admin/medicos/groups/notes',
        'visible' => true,
        'position' => 225,
    ]);
    ...
    $CI->app_tabs->add_financeiro_profile_tab('plano_contas', [
        'name' => _l('plano_contas'),
        'icon' => 'fa fa-list-ol',
        'view' => 'admin/financeiro/cadastros/groups/plano_conta',
        //    'visible'  => (has_permission('expenses', '', 'view') || has_permission('expenses', '', 'view_own')),
        'position' => 305,
    ]);

    $CI->app_tabs->add_financeiro_profile_tab('fornecedores', [
        'name' => _l('fornecedores'),
        'icon' => 'fa fa-user-circle',
        'view' => 'admin/financeiro/cadastros/groups/fornecedores',
        'visible' => true,
        'position' => 310,
    ]);

    $CI->app_tabs->add_financeiro_profile_tab('clientes', [
        'name' => _l('clientes'),
        'icon' => 'fa fa-users',
        'view' => 'admin/financeiro/cadastros/groups/clientes',
        'visible' => true,
        'position' => 315,
    ]);

    $CI->app_tabs->add_financeiro_profile_tab('centro_custo', [
        'name' => _l('centro_custo'),
        'icon' => 'fa fa-list',
        'view' => 'admin/financeiro/cadastros/groups/centro_custo',
        //  'visible'  => (has_permission('invoices', '', 'view') || has_permission('invoices', '', 'view_own') || (get_option('allow_staff_view_invoices_assigned') == 1 && staff_has_assigned_invoices())),
        'position' => 325,
    ]);
    ...
    $CI->app_tabs->add_financeiro_profile_tab('bancos', [
        'name' => 'Bancos',
        'icon' => 'fa fa-bank',
        'view' => 'admin/financeiro/cadastros/groups/bancos',
        //  'visible'  => (has_permission('invoices', '', 'view') || has_permission('invoices', '', 'view_own') || (get_option('allow_staff_view_invoices_assigned') == 1 && staff_has_assigned_invoices())),
        'position' => 335,
    ]);

    $CI->app_tabs->add_financeiro_profile_tab('forma_pagamento', [
        'name' => 'Forma de Pagamentos',
        'icon' => 'fa fa-list-ul',
        'view' => 'admin/financeiro/cadastros/groups/modo_pagamento',
        //  'visible'  => (has_permission('invoices', '', 'view') || has_permission('invoices', '', 'view_own') || (get_option('allow_staff_view_invoices_assigned') == 1 && staff_has_assigned_invoices())),
        'position' => 335,
    ]);
     ...
     * MENU UNIDADE HOSPITALAR

     */



    $CI->app_tabs->add_unidade_hospitalar_tab('unidade_hospitalar', [
        'name' => 'Cadastro',
        'icon' => 'fa fa-list-ol',
        'view' => 'admin/unidade_hospitalar/groups/unidade_hospitalar',
        //    'visible'  => (has_permission('expenses', '', 'view') || has_permission('expenses', '', 'view_own')),
        'position' => 405,
    ]);

    $CI->app_tabs->add_unidade_hospitalar_tab('setores', [
        'name' => 'Setores',
        'icon' => 'fa fa-user-circle',
        'view' => 'admin/unidade_hospitalar/groups/setores',
        'visible' => true,
        'position' => 410,
    ]);

    $CI->app_tabs->add_unidade_hospitalar_tab('configuracao', [
        'name' => 'Configuração de Escala',
        'icon' => 'fa fa-gear',
        'view' => 'admin/unidade_hospitalar/groups/configuracao',
        'visible' => true,
        'position' => 415,
    ]);
    ...
    /*
      ...
      $CI->app_tabs->add_unidade_hospitalar_tab('configuracao_escala', [
...
function load_client_language($customer_id = '') {
    ...
    if (get_contact_language() != '' && !is_language_disabled()) {
        ...
        if ((is_client_logged_in() || $customer_id != '') && !is_language_disabled()) {
            ...
            if (!empty($client_language) && file_exists(APPPATH . 'language/' . $client_language)) {
...
function can_contact_view_email_notifications_options() {
    ...
    return has_contact_permission('invoices') || has_contact_permission('estimates') || has_contact_permission('projects') || has_contact_permission('contracts');
}
 ...
 */
function get_all_customer_attachments($id, $fatura = null) {
    ...
    $has_permission_tasks_view = has_permission_intranet('tasks', '', 'view');
...
function is_individual_client() {
    ...
    return is_empty_customer_company(get_client_user_id()) && total_rows(db_prefix() . 'contacts', ['userid' => get_client_user_id()]) == 1;
}
