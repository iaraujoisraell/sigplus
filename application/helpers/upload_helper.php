 ...
 * @return mixed
 */
function handle_task_attachments_array_old($taskid, $index_name = 'attachments') {
    $uploaded_files = [];
...
function handle_task_attachments_array($taskid, $data = null, $origem = 'assets/intranet/arquivos/task_arquivos/') {
    $uploaded_files = [];
    $path = get_upload_path_by_type('task') . $taskid . '/';
    ...
    //echo get_upload_path_by_type('task') . $taskid . '/'; echo $origem . $file; exit;

    foreach ($data as $file) {

        if (rename($origem . $file, $path . $file)) {
            array_push($uploaded_files, [
                'file_name' => $file,
                'filetype' => pathinfo($path . $file, PATHINFO_EXTENSION),
            ]);
        }
    }
    ...
    if (isset($_FILES['file']['name']) && ($_FILES['file']['name'] != '' || is_array($_FILES['file']['name']) && count($_FILES['file']['name']) > 0)) {
        ...
        for ($i = 0; $i < count($_FILES['file']['name']); $i++) {
            ...
            if (!empty($tmpFilePath) && $tmpFilePath != '') {
                ...
                $filename = unique_filename($path, $_FILES['file']['name'][$i]);
                $newFilePath = $path . $filename;
                $data_hexadecimal = base64_encode($newFilePath);
                // Upload the file into the temp dir
                if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                    $attachment = [];
                    $attachment[] = [
                    ...
                    if ($customer_upload == true) {
                        ...
                        //  $attachment['hexadecimal']          = $data_hexadecimal;
                    }
...
function handle_client_attachments_upload_fatura($id, $fatura, $customer_upload = false) {
    $path = get_upload_path_by_type('customer') . $id . '/';
    $CI = & get_instance();
    $totalUploaded = 0;
        ...
        for ($i = 0; $i < count($_FILES['file']['name']); $i++) {
            ...
            if (!empty($tmpFilePath) && $tmpFilePath != '') {
                ...
                if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                    ...
                    $CI->misc_model->add_attachment_fatura_to_database($id, 'customer', $attachment, $fatura);
                    $totalUploaded++;
                }
            }
        }
...
function _upload_extension_allowed($filename) {
    ...
    if (strtolower($browser) === 'safari' && in_array('.jpg', $allowed_extensions) && !in_array('.jpeg', $allowed_extensions)
    ) {
