...
class Api_model extends App_Model
{
    ...
    public function get_login($email = '', $senha = '')
    {
        ...
        $sql = "SELECT s.hash, m.nome_profissional, m.nome_reduzido, s.email, m.cpf,  e.company as empresa, s.data_nascimento, m.celular, m.email, m.codigo_registro
               FROM tblstaff s
               inner join tblempresas e on e.id = s.empresa_id
               inner join tblmedicos m on m.medicoid = s.medico_id
       ...
       try {
           ...
           if($result){
               $dados = array(
                            'hash' => $result->hash,
                            'nome_profissional' => $result->nome_profissional,
                            'nome_reduzido' => $result->nome_reduzido,
                            'email' => $result->email,
                            'cpf' => $result->cpf,
     ...
     */
    public function get_empresa_staff_by_hash($hash = '')
    {
