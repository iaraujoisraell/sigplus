...
class Cron_model extends App_Model
{
    ...
    public function run($manually = false)
    {
        if ($this->can_cron_run()) {
            ...
            // encerra os atendimentos iniciados e abertos
            $this->invoice_overdue();
            
            // gera o fundo de caixa
            //$this->invoice_overdue();
    ...
    public function encerra_Atendimento(){
        ...
        $this->db->where('confirmar_chegada', 1);
        $this->db->where('finished', 0);
        $this->db->where('cancelled', 0);
        $this->db->update(db_prefix() . 'appointly_appointments', ['finished' => 1, 'finished_data' => $hoje]);
        
    }
    
    // retorna a tabela de preço quem tem o inicio da vigencia no dia de hoje
    public function get_tabela_preco_dia($id = '')
    {
        $hoje = date('Y-m-d');
         $sql = "SELECT * FROM tblitems_precos WHERE data_inicio = '$hoje'";
        $result = $this->db->query($sql)->result_array();
        return $result;
    }
    
    public function edit_procedimento($data, $id)
    {
        // edita se tiver
    ...
    /*
     * TABELA DE PREÇO - REPASSE MÉDICO
     */
    
    // retorna os médicos q tem repasse p atualizar no dia
    public function get_medico_tabela_preco_repasse_dia($id = '')
    {
        $hoje =  date('Y-m-d');
        $sql = "SELECT distinct(medico_id) FROM tblmedicos_procedimentos_precos where data_inicio = '$hoje' group by medico_id"; // 
        //$sql = "SELECT * FROM tblmedicos_procedimentos_precos where medico_id = 20 and item_id = 26 and data_fim is null";
        //echo $sql; exit;
        $result = $this->db->query($sql)->result_array();
        return $result;
    }
    ...
    // retorna a tabela de preço de repasse médico quem tem o inicio da vigencia no dia de hoje
    public function get_tabela_preco_repasse_medico_dia($medico_id = '')
    {
        $hoje = date('Y-m-d');
        $sql = "SELECT * FROM tblmedicos_procedimentos_precos WHERE data_inicio = '$hoje' and data_fim is null and medico_id = $medico_id";
        //$sql = "SELECT * FROM tblmedicos_procedimentos_precos where medico_id = 20 and item_id = 26 and data_fim is null";
        //echo $sql; exit;
        $result = $this->db->query($sql)->result_array();
        return $result;
    ...
    // retorna um procedimento de um médico da tabela de repasse do médico
    public function get_item_medico_tabela_repasse($item_id = '', $medico_id = '')
    {
        $hoje = date('Y-m-d');
        $sql = "SELECT * FROM tblmedicos_procedimentos WHERE item_id = '$item_id' and deleted = 0 and medicoid = '$medico_id'";
        
        $result = $this->db->query($sql)->row();
        return $result;
    }
    
     public function edit_repasse_medico_procedimento_novo($data, $id)
    {
        // edita se tiver
        //$this->db->where('medicoid', $medico_id);
        //$this->db->where('item_id', $item_id);
        //$this->db->update(db_prefix() . 'medicos_procedimentos', $data);
         
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'medicos_procedimentos', $data = ['valor' => $data['valor'], 'tipo' => $data['tipo'], 'data_alteracao' => $data['data_alteracao']]);
        
        //return false;
    }
    
    // se existir esse repasse para o médico, edita senão, cria um novo
    public function edit_repasse_medico_procedimento($data, $id)
    {
        // edita se tiver
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'medicos_procedimentos', $data);
    ...
    public function add_new_repasse_medico_procedimento($data_item)
    {
        $this->db->insert(db_prefix() . 'medicos_procedimentos', $data_item);
     ...
     */
    // retorna as parcelas de emprestimos vencendo no dia de hoje
    public function get_emprestimos_dia($id = '')
    {
        $hoje = date('Y-m-d');
         $sql = "SELECT * FROM tblemprestimo_parcelas WHERE data_vcto = '$hoje'";
        $result = $this->db->query($sql)->result_array();
        return $result;
    }
    
    public function edit_emprestimo($data, $id)
    {
        // edita se tiver
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'emprestimo_parcelas', $data);
        if ($this->db->affected_rows() > 0) {
            log_activity('Tabela de preço Updated ');
            return true;
        }
     ...
     * INVOICES
     */
     public function get_faturas_abertas($dias = '')
    {
        $hoje = date('Y-m-d');
        $dias_tratado = "-$dias days";
        $due_date = date('Y-m-d', strtotime($dias_tratado, strtotime($hoje)));
        $sql = "SELECT * FROM tblinvoices where status in (1,4) and duedate = '$due_date'";
       
        $result = $this->db->query($sql)->result_array();
        return $result;
    }
    
    public function edit_fatura($data, $id)
    {
        // edita se tiver
        $this->db->where('id', $id);
    ...
    /*
     * ROTINA DE PRODUÇÃO MÉDICA
     */
    
    
    /*
     *  1 - RETORNA OS ATENDIMENTO FINALIZADOS QUE NÃO TEM PRODUÇÃO
     */
    public function get_atendimentos_sem_producao($id = '')
    {
        $hoje = date('Y-m-d');
        $data_pers = date('Y-m-d', strtotime('-30 days', strtotime($hoje)));
        
        
         $sql = "SELECT i.id as atemdimento_item_id,a.id as atendimento_id, a.date as data_atendimento, a.empresa_id, c.id as convenio, c.particular as particular, c.repasse as repasse, c.valida_fatura_producao, a.contact_id, i.id as atendimento_item_id, i.item_id as item_id, a.medico_id as medico_id 
                FROM tblappointly_appointments a
                inner join tblappointly_appointments_items i on i.agenda_id = a.id
                inner join tblconvenio c on c.id = a.convenio
                where finished = 1  and a.date > '$data_pers' and i.producao_medica = 0 "; // 
       // echo $sql; exit;
        $result = $this->db->query($sql)->result_array();
        return $result;
    }
    ...
    /*
     *  2 - RETORNA AS FATURAS RELACIONADAS AO ATENDIMENTO
     */
    public function get_fatura_atendimentos($atendimento_id = '', $item_id = '')
    {
        $hoje = date('Y-m-d');
         $sql = "SELECT i.id as fatura_id, t.medicoid as medicoid, t.rel_id, t.qty, t.rate_desconto, t.item_id, t.valor_medico, t.valor_tesouraria, i.status as status, t.desconto_valor, t.destino_desconto
                FROM tblinvoices i
                inner join tblitemable t on t.rel_id = i.id and t.rel_type = 'invoice'
                where i.atendimento_id = $atendimento_id and t.item_id = $item_id and i.status not in (4,5,6)";
        // echo $sql; exit;
        $result = $this->db->query($sql)->row();
        return $result;
    }
    ...
    /*
     * 3 - ADD PRODUÇÃO MÉDICA
     */
    
     public function add_producao_medica($data_item)
    {
        $this->db->insert(db_prefix() . 'appointly_producao_medica', $data_item);
    }
    
    /*
     * 
     */
    public function edit_atendimento_procedimento($data, $id)
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'appointly_appointments_items', $data);
        return true;
    }
     ...
     * 
     */
    public function get_procedimentos_sem_repasse_cadastrados()
    {
        $hoje = date('Y-m-d');
        $data_pers = date('Y-m-d', strtotime('-30 days', strtotime($hoje)));
        
         $sql = "SELECT t.* FROM tblitemable t
                    inner join tblinvoices s on s.id = t.rel_id
                    inner join tblitems i on i.id = t.item_id
                    inner join tblconvenio c on c.id = i.convenio_id
                    where t.valor_medico in (0, '0.00')  and c.repasse = 1 and t.rel_type = 'invoice' and s.date >= '$data_pers'
                    order by rel_id asc";
        // echo $sql; exit;
        $result = $this->db->query($sql)->result_array();
        return $result;
    ...
    public function get_valida_procedimento_repasse($rel_id, $item_id)
    {
        $hoje = date('Y-m-d');
         $sql = "SELECT *
                FROM tblappointly_producao_medica
                where fatura_id = $rel_id and item_id = $item_id";
         //echo $sql; exit;
        $result = $this->db->query($sql)->row();
        return $result;
    }
    
    public function edit_repasse_medico_producao($data, $rel_id, $item_id)
    {
        $this->db->where('fatura_id', $rel_id);
        $this->db->where('item_id', $item_id);
    ...
    public function edit_repasse_medico_faturamento($data, $id)
    {
        $this->db->where('id', $id);
    ...
    // retorna os procedimentos do agendamento
    public function get_tabela_items_agenda($id = '')
    {
        $sql = "SELECT ai.*, i.description, i.rate 
                FROM tblappointly_appointments_items ai
    ...
    // retorna o cadastro de repasse médico procedimento
    ...
    public function get_fatura_procedimento_by_id($atendimento_id = '', $item_id = '')
    {
        $hoje = date('Y-m-d');
         $sql = "SELECT i.id as fatura_id, t.medicoid as medicoid, t.rel_id, t.qty, t.rate_desconto, t.item_id, t.valor_medico, t.valor_tesouraria, i.status as status, t.desconto_valor, t.destino_desconto
                FROM tblinvoices i
    ...
    public function edit_file_hexadecimal($data, $id)
    {
        // edita se tiver
        $this->db->where('id', $id);
    ...
    public function get_pacienes_doctos()
    {
        $sql = "SELECT * FROM pacientes_caval where sexo is null limit 2000";
       
        $result = $this->db->query($sql)->result_array();
        return $result;
    }

    public function get_dados_pacienes_doctos($codigo = '')
    {
        $sql = "SELECT * FROM tbldoctors_paciente where CodPaciente = $codigo";
        //echo $sql; exit;
        $result = $this->db->query($sql)->row();
        return $result;
    }
    
    public function edit_dados_paciente($id_paciente = '', $sexo = '', $data_nascimento = '', $Endereco = '', $Estado = '', $Cidade = '', $CEP = '', $Fone = '', $FoneAdicional = '', $Profissao = '', $Pai = '', $Mae = '', $TemObs = '')
    {

        //$this->db->where('id', $id)->update('pacientes_caval', ['Endereco'=>'teste']);
        $sql = "update pacientes_caval set sexo = '$sexo', data_nascimento = '$data_nascimento',  Endereco =  '$Endereco', Estado = '$Estado', Cidade= '$Cidade',  CEP = '$CEP', Fone = '$Fone', FoneAdicional = '$FoneAdicional', Profissao = '$Profissao', Pai = '$Pai', Mae='$Mae', TemObs = '$TemObs'   where id = $id_paciente";
       // echo $sql; exit;
        $this->db->query($sql);
