...
class Convenios_model extends App_Model
{
    ...
    public function get($id = '')
    {
      ...
      $this->db->where('empresa_id', $empresa_id);
       // echo 'aqui : '.$id;exit;
         if (is_numeric($id)) {
            $this->db->where(db_prefix() . 'convenio.id', $id);
            $convenio = $this->db->get(db_prefix() . 'convenio')->row();
            return $convenio;
         }   
        
        $this->db->where('deleted', 0);
        $this->db->order_by('name', 'desc');
        return $this->db->get(db_prefix() . 'convenio')->result_array();
    }
    ...
    public function get_particular($id = '')
    {
      ...
      $this->db->where('empresa_id', $empresa_id);
       // echo 'aqui : '.$id;exit;
        ...
        $this->db->where('deleted', 0);
        $this->db->where('gera_fatura', 1);
        $this->db->where('particular', 1);
        $this->db->order_by('name', 'desc');
        return $this->db->get(db_prefix() . 'convenio')->result_array();
    }
    
    
    public function get_controla_guia($id = '')
    {
        $empresa_id = $this->session->userdata('empresa_id');
      $this->db->where('empresa_id', $empresa_id);
       // echo 'aqui : '.$id;exit;
