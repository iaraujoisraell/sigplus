...
class Link_destaque_model extends App_Model {
    ...
    public function add($data = null, $id = 0) {
        if ($data) {
            ...
            if ($id) {
                ...
                if ($this->db->insert("tbl_intranet_link_destaque", $data)) {
    ...
    public function add_categoria($data = null, $id = 0) {
        if ($data) {
            ...
            if ($id) {
                ...
                $this->db->where('id', $id);
                if ($this->db->update("tbl_intranet_link_destaque_categoria", $data)) {
                    return $id;
                } else {
                    return false;
                }
                ...
                if ($this->db->insert("tbl_intranet_link_destaque_categoria", $data)) {
                    $id_insert = $this->db->insert_id();
                    return $id_insert;
                } else {
                    return false;
    ...
    public function get_link_categorias() {
        $sql = "SELECT * from tbl_intranet_link_destaque_categoria
    ...
    public function get_link_staffs($id = 0) {
        $sql = "SELECT * from tbl_intranet_link_destaque_send
    ...
    public function get($empresa_id = 0) {
        $sql = "SELECT * from tbl_intranet_link_destaque

        where empresa_id = $empresa_id and deleted = 0 limit 12";
        return $this->db->query($sql);
    }

    public function get_enviados($id = 0) {
        $empresa_id = $this->session->userdata('empresa_id');
        $sql = "SELECT staff_id from tbl_intranet_link_destaque_send

        where link_id = $id and deleted = 0";
        $arrays = $this->db->query($sql)->result_array();
        for ($i = 0; $i < count($arrays); $i++) {
            $enviados[$i] = $arrays[$i]['staff_id'];
        }
     ...
     * 16/01/2022
     * @AUTOR: Israel Araujo
     * Retorna as categorias que o usuário tem links des destaques disponíveis
     */
    ...
    public function get_categorias_links_by_user_id() {
        ...
        $sql = "SELECT c.id, c.titulo as categoria 
                FROM tbl_intranet_link_destaque_send ld
                inner join tbl_intranet_link_destaque l on l.id = ld.link_id
                inner join tbl_intranet_link_destaque_categoria c on c.id = l.categoria_id
     ...
     * 16/01/2022
     * @AUTOR: Israel Araujo
     * Retorna os links destaques de uma categorias que o usuário tem links des destaques disponíveis
     */
    ...
    public function get_ways($id = 0, $only_ways = false) {
        ...
        where rel_id = $id and deleted = 0 and rel_type = 'links_destaque'";
    ...
    public function get_link_department_for_user_id() {
        ...
        $tbl_link = 'tbl_intranet_link_destaque';
        ...
        $sql = "SELECT $tbl_link.*, $tbl_cat.titulo FROM $tbl_link
            ...
            WHERE $tbl_cat.rel_type = 'links_destaque' and $tbl_link.deleted = 0 and $tbl_link.empresa_id = $empresa_id and $tblstaff_dep.staffid = ".get_staff_user_id()."
