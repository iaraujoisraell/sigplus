...
class Reports_model extends App_Model
{
    ...
    {
        return $this->db->query('SELECT DISTINCT(YEAR(date)) as year FROM ' . db_prefix() . 'invoicepaymentrecords WHERE deleted = 0')->result_array();
    }

    public function get_distinct_customer_invoices_years()
    {
        return $this->db->query('SELECT DISTINCT(YEAR(date)) as year FROM ' . db_prefix() . 'invoices WHERE clientid=' . get_client_user_id().' and deleted = 0')->result_array();
    }
     ...
     */
    public function get_receita_conta_medica($medico_id = "", $convenios = "", $data_de = "", $data_ate = "")
    {
        
       $sql = 'SELECT SQL_CALC_FOUND_ROWS `tblconta_financeira`.`nome` AS `conta_financeira`, sum(amount) as amount
     ...
     // recebimento por mês 
    public function get_dashboard_expenses_vs_income_report($year = '')
    {
        $this->load->model('expenses_model');
        ...
        for ($m = 1; $m <= 12; $m++) {
          ...
          //  array_push($dashboard_array, $months_labels[$i], $total_income[$i], $total_expenses[$i]);
            $dashboard_array[$i] = [
                                    'mes' =>   _l(date('F', mktime(0, 0, 0, $m, 1))),
                                    'entrada' => $total_income[$i],
                                    'saida' => $total_expenses[$i]
                                    ];
    ...
    /*
     * retorna AS ENTRADAS NA TESOURARIA POR DIA
     */
    public function get_recebimentos_tesouraria($medico_id = "", $convenios = "", $data_de = "", $data_ate = "")
    {
        
       $sql = 'SELECT SQL_CALC_FOUND_ROWS DAY(tblinvoicepaymentrecords.date) AS DAY, MONTH(tblinvoicepaymentrecords.date) AS MONTH, sum(amount) as amount
                FROM tblinvoicepaymentrecords JOIN tblinvoices ON tblinvoices.id = tblinvoicepaymentrecords.invoiceid
                JOIN tblconta_financeira ON tblconta_financeira.id = tblinvoicepaymentrecords.conta_id
                LEFT JOIN tblpayment_modes ON tblpayment_modes.id = tblinvoicepaymentrecords.paymentmode
                ...
                AND tblconta_financeira.id IN ('.$medico_id.')
                AND tblinvoices.convenio IN ('.$convenios.')     
                GROUP BY MONTH(tblinvoicepaymentrecords.date), DAY(tblinvoicepaymentrecords.date) ORDER BY MONTH(tblinvoicepaymentrecords.date), DAY(tblinvoicepaymentrecords.date) ASC';
        //echo $sql; exit;
       //    and tblinvoices.id = 10002
       $result = $this->db->query($sql)->result_array(); // $this->db->query()->result_array();
       ...
       return $result;
        ...
        /*
     * retorna AS ENTRADAS NA TESOURARIA POR FORMA DE PAGAMENTO
     */
    public function get_recebimentos_tesouraria_forma_pagamento($medico_id = "", $convenios = "", $data_de = "", $data_ate = "")
    {
        
       $sql = 'SELECT SQL_CALC_FOUND_ROWS count(*), tblpayment_modes_tipo.name as paymentmode, sum(amount) as amount
        FROM tblinvoicepaymentrecords JOIN tblinvoices ON tblinvoices.id = tblinvoicepaymentrecords.invoiceid
        JOIN tblconta_financeira ON tblconta_financeira.id = tblinvoicepaymentrecords.conta_id
        LEFT JOIN tblpayment_modes ON tblpayment_modes.id = tblinvoicepaymentrecords.paymentmode
        LEFT JOIN tblpayment_modes_tipo ON tblpayment_modes_tipo.id = tblpayment_modes.tipoid
        ...
        AND (tblinvoicepaymentrecords.date between "'.$data_de.'" and "'.$data_ate.'")
                    AND tblconta_financeira.id IN ('.$medico_id.')
                    AND tblinvoices.convenio IN ('.$convenios.')     
        GROUP BY tblpayment_modes_tipo.id ORDER BY tblpayment_modes.name DESC';
        //echo $sql; exit;
       //    and tblinvoices.id = 10002
       $result = $this->db->query($sql)->result_array(); // $this->db->query()->result_array();
     ...
     /*
     *  RETORNA OS PROCEDIMENTOS AGRUPADOS POR CATEGORIAS
     */
    public function get_categorias_procedimentos($medico_id = "", $convenios = "", $categoria = "", $procedimento_faturamento="", $data_de = "", $data_ate = "")
    {
        
       $where = "";
        if($procedimento_faturamento){
            $where .= "AND tblitems.id IN ($procedimento_faturamento)";
        }
        
       $sql = '
           select
            tblitems_groups.name as categoria,
            sum(tblitemable.qty) as quantidade,
            sum(tblitemable.rate_desconto) as total_faturado,
            ...
            JOIN tblconta_financeira ON tblconta_financeira.medico_id = tblmedicos.medicoid

            where tblconta_financeira.id IN ('.$medico_id.')
            AND tblinvoices.convenio IN ('.$convenios.')
            AND tblitems.group_id IN ('.$categoria.')
            '.$where.'
            and ((select tblinvoicepaymentrecords.date from tblinvoicepaymentrecords where tblinvoicepaymentrecords.invoiceid = tblinvoices.id group by tblinvoicepaymentrecords.invoiceid) BETWEEN "'.$data_de.'" AND "'.$data_ate.'")
            group by tblitems_groups.id
            order by sum(tblitemable.qty) desc';
     ...
     /*
     *  RETORNA OS PROCEDIMENTOS AGRUPADOS POR PROCEDIMENTOS
     */
    public function get_all_procedimentos($medico_id, $convenios = "", $categoria = "", $procedimento_faturamento="", $data_de = "", $data_ate = "")
    {
       ...
       $sql = '
           select
            sum(tblitemable.qty) as quantidade,
            tblitems.id,
            tblitems.description as procedimento,
            tblitems_groups.name as categoria,
            sum(tblitemable.qty) as quantidade,
            sum(tblitemable.rate_desconto) as total_faturado,
            sum(tblitemable.valor_tesouraria) as valor_tesouraria,
