...
class Misc_model extends App_Model
{
    ...
    public function get_medicos_dropdown_template($name, $medicoid = '')
    {
        // if passed manually - like in proposal convert items or project
        ...
        //$name = 'medicoid';
        $selected = '';
        $select = "<select class='selectpicker display-block' data-width='100%' name='$name'  data-none-selected-text='". _l('sem_medico')." ' required='true'>";
        if(!$medicoid){
        $select .= '<option value=""'.$selected.'" >'._l('sem_medico').'</option>';
        }
        foreach ($medicos as $tax) {
            $selected = '';
           
                if ($medicoid == $tax['medicoid']) {
                    $selected = 'selected';
                }
            

            $select .= '<option value="' . $tax['medicoid'] . '"    ' . $selected . '  >' . $tax['nome_profissional'] . '</option>';
        }
        $select .= '</select>';
    ...
    public function add_attachment_to_database($rel_id, $rel_type, $attachment, $external = false)
    {
        ...
        //if (isset($attachment[0]['hexadecimal'])) {
            $data['dossie'] = 0;
        //}
    ...
    public function add_attachment_fatura_to_database($rel_id, $rel_type, $attachment, $fatura, $external = false)
    {
        $data['dateadded'] = date('Y-m-d H:i:s');
        $data['rel_id']    = $rel_id;
        if (!isset($attachment[0]['staffid'])) {
    ...
    /**************************************************************************/
    public function get_notes_atendimento($rel_id, $rel_type)
    {
       
        $this->db->join(db_prefix() . 'staff', db_prefix() . 'staff.staffid=' . db_prefix() . 'notes_atendimento.addedfrom');
        $this->db->where('atendimento_id', $rel_id);
        $this->db->where('rel_type', $rel_type);
        $this->db->where(db_prefix() .'notes_atendimento.deleted', 0);
        $this->db->order_by('dateadded', 'desc');

        $notes = $this->db->get(db_prefix() . 'notes_atendimento')->result_array();
     
        return hooks()->apply_filters('get_notes_atendimento', $notes, ['atendimento_id' => $rel_id, 'rel_type' => $rel_type]);
    }
    
    public function add_evolucao($dados,  $rel_id)
    {
        $empresa_id = $this->session->userdata('empresa_id');
        $data['atendimento_id']     = $rel_id;
        $data['rel_type']           = $dados['tipo'];
        $data['description']        = nl2br($dados['description']);
        $data['addedfrom']          = get_staff_user_id();
        $data['paciente_id']        = $dados['paciente_id'];
        $data['dateadded']          = date('Y-m-d H:i:s');
        $data['deleted']            = 0;
        $data['empresa_id']         = $empresa_id;

       
        $data = hooks()->apply_filters('create_note_data', $data, $dados['tipo'], $rel_id);

        $this->db->insert(db_prefix() . 'notes_atendimento', $data);
        $insert_id = $this->db->insert_id();
    ...
    public function edit_evolucao($data, $id)
    {
        hooks()->do_action('before_update_note', [
            'data' => $data,
            'id'   => $id,
        ...
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'notes_atendimento', $data = [
            'description' => nl2br($data['description']),
        ]);
    ...
    public function delete_evolucao($note_id)
    {
        hooks()->do_action('before_delete_note', $note_id);

        $this->db->where('id', $note_id);
        $note = $this->db->get(db_prefix() . 'notes_atendimento')->row();
        ...
        $this->db->where('id', $note_id);
         $this->db->update(db_prefix() . 'notes_atendimento', $data = ['deleted' => 1]);
        //$this->db->delete(db_prefix() . 'notes_atendimento');
        if ($this->db->affected_rows() > 0) {
            hooks()->do_action('note_deleted', $note_id, $note);
    ...
    /**************************************************************************/
     ...
     public function get_anamnese_medico($rel_id)
    {
       //$this->db->where('deleted', 0);
       $this->db->where('medico_id', $rel_id);
       $notes = $this->db->get(db_prefix() . 'medicos_anamnese')->row();
        
        return hooks()->apply_filters('medicos_anamnese', $notes, ['medico_id' => $rel_id]);
      
    }
    
    public function add_anamnese_medico($data)
    {
       
        $this->db->insert(db_prefix() . 'medicos_anamnese', $data);
        $insert_id = $this->db->insert_id();

        if ($insert_id) {
            hooks()->do_action('anamnese_medico_created', $insert_id, $data);
    ...
    public function edit_anamnese_medico($data, $id)
    {
       
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'medicos_anamnese', $data);
        
        if ($this->db->affected_rows() > 0) {
            hooks()->do_action('medicos_anamnese_updated', $id, $data);
     ...
     */
     ...
     public function get_historico_ficha($rel_id)
    {
       $this->db->join(db_prefix() . 'staff', db_prefix() . 'staff.staffid=' . db_prefix() . 'cliente_historico.addedfrom');
       $rel_type = 'customer';
       $this->db->where('deleted', 0);
       // $this->db->where('rel_type', $rel_type);
        $this->db->where('rel_id', $rel_id);
       ...
       // $client = $this->clients_model->get($rel_id);
        $notes = $this->db->get(db_prefix() . 'cliente_historico')->result_array();
        
        return hooks()->apply_filters('get_historico', $notes, ['rel_id' => $rel_id]);
        /*
        $this->db->join(db_prefix() . 'staff', db_prefix() . 'staff.staffid=' . db_prefix() . 'notes.addedfrom');
        $this->db->where('rel_id', $rel_id);
        $this->db->where('rel_type', $rel_type);
     ...
     public function get_historico($rel_id)
    {
       $this->db->join(db_prefix() . 'staff', db_prefix() . 'staff.staffid=' . db_prefix() . 'cliente_historico.addedfrom');
    ...
    {
        $data['data_visita']   = date('Y-m-d');
        $data['dateadded']   = date('Y-m-d H:i:s');
        $data['addedfrom']   = get_staff_user_id();
        $data['rel_type']    = 'customer';
        $data['rel_id']      = $data['rel_id'];
        $data['queixas_hda'] = nl2br($data['queixas_hda']);
        $data['antecedentes'] = nl2br($data['antecedentes']);
        $data['refracao'] = nl2br($data['refracao']);
        $data['biomicroscopia'] = nl2br($data['biomicroscopia']);
        $data['fundoscopia'] = nl2br($data['fundoscopia']);
        $data['diagnostico'] = nl2br($data['diagnostico']);
        $data['conduta'] = nl2br($data['conduta']);
        $data['informacoes_adicionais'] = nl2br($data['informacoes_adicionais']);
        unset($data['quantidade']);
        $data = hooks()->apply_filters('create_note_data', $data);
        
        $this->db->insert(db_prefix() . 'cliente_historico', $data);
        $insert_id = $this->db->insert_id();

        if ($insert_id) {
            hooks()->do_action('historico_created', $insert_id, $data);
    ...
    public function edit_historico($data, $id)
    {
        hooks()->do_action('before_update_note', [
            'data' => $data,
            'id'   => $id,
        ...
        unset($data['atendimento_id']);
        unset($data['rel_id']);
        unset($data['quantidade']);
        $this->db->where('atendimento_id', $id);
        $this->db->where('rel_id', $rel_id);
        

        
        $this->db->update(db_prefix() . 'cliente_historico', $data);

        if ($this->db->affected_rows() > 0) {
            hooks()->do_action('historico_updated', $id, $data);
    ...
    public function delete_historico($note_id)
    {
        hooks()->do_action('before_delete_note', $note_id);

        $this->db->where('id', $note_id);
        $note = $this->db->get(db_prefix() . 'cliente_historico')->row();
       ...
       /* if ($note->addedfrom != get_staff_user_id() && !is_admin()) {
        ...
        $this->db->where('id', $note_id);
       // $this->db->delete(db_prefix() . 'cliente_historico');
         $this->db->update(db_prefix() . 'cliente_historico', $data = ['deleted' => 1]);
        if ($this->db->affected_rows() > 0) {
            hooks()->do_action('historico_deleted', $note_id, $note);
            return true;
        }
        return false;
    }
     ...
     * NOTA FISCAL
     */
    public function add_nota_fiscal($data)
    {
       
        
        $fileid = $data['id'];
        $data['nf_numero']  = $data['numero'];
        $data['nf_cnpj']    = $data['cnpj'];
        $data['valor']      = $data['valor'];
        $data['data_emissao'] = $data['data_emissao'];
        $data['nf_file_path'] = $data['file_path'];
        ...
        unset($data['file_name']);
         ...
         $this->db->update(db_prefix() . 'files', $data);
        
       // $this->db->insert(db_prefix() . 'cliente_historico', $data);
      //  $insert_id = $this->db->insert_id();

        if ($this->db->affected_rows() > 0) {
            hooks()->do_action('Nota Fiscal atualizada', $fileid, $data);
     ...
     * PALAVRAS CHAVES
     */
    public function get_palavras($rel_id, $rel_type)
    {
        
        $client = $this->clients_model->get($rel_id);
        $cod_paciente = $client->codpaciente;
        
        if($cod_paciente){
            $this->db->where('paciente_id', $cod_paciente);
        }else{
            $this->db->where('paciente_id', $rel_id);
        }
        
       
        
         $this->db->order_by('palavra_chave', 'desc');
        $notes = $this->db->get(db_prefix() . 'clients_palavras')->result_array();
        return hooks()->apply_filters('get_historico', $notes, ['paciente_id' => $rel_id]);
        /*
        $this->db->join(db_prefix() . 'staff', db_prefix() . 'staff.staffid=' . db_prefix() . 'notes.addedfrom');
        $this->db->where('rel_id', $rel_id);
        $this->db->where('rel_type', $rel_type);
    ...
    public function add_palavra($data, $rel_type, $rel_id)
    {
        ...
        if($cod_paciente){
            $data['paciente_id']      = $cod_paciente;
        }else{
            $data['paciente_id']      = $rel_id;
        }
       ...
       // $data['rel_type']    = $rel_type;
        ...
        $data['palavra_chave'] = nl2br($data['palavra_chave']);

        
        $this->db->insert(db_prefix() . 'clients_palavras', $data);
        $insert_id = $this->db->insert_id();
    ...
    public function delete_palavra($note_id)
    {
        hooks()->do_action('before_delete_note', $note_id);

        $this->db->where('id', $note_id);
        $note = $this->db->get(db_prefix() . 'clients_palavras')->row();
       ...
       /* if ($note->addedfrom != get_staff_user_id() && !is_admin()) {
        ...
        $this->db->where('id', $note_id);
        $this->db->delete(db_prefix() . 'clients_palavras');
        if ($this->db->affected_rows() > 0) {
            hooks()->do_action('palavra_deletada', $note_id, $note);
            return true;
        }
        return false;
    }
    ...
    /*
     * REPASSE MEDICOS PROCEDIMENTOS
     */
    public function get_medico_procedimento($medico_id)
    {
        $this->db->select(db_prefix() . 'medicos_procedimentos.* ,'.db_prefix() . 'medicos_procedimentos.id as mp_id,'.db_prefix() . 'items.rate as rate,'.db_prefix() . 'items.description as description,'.   'items.id as items_id, '.db_prefix().'convenio.name as convenio, '.db_prefix() . 'items.ativo as ativo,'.db_prefix() . 'items_groups.name as categoria,'.db_prefix() . 'medicos_procedimentos.tipo as tipo');
        $this->db->where(db_prefix().'medicos_procedimentos.medicoid', $medico_id);
        $this->db->where(db_prefix().'medicos_procedimentos.deleted', 0);
        
        $this->db->order_by('description', 'asc');
        $this->db->join(db_prefix() . 'items', db_prefix() . 'items.id=' . db_prefix() . 'medicos_procedimentos.item_id');
        $this->db->join(db_prefix() . 'items_groups', db_prefix() . 'items_groups.id=' . db_prefix() . 'items.group_id');
        $this->db->join(db_prefix() . 'convenio', db_prefix() . 'convenio.id=' . db_prefix() . 'items.convenio_id');
        
        $procedimentos = $this->db->get(db_prefix() . 'medicos_procedimentos')->result_array();
        return hooks()->apply_filters('get_medico_procedimento', $procedimentos, ['tblmedicos_procedimentos.medicoid' => $medico_id]);
        /*
        $this->db->join(db_prefix() . 'staff', db_prefix() . 'staff.staffid=' . db_prefix() . 'notes.addedfrom');
        $this->db->where('rel_id', $rel_id);
        $this->db->where('rel_type', $rel_type);
    ...
    public function add_repasse_medico($data)
    {
        $empresa_id = $this->session->userdata('empresa_id');
        ...
        $this->db->insert(db_prefix() . 'medicos_procedimentos', $data);
        $insert_id = $this->db->insert_id();
    ...
    public function edit_repasse_medico($data, $id)
    {
      
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'medicos_procedimentos', $data = ['valor' => $data['valor']]);

        if ($this->db->affected_rows() > 0) {
            hooks()->do_action('repasse_updated_successfully', $id, $data);
    ...
    public function delete_repase_medico($note_id)
    {
        $usuario = get_staff_user_id();
        $data_atual  = date('Y-m-d H:i:s');
        
        $this->db->where('id', $note_id);
        //$this->db->delete(db_prefix() . 'medicos_procedimentos');
        $this->db->update(db_prefix() . 'medicos_procedimentos', $data = ['deleted' => '1','usuario_alteracao' => $usuario,'data_alteracao' => "$data_atual"]);
        if ($this->db->affected_rows() > 0) {
            hooks()->do_action('repasse_medico_deletada', $note_id);
            return true;
        }
        return false;
    }
    ...
    public function get_options()
    {
        ...
        return $this->db->get(db_prefix() . 'options_padrao')->result_array();
    }
     ...
     */
    public function perform_search_original($q)
    {
        $q = trim($q);
        ...
        if ($have_assigned_customers || $have_permission_customers_view) {
            ...
            // Clients
            $this->db->select(implode(',', prefixed_table_fields_array(db_prefix() . 'clients')) . ',' . get_sql_select_client_company().', '. db_prefix() . 'clients.vat');
        ...
        // Tasks Search
        $tasks = has_permission_intranet('tasks', '', 'view');
        // Staff tasks
        $this->db->select();
    ...
    public function perform_search_fatura($q)
    {
        $q = trim($q);
        $this->load->model('staff_model');
        $is_admin                       = is_admin();
    ...
    public function _search_projects($q, $limit = 0, $where = false)
    {
        ...
        $projects = has_permission_intranet('projects', '', 'view');
        // Projects
        $this->db->select();
    ...
    public function add_agenda($data, $medico_id)
    {
        
        $data_de =  $data['date_de'];
        $hora_de =  $data['hora_de'];
        $data_ate =  $data['data_ate'];
        $hora_ate =  $data['hora_ate'];
        $dias_semana =  $data['dias_semana'];
        $tempo_consulta =  $data['tempo_consulta'];
        $consultorio =  $data['consultorio'];       
        
        
        // verifica qtos minutos tem o tempo da consulta
        $partes_tempo = explode(":", $tempo_consulta);
        $minutos_consulta = $partes_tempo[0]*60+$partes_tempo[1];
       
        
        /* Regras para gerar agenda
         *  1 - while data
         * percorre todos os dias informados
         */
        ...
        while($data_de <= $data_ate){
            ...
            $diasemana_numero = date('w', strtotime($data_de));
          
            // 2 - Verifica se a data atual, está no dia da semana selecionado.
            if(array_search($diasemana_numero, $dias_semana) >= 0){
               
                // while horarios
                // 3 - gera os horários de acordo com o intervalo informado e a hora de início e fim
                while($hora_de < $hora_ate){
                    
                    // 4- consulta se ja tem um registro salvo nessa hora, data para este médico
                    $horario_disponível = $this->appointly_model->getQuantidadeAgendaMedico_data_hora($hora_de, $medico_id, $data_de, $consultorio);
                 
                    if(!$horario_disponível){
                        // se não tiver registro, salva essa agenda
                        $data_agenda['date'] = $data_de;
                        $data_agenda['start_hour'] = $hora_de;
                        $data_agenda['medico_id'] = $medico_id;
                        $data_agenda['consultorio_id'] = $consultorio;
                        $data_agenda['tempo_consulta'] = $minutos_consulta;
                      
                        $empresa_id = $this->session->userdata('empresa_id');
                        $data_agenda['empresa_id'] = $empresa_id;
                       
                        if (!empty($data_agenda)) {
                            if ($this->appointly_model->insert_agenda($data_agenda)) {
                             //   appointly_redirect_after_event('success', _l('appointment_created'));
                            }
                        }
                       
                    }// fim if valida horário disponível
                    // incrementa a hora para o próximo horário
                   $hora_de = date('H:i', strtotime("+$minutos_consulta minute", strtotime($hora_de)));
                   
                }//fim while hora
                
            }// fim if que verifica se a data é um dia da semana selecionado
          
           
            // incrementa para validar se continua ou não no while
            $data_de =  date('Y-m-d', strtotime('+1 days', strtotime($data_de)));
            $hora_de =  $data['hora_de'];
        }
    ...
    public function edit_agenda($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'cliente_historico', $data = [
            'historico' => nl2br($data['description']),
        ]);
    ...
    public function delete_agenda($note_id)
    {
        hooks()->do_action('before_delete_note', $note_id);
    ...
    // ATUALIZA REPASSE MÉDICO
    public function atualiza_repasses_medicos_old($medicoid)
    {
       $sql = "SELECT t.* FROM tblitemable t
                inner join tblinvoices on tblinvoices.id = t.rel_id and t.rel_type = 'invoice'
                where discount_total = 0 and t.qty > 1 and tblinvoices.date > '2021-03-01' and status not in (5)";  // where r.date  between '2021-02-01' and '2021-02-28' and it.medicoid not in (18,33,21,26,45, 28)
       
        $medico_procedimentos = $this->db->query($sql)->result_array();
        
        foreach ($medico_procedimentos as $medico_proc) {
            $desconto_medico = 0;
            $itemtable_id = $medico_proc['id'];
            $medicoid_r = $medico_proc['medicoid'];
            $item_id = $medico_proc['item_id'];
            $qty = $medico_proc['qty'];
            $rate = $medico_proc['rate'];
            $valor_desconto = $medico_proc['valor_desconto'];
            $rate_desconto = $medico_proc['rate_desconto'];
            $valor_total_procedimento = ($qty * $rate);
            $valor_total_procedimento = $valor_total_procedimento - $valor_desconto;
            
            $update_valor_medico = "UPDATE tblitemable SET tblitemable.rate_desconto = '$valor_total_procedimento' WHERE id=$itemtable_id";
    ...
    public function atualiza_repasses_medicos()
    {
      
       $sql = "SELECT i.id as fatura_id,  it.id, it.medicoid, i.discount_total, tblconta_financeira.nota_fiscal_propria, tblconta_financeira.nome, it.item_id, it.qty,  it.rate, (it.qty * it.rate) as vl_tota,  it.valor_medico, it.valor_tesouraria, tblitems_groups.comissao as comissao_padrao,
                (select valor from tblmedicos_procedimentos mp where mp.medicoid = it.medicoid and mp.item_id = it.item_id) as valor_repasse_tipo1,
                (((it.qty * it.rate) * (select valor from tblmedicos_procedimentos mp where mp.medicoid = it.medicoid and mp.item_id = it.item_id and tipo = 2 ))/100) as valor_repasse_tipo2
                    from tblitemable it
                    inner join tblinvoices i on i.id = it.rel_id and it.rel_type = 'invoice'
                    JOIN tblitems ON tblitems.id = it.item_id
                       JOIN tblitems_groups ON tblitems_groups.id = tblitems.group_id
                join tblconta_financeira on tblconta_financeira.medico_id = it.medicoid
                inner join tblinvoicepaymentrecords  r on r.invoiceid = i.id
                    where   i.id in (9800,9164,9862,9417)
                    and i.status = 2 ";
        ...
        * $sql = "SELECT i.id as fatura_id,  it.id, it.medicoid, i.discount_total, tblconta_financeira.nota_fiscal_propria, tblconta_financeira.nome, it.item_id, it.qty,  it.rate, (it.qty * it.rate) as vl_tota,  it.valor_medico, it.valor_tesouraria, tblitems_groups.comissao as comissao_padrao,
                ...
                (((it.qty * it.rate) * (select valor from tblmedicos_procedimentos mp where mp.medicoid = it.medicoid and mp.item_id = it.item_id and tipo = 2 ))/100) as valor_repasse_tipo2
                FROM tblitemable it
                inner join tblinvoices i on i.id = it.rel_id
                JOIN tblitems ON tblitems.id = it.item_id
                JOIN tblitems_groups ON tblitems_groups.id = tblitems.group_id
                join tblconta_financeira on tblconta_financeira.medico_id = it.medicoid
                inner join tblinvoicepaymentrecords  r on r.invoiceid = i.id
        ...
        */
        ...
        foreach ($medico_procedimentos as $medico_proc) {
            $desconto_medico = 0;
            $valor_repasse_tipo2 = $medico_proc['valor_repasse_tipo2'];
            $valor_repasse_tipo1 = $medico_proc['valor_repasse_tipo1'];
            $itemtable_id = $medico_proc['id'];
            $medicoid_r = $medico_proc['medicoid'];
            $item_id = $medico_proc['item_id'];
            $qty = $medico_proc['qty'];
            ...
            $rate = $medico_proc['rate'];
            $valor_total_procedimento = ($qty * $rate);
            $comissao = $medico_proc['comissao_padrao'];
            // busca o valor do repasse
            // $sql_repasse_medico = "SELECT * FROM tblmedicos_procedimentos where medicoid = $medicoid_r and item_id = $item_id";
            // $retorno_repasse = $this->db->query($sql_repasse_medico)->row();
            // $valor_repasse = $retorno_repasse->valor;
           
            if($valor_repasse_tipo2){
              //  if($valor_repasse_tipo2 != $valor_medico ){
                    $this->db->where('id', $itemtable_id);
                    $this->db->where('medicoid', $medicoid_r);
                    $this->db->where('item_id', $item_id);
                    
                 /*   if($nota_fiscal_propria == 1){
                        $valor_tesouraria = $valor_total_procedimento - $valor_repasse_tipo2 - $discount_total;
                        $this->db->update(db_prefix() . 'itemable', $data = ['desconto_valor' => $discount_total,    'valor_medico' => $valor_repasse_tipo2, 'valor_tesouraria' => $valor_tesouraria]);
                    }else{ */
                        $valor_tesouraria = $valor_total_procedimento - $valor_desconto;
                       
                       // $valor_tesouraria = $valor_desconto; // - $valor_repasse_tipo2 - $valor_desconto;
                        $this->db->update(db_prefix() . 'itemable', $data = ['desconto_valor' => $valor_desconto, 'valor_medico' => $valor_repasse_tipo2, 'valor_tesouraria' => $valor_tesouraria]);
                //    }
            ...
            if($valor_repasse_tipo1 > 0){
                 
                $this->db->where('id', $itemtable_id);
                $this->db->where('medicoid', $medicoid_r);
                $this->db->where('item_id', $item_id);
             /*   if($nota_fiscal_propria == 1){
                    $valor_tesouraria = $valor_total_procedimento - $valor_repasse_tipo1- $discount_total;
                    $this->db->update(db_prefix() . 'itemable', $data = ['desconto_valor' => $discount_total, 'valor_medico' => $valor_repasse_tipo1, 'valor_tesouraria' => $valor_tesouraria]);
                }else{ */
                    
                    $valor_tesouraria = $valor_total_procedimento - $valor_desconto;
                   
                    //$valor_tesouraria = $valor_desconto; // - $valor_repasse_tipo1- $valor_desconto;
                    $this->db->update(db_prefix() . 'itemable', $data = ['desconto_valor' => $valor_desconto, 'valor_medico' => $valor_repasse_tipo1, 'valor_tesouraria' => $valor_tesouraria]);
            //    }
             

             
            }else{
                // busca regra na % da categoria
                $comissao = $comissao/100;
                $valor_repasse_comissao =  ($valor_total_procedimento * $comissao);
                
               // if($valor_repasse_comissao != $valor_medico ){
                    
            /*    if($nota_fiscal_propria == 1){
                    $valor_tesouraria = $valor_total_procedimento - $valor_repasse_comissao- $discount_total;
                    $update_valor_medico = "UPDATE tblitemable SET tblitemable.desconto_valor = '$discount_total', tblitemable.rate_desconto = '$valor_total_procedimento', tblitemable.valor_medico = '$valor_repasse_comissao', valor_tesouraria = '$valor_tesouraria' WHERE id=$itemtable_id and medicoid=$medicoid_r and item_id = $item_id  ";
                 //  echo $update_valor_medico; exit;
                }else{ */
                    $valor_tesouraria = $valor_total_procedimento - $valor_desconto;
                    
                 //   $valor_tesouraria = $valor_desconto; // - $valor_repasse_comissao- $valor_desconto;
                   // echo $valor_tesouraria; exit;
                    $update_valor_medico = "UPDATE tblitemable SET tblitemable.desconto_valor = '$valor_desconto' , tblitemable.valor_medico = $valor_repasse_comissao, valor_tesouraria = $valor_tesouraria WHERE id=$itemtable_id and medicoid=$medicoid_r and item_id = $item_id  ";
            ...
            if($medicoid_r == 28){
                    $valor_tesouraria = $valor_total_procedimento  ;
                    $update_valor_medico = "UPDATE tblitemable SET tblitemable.desconto_valor = '$discount_total',tblitemable.rate_desconto = '$valor_total_procedimento',"
                            . "tblitemable.valor_medico = $valor_repasse_comissao, valor_tesouraria = 0 WHERE id=$itemtable_id and medicoid=$medicoid_r and item_id = $item_id  ";
                //    $this->db->query($update_valor_medico);
            }
     ...
     // ATUALIZA REPASSE MÉDICO
    public function atualiza_valor_pocedimento_final($medicoid)
    {
       $sql = "select i.number, i.date,  it.* from tblitemable it
                inner join tblinvoices i on i.id = it.rel_id and it.rel_type = 'invoice'
                where i.date >= '2021-04-01' and valor_medico = 0 and valor_tesouraria = 0
                and i.status = 2 ";
       
        $medico_procedimentos = $this->db->query($sql)->result_array();
        
        foreach ($medico_procedimentos as $medico_proc) {
            $itemtable_id = $medico_proc['item_id'];
            $rate_desconto = $medico_proc['rate_desconto'];
            $valor_medico = $medico_proc['valor_medico'];
            $discount_total = $medico_proc['discount_total'];
            $qty = $medico_proc['qty'];
            $rate = $medico_proc['rate'];
            $valor_total_procedimento = ($qty * $rate);
            
            $novo_valor_procedimento = $valor_total_procedimento - $discount_total;
            
            // 1 - ve se a fatura ta paga
            // 
            ...
            if($rate_desconto < $valor_total_procedimento ){
                    $this->db->where('id', $itemtable_id);
                    $this->db->update(db_prefix() . 'itemable', $data = ['rate_desconto' => $novo_valor_procedimento]);
    ...
    public function edit_status_producao_medica($id)
    {
        $data['status'] = 1;
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'appointly_producao_medica', $data);
    ...
    public function deletar_producao_medica($id)
    {
        $data['deleted'] = 1;
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'appointly_producao_medica', $data);
    ...
    // editar agendamento
    public function confirmar_entrega_guia_agendamento($id)
    {
        $data['guia_entrega'] = 1;
        $this->db->where('id', $id);
    ...
    public function cancelar_entrega_guia_agendamento($id)
    {
        $data['guia_entrega'] = 0;
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'appointly_appointments', $data);
    ...
    public function edit_status_producao_repasse_medica($data, $id)
    {
    ...
    public function get_producao_medica($id)
    {
        
       $this->db->where('id', $id);
       return $this->db->get(db_prefix() . 'appointly_producao_medica')->row();
    }
    
    public function add_repasse($data, $rel_type, $rel_id)
    {
    ...
    public function gera_repasse_producao_medica($id)
    {
        $data['status'] = 1;
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'appointly_producao_medica', $data);
    ...
    /*
     * ATUALIZA REGSTRO SINCONECTA NAS FATURAS
     */
    public function get_registros()
    {
        
       //$this->db->where('id', $id);
       return $this->db->get(db_prefix() . 'depara_registros')->result_array();
    }
    
    public function atualiza_registros_invoices($fatura, $registro)
    {
        $this->db->where('id', $fatura);
    ...
    /*
     * ATUALIZA SALA CIRURGICA
     */
    public function get_salas()
    {
       //$this->db->where('id', $id);
       return $this->db->get(db_prefix() . 'depara_salas_irurgicas')->result_array();
    }
    
    public function atualiza_sala_invoices($fatura, $registro)
    {
        $this->db->where('id', $fatura);
        $this->db->update(db_prefix() . 'invoices', [
            'sala' => $registro,
