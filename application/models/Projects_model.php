...
class Projects_model extends App_Model
{
     ...
     * 27/09/2022
     * @WannaLuiza
     * Rotornas as categorias pelo id da empresa
     */
    public function get_project_category()
    {
        $empresa_id = $this->session->userdata('empresa_id');
        return $this->db->query("SELECT * FROM " . db_prefix() . "projects_category WHERE empresa_id = $empresa_id and deleted = 0")->result_array();
    }
    ...
    public function add_categoria($data = null, $id = 0) {
        ...
        if ($data) {
            ...
            if ($id) {
                ...
                $this->db->where('id', $id);
                if ($this->db->update("tblprojects_category", $data)) {
                    return $id;
                } else {
                    return false;
                }
                ...
                $data['user_ultima_alteracao'] = get_staff_user_id();
                $data['data_ultima_alteracao'] = date('Y-m-d');
                if ($this->db->insert("tblprojects_category", $data)) {
    ...
    public function get($id = '', $where = [])
    {
        ...
        $this->db->where($where);
        $this->db->where('tblprojects.empresa_id', $empresa_id);
        ...
        if (is_numeric($id)) {
            ...
            if ($project) {
                ...
                if($project->gerente){
                $project->gerente_data = $this->staff_model->get($project->gerente);
                }
    ...
    {
        $has_permission                    = has_permission_intranet('tasks', '', 'view');
    ...
    public function get_project_overview_weekly_chart_data($id, $type = 'this_week')
    {
        ...
        $has_permission_create = has_permission_intranet('projects', '', 'create');
