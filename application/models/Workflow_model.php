...
class Workflow_model extends App_Model
{
    ...
    public function get_fluxos_andamento_estornados($id = 0)
    {
        $empresa_id = $this->session->userdata('empresa_id');
        $tbl_workflow_fluxo = 'tbl_intranet_workflow_fluxo_andamento';
        $tbl_departments = 'tbldepartments';
        ...
        where $tbl_workflow_fluxo.empresa_id = $empresa_id and $tbl_workflow_fluxo.estornado = 1 and $tbl_workflow_fluxo.workflow_id = $id ";
        /*if ($date_end) {
            $sql .= " and $tbl_workflow_fluxo.data_concluido <= '$date_end' ";
        }*/
    ...
    public function get_workflow_by_id($id = '')
    {
        ...
        $empresa_id = $this->session->userdata('empresa_id'); //echo "$empresa_id"; exit;
    ...
    public function get_workflow_back($fluxo_andamento_id = '', $id = '')
    {
        ...
        if (is_numeric($id) && !empty($id)) {
            ...
            $row = $this->db->get(db_prefix() . '_intranet_workflow_back')->row_array();
            return $row ?: [];
        }

        if (!empty($fluxo_andamento_id)) {
            $this->db->where(db_prefix() . '_intranet_workflow_back.workflow_andamento_id', $fluxo_andamento_id);
            $result = $this->db->get(db_prefix() . '_intranet_workflow_back')->result_array();
...
WHERE tbl_intranet_categorias_campo_values.rel_id = tbl_intranet_workflow.id AND tbl_intranet_categorias_campo.nome IN ($values)) AS value";
    ...
    public function get_report($cat = '', $start, $end, $campos_system = [], $status, $campos_ = null, $campos__ = null, $current = '', $departmentid = '', $staffid = '', $filter = null)
    {
        //print_r($filter); exit;
        ...
        foreach ($filter as $index => $campo) {
            $joins[] = " INNER JOIN tbl_intranet_categorias_campo_values v$index 
                ON tbl_intranet_workflow.id = v$index.rel_id AND v$index.campo_id = {$campo['id']}";
            $v = is_array($campo['value']) ? implode(',', $campo['value']) : $campo['value'];
            $conditions[] = " v$index.value = '{$v}' ";
        }
        ...
        if ($values2) {
            $sql .= ",(
                SELECT JSON_OBJECTAGG(
                    REPLACE(tbl_intranet_categorias_campo.id, ' ', ''),
                    tbl_intranet_categorias_campo_values.value
                )
                FROM tbl_intranet_categorias_campo_values
                INNER JOIN tbl_intranet_categorias_campo ON tbl_intranet_categorias_campo.id = tbl_intranet_categorias_campo_values.campo_id
                WHERE tbl_intranet_categorias_campo_values.rel_id = tbl_intranet_workflow.id
...
LEFT JOIN tbl_intranet_workflow_fluxo_andamento on tbl_intranet_workflow_fluxo_andamento.workflow_id = tbl_intranet_workflow.id and tbl_intranet_workflow_fluxo_andamento.concluido = 0";
        if ($from_department == true) {
            $sql .= " INNER JOIN tblstaff_departments ON tblstaff_departments.staffid = tbl_intranet_workflow.user_created and  tblstaff_departments.departmentid = $departmentid ";
        }
        ...
        if ($from_staff == true) {
            $sql .= " and tbl_intranet_workflow.user_created = $staffid ";
        }
...
";
    ...
    public function verifica_duplicacao($id)
    {

        $sql = "SELECT
        min(id) as id,  max(id) as max_id,
    fluxo_sequencia,
    COUNT(*) as quantidade
FROM
    tbl_intranet_workflow_fluxo_andamento
WHERE
    empresa_id = 4
    AND deleted = 0
     AND workflow_id = $id
GROUP BY
    ...
    fluxo_sequencia
HAVING       quantidade > 1";
        $duplicacoes = $this->db->query($sql)->result_array();
        foreach ($duplicacoes as $d) {
            $data_update['deleted'] = 1;
            $this->db->where('id', $d['id']);
            $this->db->where('atribuido_a', 0);
            // $this->db->update('tbl_intranet_workflow_fluxo_andamento', $data_update);
            return true;
        }
