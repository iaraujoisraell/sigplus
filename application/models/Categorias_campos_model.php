...
class Categorias_campos_model extends App_Model {
    ...
    public function get_categoria_workflow() {
        $tbl_categoria = 'tbl_intranet_categorias';
        $sql = "SELECT * from $tbl_categoria
        where $tbl_categoria.deleted = 0 and $tbl_categoria.rel_type = 'workflow'  ";
        return $this->db->query($sql)->result_array();
    }    
...
//echo $sql; exit;
     ...
     * Retorna os campos de um determinado tipo(pela empresa_conectada) - NECESSÁRIO ID DO TIPO PARA RETORNAR OS CAMPOS DO MESMO 
     */
    public function get_categoria_campos($tipo_id = 0, $preenchido_por = 0, $not_category = false, $rel_type = '', $id = '', $in_out = '', $just_type = null) {
        $empresa_id = $this->session->userdata('empresa_id');
        $tbl_campos = 'tbl_intranet_categorias_campo';
        $sql = "SELECT *, $tbl_campos.id as id from $tbl_campos ";
        if ($id) {
        ...
        if (is_array($just_type)) {
            //print_r($without); exit;
            $values = array();

// Loop através de $just_type para construir a lista de valores
            foreach ($just_type as $in) {
                $values[] = "'" . $in . "'";
            }

// Construir a cláusula IN
            $in_clause = implode(', ', $values);
            $sql .= " AND $tbl_campos.type IN ($in_clause) ";
        }
     ...
     */
    public function get_values($id = '', $rel_type = '', $preenchido_por = 0, $chave = false, $solicitante = false, $config = false, $portal = '') {
        ...
        if( $portal == true){
            $sql .= " tbl_intranet_categorias_campo.portal = 0";
        }
        $sql .= " ORDER BY $tbl_campos.ordem asc ";
      //  return $sql;
    // echo $sql; exit;
...
//echo $sql;
    ...
    public function get_categoria_fluxos($categoria_id = '', $group_by = '') {
        ...
        if (is_numeric($categoria_id)) {
            ...
            // Get compiled select
            $compiled_select = $this->db->get_compiled_select(db_prefix() . '_intranet_categorias_fluxo');

            // Debugging: Echo the compiled SQL query
            //echo $compiled_select;

            // Reset query builder to prevent incorrect query execution
            //$this->db->reset_query();

            // Execute the query and return the result
            return $this->db->query($compiled_select)->result_array();
        }
    }
...
//      exit;
    ...
    public function get_categoria_fluxo($categoria_id = '', $id = '', $template = false) {

        if (is_numeric($id)) {
            $this->db->where('id', $id);
            return $this->db->get(db_prefix() . '_intranet_categorias_fluxo')->row_array();
        } else {
            $this->db->where(db_prefix() . '_intranet_categorias_fluxo.deleted', 0);
            if ($template == true) {
                $this->db->where(db_prefix() . '_intranet_categorias_fluxo.template', 1);
            }
    ...
    public function get($id = '', $rel_type, $where = array() )
    {
        ...
        if ($id) {
            $this->db->where('id', $id);
            return $this->db->get(db_prefix().'_intranet_categorias')->row_array();
        }

        $this->db->where('deleted', 0);
        $this->db->where('rel_type', 'atendimento');

        return $this->db->get(db_prefix().'_intranet_categorias')->result_array();
