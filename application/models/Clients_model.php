...
class Clients_model extends App_Model
{
    ...
    public function get_paciente_portal($id = '', $where = [])
    {
       $sql = "SELECT * from tblclients_portal where id = $id and active = 1 and deleted = 0";
       return $this->db->query($sql)->row(); 
    }
    

    public function get_pacientes_by_nome($nome)
    {
        $hoje = date('Y-m-d');
    ...
    public function delete($id)
    {
        ...
        $this->db->update(db_prefix() . 'clients', ['deleted' => 1]);
        if ($this->db->affected_rows() > 0) {
            $affectedRows++;
    ...
    public function delete_contact($id)
    {
        ...
        $this->db->update(db_prefix() . 'contacts', ['deleted' => 1]);
        ...
        if ($this->db->affected_rows() > 0) {
           ...
           // $this->db->delete(db_prefix() . 'customfieldsvalues');
            ...
            if ($last_activity) {
                $this->db->where('id >', $last_activity->id);
                //$this->db->delete(db_prefix() . 'activity_log');
                $this->db->update(db_prefix() . 'activity_log', ['deleted' => 1]);
            }
    ...
    */
    ...
    public function get_todos_agendamentos_paciente($cliente)
    {
         $sql = "SELECT a.date, a.start_hour, a.finished, c.name as convenio, m.nome_profissional, i.id as fatura, t.type FROM tblappointly_appointments a
                inner join tblconvenio c on c.id = a.convenio
                inner join tblmedicos m on m.medicoid = a.medico_id
                left join tblinvoices i on i.atendimento_id = a.id
                inner join tblappointly_appointment_types t on t.id = a.type_id
                where a.contact_id = $cliente 
