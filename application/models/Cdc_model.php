...
class Cdc_model extends App_Model {
    ...
    public function get_log($id = 0) {
        ...
        $sql = "SELECT $tbl_log.*, $tbl_documento.*, $tbl_staff.firstname, $tbl_staff.lastname FROM $tbl_log "
                ...
                . "WHERE  $tbl_log.rel_id = $id AND $tbl_log.rel_type = 'cdc' AND $tbl_log.empresa_id = $this->empresa_id ORDER BY $tbl_log.id desc";
        //echo $sql; exit;
        return $this->db->query($sql)->result_array();
    }
    ...
    public function get_categorias($withpermission = true) {
        ...
        $sql = "SELECT * FROM $tbl_categoria "
                ...
                . "WHERE  $tbl_categoria.empresa_id = $this->empresa_id AND $tbl_categoria.deleted = 0 AND $tbl_categoria.active = 1 AND $tbl_categoria.rel_type = 'cdc' ";
    ...
    public function get_categoria($id){
        $sql = "SELECT * FROM tbl_intranet_categorias where id = $id";
        return $this->db->query($sql)->row_array();
    }

    public function get_categorias_regra(){

        $sql = "SELECT * FROM tbl_intranet_categorias tic where rel_type = 'cdc' and active = 1 and deleted = 0 and exclusivo_responsavel = 0";
        $categorias = $this->db->query($sql)->result_array();

        $staff_id = get_staff_user_id();
        $sql_staff_departments = "SELECT * from tblstaff_departments td where staffid = $staff_id";
        $staff_departments = $this->db->query($sql_staff_departments)->result_array();

     //   print_r($staff_departments); exit;

        $categorias_exclusivas = $this->db->query("SELECT * FROM tbl_intranet_categorias tic where rel_type = 'cdc' and active = 1 and deleted = 0 and exclusivo_responsavel = 1")->result_array(); 
       // print_r($categorias_exclusivas); exit;
        foreach($categorias_exclusivas as $cat){
            foreach($staff_departments as $staff_department){
                if($staff_department['departmentid'] == $cat['responsavel']){
...
//        return $this->db->get()->result_array();
    ...
    public function get_fluxos_cdc($id_cdc){
 
        $sql = "SELECT id, rel_id, staff_id, fluxo_id, fluxo_sequencia, fluxo_nome from tbl_intranet_categorias_loading where rel_id = $id_cdc and rel_type = 'cdc' and deleted = 0 and fluxo_sequencia > 0 order by fluxo_sequencia";
    ...
    public function get_cdc($id_cdc){

        $sql = "SELECT * from tbl_intranet_cdc where id = $id_cdc";
        return $this->db->query($sql)->row_array();
