...
class Announcements_model extends App_Model
{
    ...
    public function get($id = '', $where = [], $limit = '')
    {
        ...
        $this->db->where($where);         
        $this->db->where(db_prefix() . 'announcements' . '.empresa_id', $empresa_id);
    ...
    {
        $empresa_id = $this->session->userdata('empresa_id');
        $lastAnnouncement = $this->db->query('SELECT announcementid FROM ' . db_prefix() . 'announcements '
                . ' WHERE ' . (!$staff ? 'showtousers' : 'showtostaff') . ' = 1 '
                . ' AND deleted = 0 and empresa_id = '.$empresa_id
                . ' AND announcementid = (SELECT MAX(announcementid) FROM ' . db_prefix() . 'announcements)')->row();
        if ($lastAnnouncement) {
