...
class Tasks_model extends App_Model
{
    ...
    public function get_all($rel_id, $rel_type)
    {
        ...
        return hooks()->apply_filters('get_all', $task);
    }
    ...
    {
        $empresa_id = $this->session->userdata('empresa_id');
        return $this->db->query('SELECT DISTINCT(YEAR(' . $this->db->escape_str($get_from) . ')) as year FROM ' . db_prefix() . 'tasks '
                . ' WHERE ' . $this->db->escape_str($get_from) . ' IS NOT NULL AND empresa_id = '.$empresa_id.' and deleted = 0 ORDER BY year DESC')->result_array();
    }
    ...
    {
        $has_permission_view = has_permission_intranet('tasks', '', 'view');
    ...
    public function edit_comment($data)
    {
        ...
        $comment = $this->db->get(db_prefix() . 'task_comments')->row();
        if ($comment->staffid == get_staff_user_id() || has_permission_intranet('tasks', '', 'edit') || $comment->contact_id == get_contact_user_id()) {
    ...
    public function remove_comment($id, $force = false)
    {
        ...
        if ($comment->staffid == get_staff_user_id() || has_permission_intranet('tasks', '', 'delete') || $comment->contact_id == get_contact_user_id() || $force === true) {
            $comment_added = strtotime($comment->dateadded);
            $minus_1_hour  = strtotime('-1 hours');
     ...
     * 05/10/2022
     * @WannaLuiza
     * Retorna o ultimo sequencial
     */
    public function get_next_sequencial()
    {
        $tbltasks = 'tbltasks';
        $sql = "SELECT sequencial, count(*) as todos FROM $tbltasks "
                . " WHERE $tbltasks.empresa_id = $this->empresa_id AND $tbltasks.deleted = 0 "
                . " ORDER BY $tbltasks.sequencial DESC LIMIT 1";
