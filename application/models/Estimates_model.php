...
class Estimates_model extends App_Model
{
    ...
    public function convert_to_invoice($id, $client = false, $draft_invoice = false)
    {
        ...
        $new_invoice_data['medicoid'] = $_estimate->medicoid;
        $new_invoice_data['convenio'] = $_estimate->convenio;
    ...
    public function copy($id)
    {
        ...
        $new_estimate_data['medicoid']   = $_estimate->medicoid;
        $new_estimate_data['convenio']   = $_estimate->convenio;
        ...
        $new_estimate_data['empresa_id'] = $empresa_id;
        ...
        foreach ($_estimate->items as $item) {
            ...
            $new_estimate_data['newitems'][$key]['medicoid']      = $item['medicoid'];
            $new_estimate_data['newitems'][$key]['item_id']      = $item['item_id'];
            $new_estimate_data['newitems'][$key]['desconto_valor']   = $item['desconto_valor'];
            $new_estimate_data['newitems'][$key]['rate_desconto']    = $item['rate_desconto'];
            $new_estimate_data['newitems'][$key]['valor_medico']     = $item['valor_medico'];
    ...
    public function add($data)
    {
        ...
        if ($insert_id) {
            // coloca o numero do orçamento
             $this->db->where('id', $insert_id);
             $this->db->set('number', $insert_id);
             $this->db->update(db_prefix() . 'estimates');
            ...
            foreach ($items as $key => $item) {
                if ($itemid = add_new_sales_item_post($item, $insert_id, 'estimate',$destino_desconto)) {
       ...
       // print_r($data); exit;
        ...
        foreach ($items as $key => $item) {
            ...
            if (update_sales_item_post($item['itemid'], $item, 'description')) {
                $this->log_estimate_activity($id, 'invoice_estimate_activity_updated_item_medico', false, serialize([
                        $original_item->description,
                        $item['description'],
                    ]));
                $affectedRows++;
        ...
        foreach ($newitems as $key => $item) {
            if ($new_item_added = add_new_sales_item_post($item, $id, 'estimate',$destino_desconto)) {
