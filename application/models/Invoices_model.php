...
class Invoices_model extends App_Model
{
    ...
    public function get($id = '', $where = [])
    {
        ...
        $this->db->select('*, ' . db_prefix() . 'currencies.id as currencyid, ' . db_prefix() . 'invoices.id as id, ' . db_prefix() . 'currencies.name as currency_name, '. db_prefix() .'invoices.status as status');
        $this->db->from(db_prefix() . 'invoices');
        $this->db->join(db_prefix() . 'currencies', '' . db_prefix() . 'currencies.id = ' . db_prefix() . 'invoices.currency', 'left');
        $this->db->join(db_prefix() . 'medicos', ''    . db_prefix() . 'medicos.medicoid = '    . db_prefix() . 'invoices.medicoid', 'left');
        $this->db->where('tblinvoices.deleted', 0);
        $this->db->where($where);
        if (is_numeric($id)) {
    ...
    {
       /* 
       $sql = 'SELECT *  FROM '.db_prefix().' itemable where id = '.$id;
    ...
    public function get_pagamentos_agrupado_por_conta($invoice_id)
    {
        
       $sql = 'SELECT r.*, c.name as caixa
                FROM tblinvoicepaymentrecords r
                left join tblcaixas_registros cr on cr.id = r.caixa_id
                left join tblcaixas c on c.id = cr.caixa_id
                where r.invoiceid = "'.$invoice_id.'" and r.deleted = 0 ';
       //echo $sql; exit;
     ...
     /*
     * retorna todos os médicos por procediment que estão na lista de itens de uma fatura
     */
    public function get_medico_invoice_item_producao($invoice_id)
    {
        
       $sql = 'SELECT tblitemable.id, tblitemable.medicoid, tblitemable.description, tblitemable.qty, tblitemable.rate, g.name as grupo, m.nome_profissional, tblitemable.valor_medico, tblitemable.valor_tesouraria, tblitemable.desconto_valor, tblitemable.destino_desconto  FROM '.db_prefix().'itemable '
               . ' inner join  '.db_prefix().'medicos m on m.medicoid = tblitemable.medicoid '
               . ' inner join tblitems it on it.id = tblitemable.item_id '
               . ' inner join tblitems_groups g on g.id = it.group_id '
               . ' where rel_id = '.$invoice_id .' and rel_type = "invoice"  ';
       //echo $sql; exit;
       $result = $this->db->query($sql)->result_array();
       return $result;
    ...
    /*
     * retorna os médicos (único) que estão na lista de itens de uma fatura
     */
    public function get_medico_invoice_item_pagamento($invoice_id)
    {
        
       $sql = 'SELECT distinct tblitemable.medicoid, repasse  FROM '.db_prefix().'itemable '
               . ' inner join  '.db_prefix().'medicos m on m.medicoid = tblitemable.medicoid '
               . ' where rel_id = '.$invoice_id .' and rel_type = "invoice" and repasse_empresa = 1  ';
      // echo $sql; exit;
       $result = $this->db->query($sql)->result_array();
       return $result;
     ...
     * retorna os médicos (único) que estão na lista de itens de uma fatura
     */
    public function get_todos_medico_invoice_item_pagamento($invoice_id)
    {
     ...
     * retorna os médicos (único) que estão na lista de itens de uma fatura
     */
    public function get_dados_itemable($item_id)
    {
        
       $sql = 'SELECT *  FROM '.db_prefix().'items '
               . ' where id = '.$item_id ;
      //echo $sql; exit;
    ...
    /*
     * Retorna os itens da fatura dos médico que não tem repasse, para abater do valor total
     */
    public function get_invoice_item_pagamento_sem_repasse($invoice_id)
    {
        
       $sql = 'SELECT i.id as itemable_id, i.*, m.* FROM '.db_prefix().'itemable i'
               . ' left join  '.db_prefix().'medicos m on m.medicoid = i.medicoid '
               . ' where rel_id = '.$invoice_id .' and rel_type = "invoice" and m.repasse_empresa = 0 ';
     ...
     /*
     * Retorna os itens da fatura dos médico que tem comissao 100% p ele
     */
    public function get_invoice_item_medico_100_comissao($invoice_id)
    {
        
       $sql = 'SELECT i.id as itemable_id, i.*, m.* FROM '.db_prefix().'itemable i'
               . ' left join  '.db_prefix().'medicos m on m.medicoid = i.medicoid '
               . ' where rel_id = '.$invoice_id .' and rel_type = "invoice" and m.comissao = 100 ';
    ...
    /*
     * Retorna os itens da fatura  - usado qdo no pagamento é selcionado uma conta
     */
    public function get_invoice_item_pagamento_fatura($invoice_id)
    {
    ...
    /*
     * Retorna os itens da fatura de cada médico
     */
    public function get_invoice_item_pagamento($invoice_id)
    {
        
       $sql = 'SELECT distinct(i.medicoid), nome_profissional, cf.id as conta_id FROM '.db_prefix().'itemable i'
               . ' left join  '.db_prefix().'medicos m on m.medicoid = i.medicoid '
                . ' left join  '.db_prefix().'conta_financeira cf on cf.medico_id = m.medicoid '
               . ' where rel_id = '.$invoice_id .' and rel_type = "invoice" ';
       //echo $sql; exit;
        $result = $this->db->query($sql)->result_array();
    ...
    /*
     * Retorna as contas financeiras de uma fatura - para pagamento
     */
    public function get_conta_financeira_pagamento($invoice_id, $medico_id)
    {
        
          
       $sql = 'SELECT '.db_prefix().'itemable.*, m.comissao, cf.id as conta_id, m.repasse, cf.nota_fiscal_propria FROM '.db_prefix().'itemable '
               . ' left join  '.db_prefix().'medicos m on m.medicoid = '.db_prefix().'itemable.medicoid '
               . ' left join  '.db_prefix().'conta_financeira cf on cf.medico_id = m.medicoid '
               . ' where rel_id = '.$invoice_id .' and '.db_prefix().'itemable.medicoid = '. $medico_id .' and rel_type = "invoice" ';
    ...
    /*
     * Retorna as contas financeiras de um médico
     */
    public function get_conta_financeira_medico($itemtable_id)
    {
        
          
       $sql = 'SELECT cf.*, m.comissao, cf.id as conta_id, m.repasse, cf.nota_fiscal_propria '
               . ' FROM '.db_prefix().'itemable '
               . ' left join  '.db_prefix().'medicos m on m.medicoid = '.db_prefix().'itemable.medicoid '
               . ' left join  '.db_prefix().'conta_financeira cf on cf.medico_id = m.medicoid '
               . ' where tblitemable.id = '.$itemtable_id .' ';
    ...
    /*
    * verifica se para um determinado procedimento e determinado existe uma tabela especial
    */
    public function get_medico_procedimento_invoice_item($item_id, $medico_id)
    {
        
       $sql = 'SELECT '.db_prefix().'medicos_procedimentos.* FROM '.db_prefix().'medicos_procedimentos '
               . ' where item_id = '.$item_id .' and medicoid = '. $medico_id .' and ativo = 1';
       //echo $sql; exit;
        $result = $this->db->query($sql)->result_array();
    ...
    public function get_medico_procedimento_invoice_item_row($item_id, $medico_id)
    {
        
       $sql = 'SELECT count(*) as quantidade FROM '.db_prefix().'medicos_procedimentos '
               . ' where item_id = '.$item_id .' and medicoid = '. $medico_id ;
       //echo $sql; exit;
        $result = $this->db->query($sql)->row();
    ...
    * verifica se para um determinado procedimento e determinado existe uma tabela especial
    */
    public function get_medico_procedimento_invoice_item_conta_selecionada($item_id, $medico_id)
    {
     ...
     */
    public function get_pagamentos_invoice($invoiceid)
    {
       
       $sql = 'SELECT sum(amount) as valor FROM '.db_prefix().'invoicepaymentrecords '
     ...
     * Retorna os pagamentos de uma fatura
     */
    public function get_pagamentos_invoice_conta_medica($invoiceid, $conta_id)
    {
       
       $sql = 'SELECT sum(amount) as valor FROM '.db_prefix().'invoicepaymentrecords '
               . ' where invoiceid = '.$invoiceid .' and conta_id = '.$conta_id.' and deleted =0 ';
    ...
    /*
     * Retorna os pagamentos de uma conta de um médico de uma fatura
     */
    public function get_pagamentos_invoice_conta_financeira($invoiceid, $conta_id)
    {
       
       $sql = 'SELECT sum(amount) as valor FROM '.db_prefix().'invoicepaymentrecords '
               . ' where invoiceid = '.$invoiceid .' and deleted =0 and conta_id = '.$conta_id;
    ...
    * verifica se para um determinado procedimento e determinado existe uma tabela especial
    */
    public function atualiza_valor_porcentagem_itemable($item_id, $valor_comissao, $valor_tesouraria)
    {
       $taskUpdateData['valor_medico'] = "$valor_comissao";
        $taskUpdateData['valor_tesouraria'] = "$valor_tesouraria";
        
        $this->db->where('id', $item_id);
        $this->db->update(db_prefix() . 'itemable', $taskUpdateData);
      
        return 1;
    }
    
    /*
    * atualiza se tem retorno na fatura
    */
    public function atualiza_retorno_fatura($invoice_id, $do_not_redirect)
    {
        $taskUpdateData['do_not_redirect'] = "$do_not_redirect";
        
        $this->db->where('id', $invoice_id);
        $this->db->update(db_prefix() . 'invoices', $taskUpdateData);
      
        return 1;
    }
    
   /*
    * retorna o cadastro do medico
    */
    public function get_medico($medico_id)
    {
        
       $sql = 'SELECT '.db_prefix().'medicos.* FROM '.db_prefix().'medicos '
    ...
    /*
    * Retorna o  item e o grupo do item (categoria) de uma fatura
     * usado em libraries/ app_items_table_template
    */
    public function get_group_item_procedimento_invoice_item($item_id)
    {
        
       $sql = 'SELECT '.db_prefix().'items.*,'.db_prefix().'items_groups.comissao as comissao  FROM '.db_prefix().'items '
               . ' INNER JOIN '.db_prefix().'items_groups on '.db_prefix().'items_groups.id = '.db_prefix().'items.group_id '
               . ' where '.db_prefix().'items.id = '.$item_id;
      // echo $sql; exit;
        $result = $this->db->query($sql)->row();
    ...
    /*
     * Retorna um item da lista de procedimentos da fatura. usado no procedimento de pagamento.  Payments_model
     */
    public function get_invoice_item_pagamento_by_id($id)
    {
        
       $sql = 'SELECT '.db_prefix().'itemable.*, '.db_prefix().'items_groups.comissao,  cf.id as conta_id  FROM '.db_prefix().'itemable '
               . ' INNER join  '.db_prefix().'medicos m on m.medicoid = '.db_prefix().'itemable.medicoid'
               . ' INNER join  '.db_prefix().'conta_financeira cf on cf.medico_id = m.medicoid '
               . ' INNER JOIN '.db_prefix().'items on '.db_prefix().'items.id = '.db_prefix().'itemable.item_id '
               . ' left JOIN '.db_prefix().'items_groups on '.db_prefix().'items_groups.id = '.db_prefix().'items.group_id '
               . ' where '.db_prefix().'itemable.id = '. $id .' and rel_type = "invoice" ';
       //echo $sql; exit
        $result = $this->db->query($sql)->row();
    ...
    public function get_invoices_total($data)
    {
        ...
        for ($i = 1; $i <= 3; $i++) {
            $select = 'id,total';
            if ($i == 1) {
                $select .= ', (SELECT total - (SELECT COALESCE(SUM(amount),0) FROM ' . db_prefix() . 'invoicepaymentrecords WHERE invoiceid = ' . db_prefix() . 'invoices.id AND tblinvoicepaymentrecords.deleted = 0) - (SELECT COALESCE(SUM(amount),0) FROM ' . db_prefix() . 'credits WHERE ' . db_prefix() . 'credits.invoice_id=' . db_prefix() . 'invoices.id)) as outstanding';
            } elseif ($i == 2) {
                $select .= ',(SELECT SUM(amount) FROM ' . db_prefix() . 'invoicepaymentrecords WHERE invoiceid=' . db_prefix() . 'invoices.id AND tblinvoicepaymentrecords.deleted = 0) as total_paid';
            }
    ...
    public function add($data, $expense = false)
    {
        ...
        if($data['payment_modes'] == 1){
            $data['allowed_payment_modes'] = 'a:7:{i:0;s:1:"1";i:1;s:1:"2";i:2;s:1:"4";i:3;s:1:"6";i:4;s:1:"9";i:5;s:2:"10";i:6;s:2:"13";}';
        }else{
            $data['allowed_payment_modes'] = 'a:14:{i:0;s:1:"1";i:1;s:1:"2";i:2;s:1:"3";i:3;s:1:"4";i:4;s:1:"5";i:5;s:1:"6";i:6;s:1:"7";i:7;s:1:"8";i:8;s:1:"9";i:9;s:2:"10";i:10;s:2:"11";i:11;s:2:"12";i:12;s:2:"13";i:13;s:2:"14";}';
        }
       ...
       $data['empresa_id'] = $empresa_id;
        ...
        if($data['destino_desconto']){
         $destino_desconto = $data['destino_desconto'];
        }else{
            $destino_desconto = null;
        }
       ...
       // $data['duedate'] = $duedate;
        ...
        $data_exec = $data['data_execucao'];
        unset($data['data_execucao']);
        $data_n = str_replace("/", "-", $data_exec);
        $new_data_Execuxao = date('Y-m-d', strtotime($data_n));
        //$data['data_execucao'] = $new_data_Execuxao;
        unset($data['destino_desconto']);
        ...
        if ($insert_id) {
            // coloca o número da fatura de acordo c o ID gerado
        
            $this->db->where('id', $insert_id);
            $this->db->update(db_prefix() . 'invoices', ['number' => $insert_id]);
            ...
            foreach ($items as $key => $item) {
                if ($itemid = add_new_sales_item_post($item, $insert_id, 'invoice',$destino_desconto)) {
            ...
            $this->log_invoice_activity($insert_id, $lang_key, false, serialize($data));
    ...
    public function add_agenda($data, $expense = false)
    {
        $data['prefix'] = get_option('invoice_prefix');
    ...
    public function copy($id)
    {
        ...
        $new_invoice_data['convenio'] = $_invoice->convenio;
        $new_invoice_data['medicoid'] = $_invoice->medicoid;
        $new_invoice_data['centrocustoid'] = $_invoice->centrocustoid;
        ...
        //$new_invoice_data['save_as_draft']    = false;
        ...
        foreach ($_invoice->items as $item) {
            ...
            $new_invoice_data['newitems'][$key]['medicoid']         = $item['medicoid'];
            $new_invoice_data['newitems'][$key]['item_id']          = $item['item_id'];
            $new_invoice_data['newitems'][$key]['desconto_valor']   = $item['desconto_valor'];
            $new_invoice_data['newitems'][$key]['rate_desconto']    = $item['rate_desconto'];
            $new_invoice_data['newitems'][$key]['valor_medico']     = $item['valor_medico'];
            $new_invoice_data['newitems'][$key]['destino_desconto'] = $item['destino_desconto'];
        ...
        if ($id) {
            ...
            $this->db->where('id', $id);
            $this->db->update(db_prefix() . 'invoices', ['number' => $id]);
    ...
    public function update($data, $id)
    {
       ...
       //print_r($data['allowed_payment_modes']); exit;
        if (isset($data['recurring'])) {
            if ($data['recurring'] == 'custom') {
                $data['recurring_type']   = $data['repeat_type_custom'];
                $data['custom_recurring'] = 1;
       ...
       // $data['data_execucao'] = $new_data_Execuxao;
        ...
        if (count($items) > 0) {
            foreach ($items as $key => $item) {
                ...
                if (update_sales_item_post($item['itemid'], $item, 'medicoid')) {
                    $affectedRows++;
                }
                ...
                if (update_sales_item_post($item['itemid'], $item, 'desconto_valor')) {
                    $this->log_invoice_activity($id, 'invoice_estimate_activity_updated_item_desconto', false, serialize([
                        $original_item->desconto_valor,
                        $item['desconto_valor'],
                    ]));
                    $affectedRows++;
                }
                
                if (update_sales_item_post($item['itemid'], $item, 'desconto_porcentagem')) {
                    $affectedRows++;
                }
                
                if (update_sales_item_post($item['itemid'], $item, 'destino_desconto')) {
                    $affectedRows++;
                }
                
                if (update_sales_item_post($item['itemid'], $item, 'rate_desconto')) {
                    $affectedRows++;
                }
        ...
        foreach ($newitems as $key => $item) {
            if ($new_item_added = add_new_sales_item_post($item, $id, 'invoice',$destino_desconto)) {
                if (isset($billed_tasks[$key])) {
                    foreach ($billed_tasks[$key] as $_task_id) {
                        $this->db->insert(db_prefix() . 'related_items', [
    ...
    // recalcular valor de repasse dos médicos
    public function processa_Repasse_faturamento($invoiceid)
    {
        
        $dados_fatura = $this->get_fatura_procedimento_by_id($invoiceid);
        foreach ($dados_fatura as $dados){
            $itemable_id          = $dados['itemable_id'];
            $medico_id          = $dados['medicoid']; 
            $qty                = $dados['qty'];
            $rate                = $dados['rate'];
            $item_id            = $dados['item_id'];
            $desconto_valor     = $dados['desconto_valor'];
            $rate_desconto      = $dados['rate_desconto'];
            $destino_desconto   = $dados['destino_desconto'];
            $description        = $dados['description'];
            $valor_procedimento = $rate * $qty;
            
            //
            $dados_tabela_medico = $this->get_dados_item_medico($medico_id, $item_id);
            $valor_tabela       = $dados_tabela_medico->valor;
            ...
            if($desconto_valor > 0){
                ...
                if($destino_desconto == 'AMBOS'){
                    $calc_desc = $desconto_valor/2;
                    
                    if($calc_desc > $valor_tabela){
                        $diferenca = $calc_desc - $valor_tabela;
                        $valor_medico = 0;
                        $valor_empresa = $rate_desconto;
                    }else{
                        $valor_medico = $valor_tabela - $calc_desc;
                        $valor_medico = $valor_medico * $qty;
                        $valor_empresa = $valor_procedimento - $calc_desc - $valor_medico;
                    }
                    
                }else if($destino_desconto == 'MÉDICO'){
                    
                    if($desconto_valor > $valor_tabela){
                        $diferenca = $desconto_valor - $valor_tabela;
                        $valor_medico = 0;
                        $valor_empresa = $rate_desconto;
                    }else{
                        $valor_medico = $valor_tabela - $desconto_valor;
                        $valor_medico = $valor_medico * $qty;
                        ...
                        $valor_medico = $valor_tabela * $qty;
                        $valor_empresa = $valor_procedimento - $valor_medico - $desconto_valor;
                                        
                }
                
                
               // echo $valor_medico.'<Br>';
               // echo $valor_empresa.'<Br>';
            ...
            //echo 'medico :'.$valor_medico.'<Br>';
            //echo 'empresa :'.$valor_empresa.'<Br>';
            
            $data['valor_medico'] = $valor_medico;
            $data['valor_tesouraria'] = $valor_empresa;
          
            $this->db->where('id', $itemable_id);
            $this->db->update(db_prefix() . 'itemable', $data);
    ...
    public function get_fatura_procedimento_by_id($invoiceid = '')
    {
        $hoje = date('Y-m-d');
         $sql = "SELECT i.id as fatura_id, t.id as itemable_id, t.medicoid as medicoid, t.qty as qty, t.rate, t.rate_desconto, t.item_id as item_id, t.description, t.desconto_valor,  t.valor_medico, t.valor_tesouraria,
                i.status as status,  t.destino_desconto
                FROM tblinvoices i
                inner join tblitemable t on t.rel_id = i.id and t.rel_type = 'invoice'
              
                where i.id = $invoiceid ";
         //echo $sql; exit;   inner join tblmedicos_procedimentos mp on mp.item_id = t.item_id and mp.medicoid = t.medicoid        mp.valor as valor_tabela,
        $result = $this->db->query($sql)->result_array();
        return $result;
    }
    
    
     public function get_dados_item_medico($medico_id, $item_id)
    {
        $hoje = date('Y-m-d');
         $sql = "SELECT * FROM tblmedicos_procedimentos where medicoid = $medico_id and item_id = $item_id";
         //echo $sql; exit;
        $result = $this->db->query($sql)->row();
        return $result;
    }
    ...
    public function delete($id, $simpleDelete = false)
    {
        ...
        */
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'invoices', ['deleted' => 1]);
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if ($simpleDelete == false) {
                ...
                $this->db->update(db_prefix() . 'expenses', ['invoiceid' => null,]);
            ...
            $this->db->update(db_prefix() . 'notes', ['deleted' => 1]);
            ...
            $this->db->update(db_prefix() . 'taggables', ['deleted' => 1]);

            $this->db->where('rel_type', 'invoice');
            $this->db->where('rel_id', $id);
            //$this->db->delete(db_prefix() . 'reminders');
            $this->db->update(db_prefix() . 'reminders', ['deleted' => 1]);

            $this->db->where('rel_type', 'invoice');
            $this->db->where('rel_id', $id);
            //$this->db->delete(db_prefix() . 'views_tracking');
            $this->db->update(db_prefix() . 'views_tracking', ['deleted' => 1]);
            ...
            $this->db->update(db_prefix() . 'customfieldsvalues', ['deleted' => 1]);
            ...
            $this->db->update(db_prefix() . 'itemable', ['deleted' => 1]);
            
            foreach ($items as $item) {
                $this->db->where('item_id', $item['id']);
                //$this->db->delete(db_prefix() . 'related_items');
                $this->db->update(db_prefix() . 'related_items', ['deleted' => 1]);
            }

            $this->db->where('rel_id', $id);
            $this->db->where('rel_type', 'invoice');
            //$this->db->delete('scheduled_emails');
            $this->db->update(db_prefix() . 'scheduled_emails', ['deleted' => 1]);
            
            $this->db->where('invoiceid', $id);
            //$this->db->delete(db_prefix() . 'invoicepaymentrecords');
            $this->db->update(db_prefix() . 'invoicepaymentrecords', ['deleted' => 1]);

            $this->db->where('rel_id', $id);
            $this->db->where('rel_type', 'invoice');
            //$this->db->delete(db_prefix() . 'sales_activity');
            $this->db->update(db_prefix() . 'sales_activity', ['deleted' => 1]);
            ...
            $this->db->update(db_prefix() . 'item_tax', ['deleted' => 1]);
    ...
    public function get_invoices_years()
    {
        ...
        //return $this->db->query('SELECT DISTINCT(YEAR(date)) as year, month(date) as month FROM ' . db_prefix() . 'invoices ORDER BY year DESC, month desc')->result_array();
    }
    ...
    /*
     * Retorna os campos anamnese do médico
     */
    public function get_anamnese_medico($medico_id)
    {        
       $sql = 'SELECT * from tblmedicos_anamnese '
    ...
    /*
     * Retorna as anamnese salva
     */
    public function get_historico_cliente_atendimento($atendimento_id)
    {        
       $sql = 'SELECT * from tblcliente_historico '
     ...
     ***************************************************************/
     ...
     public function get_descontos_faturas($fatura)
    {
        
       $sql = "SELECT d.valor_empresa as valor_empresa, d.valor_profissional, d.data_log as data_log, s.firstname as firstname, s.lastname as lastname FROM tblinvoices_descontos d
inner join tblstaff s on s.staffid = d.usuario_log
where d.invoice_id = $fatura";
      // echo $sql; exit;
       $result = $this->db->query($sql)->result_array();
       return $result;
    ...
    /*****************************************************************************
     ****************************************************************************
     ************** R E L A T Ó R I O S - D A S H B O A R D S ******************
     **************************************************************************
     **************************************************************************/
     ...
     * retorna todos os médicos por procediment que estão na lista de itens de uma fatura
     */
    public function get_medico_producao($medico_id, $convenios = "", $categoria = "", $procedimento = "", $data_de = "", $data_ate = "")
    {
        
       $sql = 'SELECT
              tblmedicos.nome_profissional as medico,
              tblitemable.qty as quantidade,
              tblitemable.rate_desconto as total_faturado,
             ...
             JOIN tblitemable ON tblitemable.rel_id = tblinvoices.id AND tblitemable.rel_type="invoice"
             ...
             WHERE tblinvoices.status != 5 AND tblinvoices.status IN (2) AND tblinvoices.deleted = 0
             AND (tblinvoicepaymentrecords.date BETWEEN "'.$data_de.'" AND "'.$data_ate.'")
             AND tblitemable.medicoid IN ('.$medico_id.')
             AND tblinvoices.convenio IN ('.$convenios.')
             AND tblitems.group_id IN ('.$categoria.')
             AND tblinvoicepaymentrecords.deleted = 0
             GROUP BY tblinvoices.id, tblitemable.id';
       //echo $sql; exit;
       //    and tblinvoices.id = 10002
       $result = $this->db->query($sql)->result_array(); // $this->db->query()->result_array();
     ...
     */
     ...
     public function get_fundo_caixa_empresa($despesas_date_select)
    {
        
       $sql = "SELECT sum(amount) as valor FROM tblexpenses e
            INNER JOIN tblpayment_modes ON tblpayment_modes.id = e.paymentmode
            INNER JOIN tblpayment_modes_tipo ON tblpayment_modes_tipo.id = tblpayment_modes.tipoid
            where tblpayment_modes_tipo.id not in (1,3,4) $despesas_date_select";
      // echo $sql; exit;
       $result = $this->db->query($sql)->row();
       return $result;
     ...
     * FUNDO
     */
    public function get_fundo_caixa_dinheiro($entradas_date_select)
    {
       $empresa_id = $this->session->userdata('empresa_id');
       $sql = "SELECT sum(valor) as valor FROM tblcaixa_fundo where empresa = $empresa_id $entradas_date_select";
       //echo $sql; exit;
       $result = $this->db->query($sql)->row();
       return $result;
     ...
     * ENTRADAS
     */
    public function get_fluxo_caixa_dinheiro($entradas_date_select)
    {
        $empresa_id = $this->session->userdata('empresa_id');
       $sql = "SELECT sum(amount) as valor
              ...
              LEFT JOIN tblpayment_modes_tipo ON tblpayment_modes_tipo.id = tblpayment_modes.tipoid
              WHERE tblinvoicepaymentrecords.empresa_id = $empresa_id AND tblpayment_modes_tipo.id = 1 and tblinvoicepaymentrecords.deleted = 0 $entradas_date_select
              GROUP BY tblpayment_modes_tipo.id";
       //echo $sql; exit;
       $result = $this->db->query($sql)->row();
       return $result;
    ...
    public function get_fluxo_caixa_debito($entradas_date_select)
    {
        $empresa_id = $this->session->userdata('empresa_id');
       $sql = "SELECT  sum(amount) as valor
              FROM tblinvoicepaymentrecords
              ...
              INNER JOIN tblpayment_modes_tipo ON tblpayment_modes_tipo.id = tblpayment_modes.tipoid
              WHERE tblinvoicepaymentrecords.empresa_id = $empresa_id AND tblpayment_modes_tipo.id = 4 and tblinvoicepaymentrecords.deleted = 0 $entradas_date_select
              GROUP BY tblpayment_modes_tipo.id";
      // echo $sql; exit;
       $result = $this->db->query($sql)->row();
       return $result;
    ...
    public function get_fluxo_caixa_credito($entradas_date_select)
    {
        $empresa_id = $this->session->userdata('empresa_id');
       $sql = "SELECT  sum(amount) as valor
              FROM tblinvoicepaymentrecords
              INNER JOIN tblpayment_modes ON tblpayment_modes.id = tblinvoicepaymentrecords.paymentmode
              INNER JOIN tblpayment_modes_tipo ON tblpayment_modes_tipo.id = tblpayment_modes.tipoid
              WHERE tblinvoicepaymentrecords.empresa_id = $empresa_id AND tblpayment_modes_tipo.id = 3 and tblinvoicepaymentrecords.deleted = 0 $entradas_date_select
              GROUP BY tblpayment_modes_tipo.id";
      // echo $sql; exit;
       $result = $this->db->query($sql)->row();
       return $result;
    ...
    public function get_fluxo_caixa_outros($entradas_date_select)
    {
        $empresa_id = $this->session->userdata('empresa_id');
       $sql = "SELECT  sum(amount) as valor
              FROM tblinvoicepaymentrecords
              INNER JOIN tblpayment_modes ON tblpayment_modes.id = tblinvoicepaymentrecords.paymentmode
              INNER JOIN tblpayment_modes_tipo ON tblpayment_modes_tipo.id = tblpayment_modes.tipoid
              WHERE tblinvoicepaymentrecords.empresa_id = $empresa_id AND tblpayment_modes_tipo.id not in (1,3,4) and tblinvoicepaymentrecords.deleted = 0 $entradas_date_select
              GROUP BY tblpayment_modes_tipo.id";
      // echo $sql; exit;
       $result = $this->db->query($sql)->row();
       return $result;
     ...
     * REPASSES
     */
    public function get_repasse_fluxo_caixa_dinheiro($repasses_date_select)
    {
        $empresa_id = $this->session->userdata('empresa_id');
       $sql = "SELECT sum(valor) as valor FROM tblcaixas_repasses r
                INNER JOIN tblpayment_modes ON tblpayment_modes.id = r.forma_id
                INNER JOIN tblpayment_modes_tipo ON tblpayment_modes_tipo.id = tblpayment_modes.tipoid
                where  tblpayment_modes_tipo.id = 1 $repasses_date_select";
      // echo $sql; exit;
       $result = $this->db->query($sql)->row();
       return $result;
     ...
     public function get_repasse_fluxo_caixa_debito($repasses_date_select)
    {
        $empresa_id = $this->session->userdata('empresa_id');
       $sql = "SELECT sum(valor) as valor FROM tblcaixas_repasses r
                INNER JOIN tblpayment_modes ON tblpayment_modes.id = r.forma_id
                INNER JOIN tblpayment_modes_tipo ON tblpayment_modes_tipo.id = tblpayment_modes.tipoid
                where tblpayment_modes_tipo.id = 4 $repasses_date_select";
       //echo $sql; exit;
       $result = $this->db->query($sql)->row();
       return $result;
     ...
     public function get_repasse_fluxo_caixa_credito($repasses_date_select)
    {
        $empresa_id = $this->session->userdata('empresa_id');
       $sql = "SELECT sum(valor) as valor FROM tblcaixas_repasses r
                INNER JOIN tblpayment_modes ON tblpayment_modes.id = r.forma_id
                INNER JOIN tblpayment_modes_tipo ON tblpayment_modes_tipo.id = tblpayment_modes.tipoid
                where  tblpayment_modes_tipo.id = 3 $repasses_date_select";
      // echo $sql; exit;
       $result = $this->db->query($sql)->row();
       return $result;
     ...
     public function get_repasse_fluxo_caixa_outros($repasses_date_select)
    {
        
       $sql = "SELECT sum(valor) as valor FROM tblcaixas_repasses r
                INNER JOIN tblpayment_modes ON tblpayment_modes.id = r.forma_id
                INNER JOIN tblpayment_modes_tipo ON tblpayment_modes_tipo.id = tblpayment_modes.tipoid
                where tblpayment_modes_tipo.id not in (1,3,4) $repasses_date_select";
      // echo $sql; exit;
       $result = $this->db->query($sql)->row();
       return $result;
     ...
     * DESPESAS
     */
    public function get_despesa_fluxo_caixa_dinheiro($despesas_date_select)
    {
        
       $sql = "SELECT sum(amount) as valor FROM tblexpenses e
            INNER JOIN tblpayment_modes ON tblpayment_modes.id = e.paymentmode
            INNER JOIN tblpayment_modes_tipo ON tblpayment_modes_tipo.id = tblpayment_modes.tipoid
            where tblpayment_modes_tipo.id = 1 $despesas_date_select";
      // echo $sql; exit;
       $result = $this->db->query($sql)->row();
       return $result;
     ...
     public function get_despesa_fluxo_caixa_debito($despesas_date_select)
    {
        
       $sql = "SELECT sum(amount) as valor FROM tblexpenses e
            INNER JOIN tblpayment_modes ON tblpayment_modes.id = e.paymentmode
            INNER JOIN tblpayment_modes_tipo ON tblpayment_modes_tipo.id = tblpayment_modes.tipoid
            where tblpayment_modes_tipo.id = 4 $despesas_date_select";
      // echo $sql; exit;
       $result = $this->db->query($sql)->row();
       return $result;
     ...
     public function get_despesa_fluxo_caixa_credito($despesas_date_select)
    {
        
       $sql = "SELECT sum(amount) as valor FROM tblexpenses e
            INNER JOIN tblpayment_modes ON tblpayment_modes.id = e.paymentmode
            INNER JOIN tblpayment_modes_tipo ON tblpayment_modes_tipo.id = tblpayment_modes.tipoid
            where tblpayment_modes_tipo.id = 3 $despesas_date_select";
      // echo $sql; exit;
       $result = $this->db->query($sql)->row();
       return $result;
     ...
     public function get_despesa_fluxo_caixa_outros($despesas_date_select)
    {
