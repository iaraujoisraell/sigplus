...
class Newsfeed_model extends App_Model {
    ...
    public function load_newsfeed($offset, $post_id = '') {
        $offset = ($offset * $this->newsfeed_posts_limit); //echo $offset; exit;
    ...
    public function add($data) {
        ...
        $data['content'] = nl2br($data['conteudo_textarea']);
        $data['creator'] = get_staff_user_id();
        unset($data['conteudo_textarea']);
    ...
    public function delete_post($postid) {
        ...
        if (total_rows(db_prefix() . 'newsfeed_posts', [
            ...
            $this->db->where('postid', $postid);
            $this->db->update(db_prefix() . 'newsfeed_posts', $data);

            if ($this->db->affected_rows() > 0) {
                $data['deleted'] = 1;

                $this->db->where('postid', $postid);
                $this->db->update(db_prefix() . 'newsfeed_post_likes', $data);
                // $this->db->delete(db_prefix() . 'newsfeed_post_likes');
