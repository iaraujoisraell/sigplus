...
class Expenses_model extends App_Model
{
    ...
    public function add_caixa($data)
    {
        $empresa_id = $this->session->userdata('empresa_id');
        $data['empresa_id'] = $empresa_id;
       
       
        $this->db->insert(db_prefix() . 'caixas', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            log_activity('New Caixa Added [ID: ' . $insert_id . ']');
    ...
    public function abrir_caixa($data)
    {
        ...
        $data['empresa_id'] = $empresa_id;
        $data['caixa_id'] = $data['caixa_id'];
        $data['data_abertura'] = $data_hoje; 
        $data['usuario_abertura'] = get_staff_user_id(); 
        $entrada_caixa = $data['entrada_caixa'];
        
        //retorna o saldo atual
        $valor_inicial = 0;
        $dados_caixa = $this->caixas_model->get_saldo_caixa($data['caixa_id']); 
        $saldo = $dados_caixa->saldo;
        
       
        
        $valor_entrada = $data['entrada_caixa'];
        $valor_inicial += $saldo + $valor_entrada;
         $data['saldo'] = $valor_inicial; 
       
        $this->db->insert(db_prefix() . 'caixas_registros', $data);
        $insert_id =   $this->db->insert_id();
        if ($insert_id) {
            ...
            $data_caixa['aberto'] = 1;
            $this->db->where('id', $caixa_id);
            $this->db->update(db_prefix() . 'caixas', $data_caixa);
                        
            $caixa_usuario['caixa_id'] = $insert_id;
            $this->db->where('staffid', get_staff_user_id());
            $this->db->update(db_prefix() . 'staff', $caixa_usuario);
           
            if($data['entrada_caixa']){
                $data_movimentacao['caixa_registro_id'] = $insert_id;
                $data_movimentacao['data_registro'] = $data_hoje;
                $data_movimentacao['valor']         = $valor_inicial;
                $data_movimentacao['usuario_id']    = get_staff_user_id();
                $data_movimentacao['empresa_id']    = $empresa_id;
                $data_movimentacao['referencia']    = "SALDO INICIAL";
               // print_R($data_movimentacao);
                $this->db->insert(db_prefix() . 'caixas_movimentacao ', $data_movimentacao);
                
                // atualiza saldo
                if($entrada_caixa > 0){
                 // $this->payments_model->atualiza_saldo($entrada_caixa);  
                }
            ...
            log_activity('New Caixa Registro - Abertura de Caixa [ID: ' . $insert_id . ']');
    ...
    public function fechar_caixa($data)
    {
        $empresa_id = $this->session->userdata('empresa_id');
        $data_hoje = date('Y-m-d H:i:s');
        
        $valor_despesa = $data['valor_despesa'];
        $valor_repasses = $data['valor_repasses'];
        
        //print_r($data); exit;
        
        // PAGAMENTOS RECEBIDOS
        $data_movimentacao['caixa_registro_id'] = $data['caixa_registro_id'];
        $data_movimentacao['caixa_atual'] = $data['caixa_id'];
        $data_movimentacao['data_registro'] = $data_hoje;
        $data_movimentacao['valor'] = $data['valor_recebido'];
        $data_movimentacao['usuario_id'] = get_staff_user_id();
        $data_movimentacao['empresa_id'] = $empresa_id;
        $data_movimentacao['referencia'] = "PAGAMENTOS RECEBIDOS";
        $data_movimentacao['tipo'] = 1;
        $this->db->insert(db_prefix() . 'caixas_movimentacao ', $data_movimentacao);
        $insert_id =  $this->db->insert_id();
        
        // DESPESAS
        $data_saida['caixa_registro_id']    = $data['caixa_registro_id'];
        $data_saida['caixa_atual']          = $data['caixa_id'];
        $data_saida['data_registro']        = $data_hoje;
        $data_saida['valor']                = $valor_despesa;
        $data_saida['usuario_id']           = get_staff_user_id();
        $data_saida['empresa_id']           = $empresa_id;
        $data_saida['referencia']           = "DESPESAS / SAÍDAS";
        $data_saida['tipo'] = 2;
        $this->db->insert(db_prefix() . 'caixas_movimentacao ', $data_saida);
        
        // REPASSES
        $data_repasse['caixa_registro_id']    = $data['caixa_registro_id'];
        $data_repasse['caixa_atual']          = $data['caixa_id'];
        $data_repasse['data_registro']        = $data_hoje;
        $data_repasse['valor']                = $valor_repasses;
        $data_repasse['usuario_id']           = get_staff_user_id();
        $data_repasse['empresa_id']           = $empresa_id;
        $data_repasse['referencia']           = "REPASSES";
        $data_repasse['tipo'] = 2;
        $this->db->insert(db_prefix() . 'caixas_movimentacao ', $data_repasse);
        ...
        if ($insert_id) {
            ...
            $caixa_id = $data['caixa_id'];
            $data_caixa['aberto'] = 0;
            $data_caixa['saldo'] = $saldo;
            $this->db->where('id', $caixa_id);
            $this->db->update(db_prefix() . 'caixas', $data_caixa);
            
            
            $caixa_registro['data_fechamento'] = $data_hoje;
            $caixa_registro['usuario_fechamento'] = get_staff_user_id();
           // $caixa_registro['valor_recebido'] = $data['valor_recebido'];
            $this->db->where('id', $data['caixa_registro_id']);
            $this->db->update(db_prefix() . 'caixas_registros', $caixa_registro);
           
            
            $caixa_usuario['caixa_id'] = NULL;
            $this->db->where('staffid', get_staff_user_id());
            $this->db->update(db_prefix() . 'staff', $caixa_usuario);
            
            // ATUALIZA SALDO
           
            log_activity('New Caixa Registro - Fechamento de Caixa [ID: ' . $insert_id . ']');
     ...
     public function transferencia_caixa($data)
    {
        $empresa_id = $this->session->userdata('empresa_id');
        $data_hoje = date('Y-m-d H:i:s');
        ...
        //ORIGEM
        $caixa_origem = $data['caixa_id'];
        //$registro_origem = $data['registro_id'];
        $dados_caixa = $this->caixas_model->get($caixa_origem); 
        $nome_caixa_origem = $dados_caixa->name;
        $saldo_caixa_origem = $dados_caixa->saldo;
        $aberto_caixa_origem = $dados_caixa->aberto;
        $saldo_atualizado_origem = $saldo_caixa_origem - $valor_transferencia;
        
        
        if($aberto_caixa_origem == 0){
        // insert de um novo registro para a transferência
        $data_registro['empresa_id'] = $empresa_id;
        $data_registro['caixa_id'] = $caixa_origem;
        $data_registro['data_abertura'] = $data_hoje; 
        $data_registro['usuario_abertura'] = get_staff_user_id(); 
        $data_registro['data_fechamento'] = $data_hoje; 
        $data_registro['usuario_fechamento'] = get_staff_user_id(); 
        $data_registro['saldo'] = 0; 
        $data_registro['entrada_caixa'] = 0; 
        $this->db->insert(db_prefix() . 'caixas_registros', $data_registro);
        $registro_origem =   $this->db->insert_id();
        
        }else{
         $dados_caixa = $this->caixas_model->get_ultimo_registro_caixa($caixa_origem);
         $registro_origem = $dados_caixa->id;
         //echo $registro_origem.'<br>';
        }
        
        //echo $aberto_caixa_origem; exit;
        //DESTINO
        $caixa_destino = $data['caixa_destino'];
        $dados_caixa_destino = $this->caixas_model->get($caixa_destino); 
        $nome_caixa_destino = $dados_caixa_destino->name;
        $saldo_caixa_destino = $dados_caixa_destino->saldo;
        $dados_ultimo_registro_caixa_destino = $this->caixas_model->get_ultimo_registro_caixa($caixa_destino); 
        $registro_atual_destino = $dados_ultimo_registro_caixa_destino->id;
        $saldo_atualizado_destino = $saldo_caixa_destino + $valor_transferencia;
       
        
        // TRANSFERENCIA SAÍDA - ORIGEM
        $data_movimentacao_origem['caixa_registro_id'] = $registro_origem;
        $data_movimentacao_origem['caixa_atual'] = $caixa_origem;
        $data_movimentacao_origem['data_registro'] = $data_hoje;
        $data_movimentacao_origem['valor'] = $valor_transferencia;
        $data_movimentacao_origem['usuario_id'] = get_staff_user_id();
        $data_movimentacao_origem['empresa_id'] = $empresa_id;
        $data_movimentacao_origem['referencia'] = "TRANSFERÊNCIA PARA O CAIXA : ".$nome_caixa_destino;
        $data_movimentacao_origem['tipo'] = 2;
        $this->db->insert(db_prefix() . 'caixas_movimentacao ', $data_movimentacao_origem);
        $insert_id =  $this->db->insert_id();
        //ATUALIZA SALDO ORIGEM
        if ($insert_id) {
            //tabela CAIXA
            $data_caixa['saldo'] = $saldo_atualizado_origem;
            $this->db->where('id', $caixa_origem);
            $this->db->update(db_prefix() . 'caixas', $data_caixa);
            
            //Tabela Registros
            $caixa_registro['saldo'] = $saldo_atualizado_origem;
            $this->db->where('id', $registro_origem);
            $this->db->update(db_prefix() . 'caixas_registros', $caixa_registro);
           
            log_activity("TRANSFERÊNCIA PARA O CAIXA : ".$nome_caixa_destino);
        }
            
        // TRANSFERENCIA ENTRADA - DESTINO
        $data_movimentacao_destino['caixa_registro_id'] = $registro_atual_destino;
        $data_movimentacao_destino['caixa_atual']       = $caixa_destino;
        $data_movimentacao_destino['data_registro']     = $data_hoje;
        $data_movimentacao_destino['valor']             = $valor_transferencia;
        $data_movimentacao_destino['usuario_id']        = get_staff_user_id();
        $data_movimentacao_destino['empresa_id']        = $empresa_id;
        $data_movimentacao_destino['referencia']        = "TRANSFERÊNCIA DE : ".$nome_caixa_origem;
        $data_movimentacao_destino['tipo']              = 1;
        $this->db->insert(db_prefix() . 'caixas_movimentacao ', $data_movimentacao_destino);
        $insert_destino_id =  $this->db->insert_id();
         //ATUALIZA SALDO DESTINO
        if ($insert_destino_id) {
             //tabela CAIXA
            $data_caixa_destino['saldo'] = $saldo_atualizado_destino;
            $this->db->where('id', $caixa_destino);
            $this->db->update(db_prefix() . 'caixas', $data_caixa_destino);
            
            //Tabela Registros
            $caixa_registro_destino['saldo'] = $saldo_atualizado_destino;
            $this->db->where('id', $registro_atual_destino);
            $this->db->update(db_prefix() . 'caixas_registros', $caixa_registro_destino);
            
            log_activity("TRANSFERÊNCIA DE : ".$nome_caixa_origem);
