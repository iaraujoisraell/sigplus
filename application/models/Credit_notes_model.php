...
class Credit_notes_model extends App_Model
{
    ...
    */
    ...
    public function update($data, $id)
    {
        ...
        $data_credit = array();
        $data_credit['amount'] = $data['total']; 
        //echo 'aqui : '.$data_credit['amount'].'<br>';
        //echo 'id : '.$id;
        //exit;
        
        $this->db->where('credit_id', $id);
        $this->db->update(db_prefix() . 'credits', $data_credit); 
    ...
    */
    ...
    public function credit_note_from_invoice($invoice_id)
    {
        ...
        $new_credit_note_data['medicoid'] = $_invoice->medicoid;
        $new_credit_note_data['convenio'] = $_invoice->convenio;
        ...
        $new_credit_note_data['billing_country']  = 0; //$_invoice->billing_country;
        ...
        $new_credit_note_data['shipping_country'] = 0;//$_invoice->shipping_country;
        ...
        foreach ($_invoice->items as $item) {
            ...
            $new_credit_note_data['newitems'][$key]['medicoid']             = $item['medicoid'];
            $new_credit_note_data['newitems'][$key]['item_id']              = $item['item_id'];
            $new_credit_note_data['newitems'][$key]['desconto_valor']       = $item['desconto_valor'];
            $new_credit_note_data['newitems'][$key]['desconto_porcentagem'] = $item['desconto_porcentagem'];
            $new_credit_note_data['newitems'][$key]['rate_desconto']        = $item['rate_desconto'];
            $new_credit_note_data['newitems'][$key]['valor_medico']         = $item['valor_medico'];
      ...
      //  print_r($new_credit_note_data); exit;
    ...
    public function get_applied_credits_invoice($credit_id)
    {
        $this->db->where('credit_id', $credit_id);
        $this->db->order_by('date', 'desc');

        return $this->db->get(db_prefix() . 'credits')->row();
    }
